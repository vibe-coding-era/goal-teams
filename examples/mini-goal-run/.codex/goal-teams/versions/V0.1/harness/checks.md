...
| tasklist 可追溯 | 检查 Member Ownership | GT-011/GT-012 Architecture/Environment、GT-009/GT-010 E2E 及下游 blocked 状态可见 | `../tasklist.md` |
| SPEC 不被 Harness 替代 | 检查 spec 目录 | PRD、设计、测试计划、验收清单仍保留 | `../spec/` |
| V2.35 界面验证规则 | 检查 test-plan 和 tasklist | `sample_only` / `no_runner` 不豁免 E2E；缺可执行用例和 browser Evidence 时必须 blocked | `../tasklist.md`、`../spec/test-plan.md` |
...
5. 从 `report.md` 确认静态文档检查与整体 blocked 结论分开。
...
- 本静态 Harness 不执行外部命令，因此不能支撑 E2E passed 或整体完成声明。
