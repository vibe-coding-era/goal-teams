...
只做规划和文档产物，不修改实现文件。
过程和结果保存到 V0.1 版本目录。
必须先列出 Teams 规划表，并包含需求规格卡、PRD、架构设计、HTML 原型、测试计划、验收清单和独立校验计划。
```
...
- 多文档前是否先有总索引和版本索引。
- `plan.md` 是否先列出 Teams 规划表，且成员名是否采用 `角色-具体任务名`，例如 `后端-WIKI 列表后端开发`。
- 开始前是否先汇报 `我是 Goal Teams Leader V1.0，我会帮你完成以下工作：`。
