...
def _load_checker():
    spec = importlib.util.spec_from_file_location(
        "check_package_manifest_v265_active_closure", CHECKER
    )
...
class TestV265PackageActiveClosure(unittest.TestCase):
    @classmethod
    ...
    def _v265_cutover_fixture(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        temporary = tempfile.TemporaryDirectory(
            prefix="v265-package-active-closure-", dir=DEVELOPS
        )
        root = Path(temporary.name)
        activation = copy.deepcopy(
            json.loads((ROOT / V265_ACTIVATION_PATH).read_text(encoding="utf-8"))
        )
        ...
        activation_file = root / V265_ACTIVATION_PATH
    ...
    def test_package_selection_fails_closed_for_pre_cutover_v263_active(self) -> None:
        temporary = tempfile.TemporaryDirectory(
    ...
    def test_same_package_selection_passes_after_v265_active_cutover(self) -> None:
        temporary, root = self._v265_cutover_fixture()
        try:
            self.assertEqual(
    ...
    def test_active_activation_and_generation_digests_are_exact(self) -> None:
        ...
        try:
            ...
            activation = json.loads(
                (root / V265_ACTIVATION_PATH).read_text(encoding="utf-8")
            )
