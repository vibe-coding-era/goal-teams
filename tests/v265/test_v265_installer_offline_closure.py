...
class TestV265InstallerOfflineClosure(unittest.TestCase):
    ...
    def test_v265_uses_declared_offline_closure_without_direct_route_compilation(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            root = self._root(work, "V2.65")
            generation = {
                "generation_id": "V2.65",
            ...
            def direct(*_args: object, **_kwargs: object) -> object:
                raise AssertionError("V2.65 installer used direct route compilation")
    ...
    def test_v263_predecessor_keeps_declared_offline_route_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            ...
            def direct(*_args: object, **_kwargs: object) -> object:
                raise AssertionError("V2.63 predecessor used direct route compilation")
