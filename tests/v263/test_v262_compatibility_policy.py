...
from scripts.v263.compatibility import CompatibilityError, load_compatibility_metadata, resolve_route, validate_runtime_binding_receipt
from scripts.v263.role_projections import check_role_projections, project_role_projections
...
class TestV263CompatibilityMetadata(unittest.TestCase):
    def test_nodes_have_stable_identity_ordered_dependencies_and_explicit_states(self) -> None:
        ...
        self.assertEqual("goal-teams-compatibility-v2.63-v1", metadata["schema_version"])
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for label, mutate in mutations.items():
                with self.subTest(label=label):
                    ...
                    self.assertTrue(str(raised.exception).startswith("E_V263_"))


class TestV263Resolver(unittest.TestCase):
    @classmethod
...
class TestV263RoleProjections(unittest.TestCase):
    def _projection_root(self, directory: str) -> pathlib.Path:
        root = pathlib.Path(directory); write_json(root / "references/compatibility/v2.63/role-projections.json", fixture_json("role-projections.json"))
        ...
        roles = root / "references/compatibility/v2.63/roles"; roles.mkdir(parents=True); (roles / "goal-lead.md").write_text("canonical lead role\n", encoding="utf-8"); (roles / "goal-reviewer.md").write_text("canonical reviewer role\n", encoding="utf-8")
        return root
    ...
    def test_check_detects_missing_orphaned_and_drifted_projection(self) -> None:
        ...
        expected_errors = {"missing": "E_V263_PROJECTION_MISSING", "orphaned": "E_V263_PROJECTION_ORPHAN", "drifted": "E_V263_PROJECTION_DRIFT"}
        for label, mutate in mutations.items():
...
class TestV263RuntimeBindingReceipt(unittest.TestCase):
    @classmethod
    ...
    def test_receipts_preserve_connection_class_and_verification_state(self) -> None:
        direct = {"schema_version": "goal-teams-runtime-binding-v2.63-v1", "binding_run_id": "RUN-V263-DIRECT", "captured_at": "2026-08-07T16:00:00+08:00", **resolve_route(self.metadata, "host.codex", "provider.deepseek/flash")}
        blocked_bridge = {"schema_version": "goal-teams-runtime-binding-v2.63-v1", "binding_run_id": "RUN-V263-BRIDGE", "captured_at": "2026-08-07T16:00:00+08:00", **resolve_route(self.metadata, "host.codex", "model.kimi-k3")}
    ...
    def test_bridge_cannot_be_recorded_as_direct_or_rewrite_resolved_model(self) -> None:
        ...
        common = {"schema_version": "goal-teams-runtime-binding-v2.63-v1", "binding_run_id": "RUN-V263-FORGED", "captured_at": "2026-08-07T16:00:00+08:00"}
        ...
        for label, receipt, error in (("bridge-as-direct", forged_direct, "E_V263_RECEIPT_BRIDGE_AS_DIRECT"), ("resolved-model-rewrite", rewritten_model, "E_V263_RECEIPT_RESOLVED_MODEL_REWRITE")):
            with self.subTest(label=label):
