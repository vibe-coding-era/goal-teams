...
def load_package_checker():
    ...
    spec = importlib.util.spec_from_file_location("check_package_manifest_v26", path)
...
class TestV26DeliveryClosure(unittest.TestCase):
    def test_role_projection_plan_and_generated_assets_are_closed(self) -> None:
        verdict = check_role_projections(ROOT)
        self.assertTrue(verdict["ok"], verdict)
        self.assertEqual(
            {
                "references/compatibility/v2.6/projections/codex/subagents/goal-lead.toml",
                "references/compatibility/v2.6/projections/codex/subagents/goal-reviewer.toml",
                "references/compatibility/v2.6/projections/claude-code/.claude/agents/goal-lead.md",
                "references/compatibility/v2.6/projections/claude-code/.claude/agents/goal-reviewer.md",
            },
            set(verdict["projected_paths"]),
        )

    def test_activation_contains_v26_compatibility_execution_and_schema(self) -> None:
        ...
        for required in (
            "references/compatibility/v2.6/manifest.json",
            "references/compatibility/v2.6/role-projections.json",
            "schemas/v2.6/compatibility-manifest.schema.json",
            "scripts/v26/compatibility.py",
            "scripts/v26/role_projections.py",
            "tests/v26/test_delivery_closure.py",
        ):
            self.assertIn(required, allowlist)

    def test_default_package_is_a_valid_v26_current_closure(self) -> None:
    ...
    def test_non_readme_current_identity_surfaces_are_v26(self) -> None:
        expected = {
            "SKILL.md": ("Goal Teams V2.6",),
            ".agents/skills/goal-teams/SKILL.md": ("V2.6",),
            "RULES.md": ("V2.6",),
            "goal-teams.md": ("V2.6",),
            "AGENTS.md": ("产品版本：`V2.6`",),
            "agents/openai.yaml": ("Goal Teams V2.6",),
        }
