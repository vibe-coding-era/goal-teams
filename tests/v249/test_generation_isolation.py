...
from scripts.v249.freeze_v248_snapshot import SOURCE_PATHS, materialize
...
class TestV249GenerationIsolation(unittest.TestCase):
    ...
    def test_active_pointer_can_be_atomically_switched_to_v248_snapshot(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("V2.49", resumed["generation_id"])
    ...
    def test_active_generation_binds_all_declared_current_control_assets(self) -> None:
        ...
        self.assertTrue(set(V249_REQUIRED_CONTROL_PATHS).issubset(generation["member_digests"]))
        expected_schemas = {
            path.relative_to(REPO).as_posix()
            for path in (REPO / "schemas/v2.49").glob("*.json")
        }
        self.assertTrue(set(V249_CONTROL_SCHEMA_PATHS).issubset(expected_schemas))
        self.assertEqual(expected_schemas, set(generation["control_schemas"]))
        self.assertEqual(
            {"type": "boolean"},
            generation["control_schemas"]["schemas/v2.49/project-route.schema.json"][
                "properties"
            ]["s1_current"],
        )
    ...
    def test_bound_execution_asset_drift_fails_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for relative in required:
                ...
                target.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(source, target)
            target = root / "scripts/v249/output_contract.py"
            target.write_bytes(target.read_bytes() + b"\n# drift\n")

            with self.assertRaises(GenerationLoadError) as caught:
                load_generation(root)

            self.assertEqual("E_V249_MEMBER_DIGEST_MISMATCH", caught.exception.code)
    ...
    def test_new_unbound_dynamic_control_asset_fails_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for relative in required:
                ...
                target.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(source, target)
            unbound = root / "scripts/v249/unbound_control.py"
            ...
            self.assertEqual("E_V249_CONTROL_ASSET_UNBOUND", caught.exception.code)
