...
def _embedded_installer_namespace(work: Path) -> dict[str, object]:
    ...
    namespace: dict[str, object] = {"__name__": "goal_teams_v249_installer_test"}
    environment = {
        "HOME": str(work),
...
class TestV249PackageIsolation(unittest.TestCase):
    ...
    def test_installer_can_load_v249_prompt_identity_modules(self) -> None:
        with _workspace_tempdir("goal-teams-v249-installer-") as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            loader = runtime["load_python_module"]
            module_name = "_goal_teams_v249_generation_probe"
            try:
                module = loader(
                    module_name,
                    REPO,
                ...
                self.assertEqual(
                    REPO / "scripts/v249/generation_runtime.py",
    ...
    def test_installer_loader_rejects_escape_and_symlink_ancestor(self) -> None:
        with _workspace_tempdir("goal-teams-v249-loader-boundary-") as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            loader = runtime["load_python_module"]
            ...
            with self.assertRaises(install_error):
                loader(
                    "_goal_teams_v249_symlink_probe",
                    root,
                    "linked/probe.py",
                    ("marker",),
                )
            self.assertNotIn("_goal_teams_v249_escape_probe", sys.modules)
            self.assertNotIn("_goal_teams_v249_symlink_probe", sys.modules)

    def test_installer_loader_uses_source_bytes_not_forged_pyc(self) -> None:
        with _workspace_tempdir("goal-teams-v249-loader-pyc-") as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            loader = runtime["load_python_module"]
            root = work / "root"
            ...
            module_name = "_goal_teams_v249_pyc_probe"
            try:
    ...
    def test_installer_loader_fails_closed_on_system_exit_and_missing_symbol(self) -> None:
        with _workspace_tempdir("goal-teams-v249-loader-fail-closed-") as directory:
            work = Path(directory)
            runtime = _embedded_installer_namespace(work)
            loader = runtime["load_python_module"]
            install_error = runtime["InstallError"]
            ...
            with self.assertRaises(install_error):
                loader("_goal_teams_v249_exit_probe", root, "exit.py", ())
            with self.assertRaises(install_error):
                loader(
                    "_goal_teams_v249_missing_probe",
                    root,
                    "missing.py",
                    ("required_callable",),
                )
            self.assertNotIn("_goal_teams_v249_exit_probe", sys.modules)
            self.assertNotIn("_goal_teams_v249_missing_probe", sys.modules)

    def test_installer_prompt_identity_fails_closed_on_deferred_system_exit(self) -> None:
        with _workspace_tempdir("goal-teams-v249-deferred-exit-") as directory:
            ...
            for label, generation_module, closure_module in scenarios:
                with self.subTest(label=label):
                    runtime["load_v249_prompt_modules"] = lambda _root: (
                        generation_module,
                        closure_module,
                    )
                    with self.assertRaises(install_error):
    ...
    def test_installer_restores_prepolluted_canonical_modules(self) -> None:
        with _workspace_tempdir("goal-teams-v249-loader-isolation-") as directory:
            runtime = _embedded_installer_namespace(Path(directory))
            loader = runtime["load_v249_prompt_modules"]
            names = (
                "scripts",
                "scripts.v249",
            ...
            try:
                ...
                self.assertEqual(
                    REPO / "scripts/v249/route_closure.py",
    ...
    def test_installer_computes_v249_startup_prompt_identity(self) -> None:
        with _workspace_tempdir("goal-teams-v249-installer-") as directory:
            runtime = _embedded_installer_namespace(Path(directory))
            identity = runtime["compute_prompt_identity"](REPO)
        self.assertTrue(identity["passed"])
        self.assertEqual("goal-teams-prompt-identity-v2.49", identity["prompt_identity_version"])
        self.assertEqual("V249-ROUTE-STARTUP", identity["route_id"])

    def test_installer_runs_v249_source_validation_with_prompt_identity(self) -> None:
        with _workspace_tempdir("goal-teams-v249-installer-") as directory:
            runtime = _embedded_installer_namespace(Path(directory))
            identity = runtime["compute_prompt_identity"](REPO)
            ...
            runtime["validate_skill"](REPO, "v249_source_test")
