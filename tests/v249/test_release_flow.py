...
from scripts.v249.release_flow import (
    build_s2_receipt,
    canonical_sha256,
    derive_release_plan,
...
def public_assets() -> list[dict]:
    return [
        ...
        {"name": "goal-teams-V2.49.tar.gz", "size": 42, "sha256": "d" * 64},
    ]


class TestV249ReleaseFlow(unittest.TestCase):
    ...
    def test_s2_is_one_build_without_reproducibility_or_security_claim(self) -> None:
        ...
        self.assertEqual("not_verified_by_v249_policy", receipt["reproducibility"])
        self.assertEqual("not_run_by_v249_policy", receipt["s2_security_checks"])
    ...
    def test_stale_release_gate_binding_is_rejected(self) -> None:
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V249_RELEASE_GATE_STALE", verdict["errors"])
