...
from scripts.v249 import repository_boundary
from scripts.v249.repository_boundary import (
    ...
    def test_secret_guard_precedes_each_checkout_and_transport_assertion_follows(self) -> None:
        for path in (".github/workflows/check.yml", ".github/workflows/release-gate.yml"):
            ...
            while True:
                ...
                if checkout < 0:
                    break
                guard = workflow.rfind("E_V249_CHECKOUT_SSH_KEY_REQUIRED", cursor, checkout)
...
class TestReleaseWorkflowSequence(unittest.TestCase):
    def test_released_runtime_adapter_binds_external_handoff_and_fresh_child(self) -> None:
        ...
        self.assertIn("V249-ROUTE-LARGE-RELEASE", workflow)
        self.assertIn("V249-ROUTE-MEDIUM-RELEASE", workflow)
        self.assertIn("controller_handoff_receipt_json", workflow)
        self.assertIn("Materialize the externally issued V2.48 controller handoff", workflow)
        ...
        self.assertIn("scripts/v249/runtime_host_adapter.py verify-github-key", workflow)
        adapter = workflow.index("scripts/v249/runtime_host_adapter.py launch")
        checker = workflow.index("python3 scripts/checks/check-v249.py", adapter)
    ...
    def test_single_asset_set_flows_s2_boundary_s3_s4_without_rebuild(self) -> None:
        ...
        self.assertIn("receipt_mode", text("scripts/v249/repository_boundary.py"))
    ...
    def test_same_release_root_and_receipts_are_persisted_for_s4_continuation(self) -> None:
        ...
        for relative in (
            ...
            "release/versions/V2.49/_release.json",
            "release/versions/V2.49/_files.sha256",
            "release/versions/V2.49/_receipts/",
        ):
    ...
    def test_release_order_is_consistent_in_owner_projections(self) -> None:
        ...
        self.assertEqual("release/versions/V2.49", continuation["release_root"])
        self.assertEqual(4, continuation["public_asset_count"])
        self.assertEqual(0, continuation["downstream_rebuild_invocation_limit"])

        profile = json.loads(text("references/release-profiles/v2.49.json"))
    ...
    def test_v249_profile_loader_accepts_the_boundary_before_s3_projection(self) -> None:
        profile = release_config("V2.49")
        gates = profile["release_gates"]
        self.assertLess(gates.index("single_build"), gates.index("repository_boundary_compliance"))
        self.assertLess(gates.index("repository_boundary_compliance"), gates.index("large_release_install"))
        self.assertEqual(
...
class TestBoundaryReceiptMode(unittest.TestCase):
    ...
    def test_boundary_accepts_only_the_repository_release_versions_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (expected / "V2.49").mkdir(parents=True)
            alternate = root / "alternate"
            (alternate / "V2.49").mkdir(parents=True)
            with mock.patch.object(repository_boundary, "ROOT", root):
                self.assertEqual(
                    (expected / "V2.49").resolve(),
                    repository_boundary.resolve_release_directory(expected),
                )
                with self.assertRaisesRegex(
                    ValueError, "E_V249_REPOSITORY_BOUNDARY_RELEASE_ROOT"
                ):
    ...
    def test_reused_receipt_without_live_frozen_revalidation_fails_closed(self) -> None:
        ...
        reused["receipt_sha256"] = __import__(
            "scripts.v249.repository_boundary", fromlist=["_receipt_sha256"]
        ...
        self.assertIn("E_V249_REPOSITORY_BOUNDARY_SOURCE_NOT_FROZEN", verdict["errors"])
