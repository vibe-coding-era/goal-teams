...
class TestV249RuntimeCLI(unittest.TestCase):
    def test_release_wrapper_forwards_runtime_receipt(self) -> None:
        ...
        release_invocation = next(
            ...
            if value.startswith("scripts/checks/check-v249.py --phase release")
        )
        self.assertIn(
    ...
    def test_runtime_child_cli_forbids_raw_lineage_and_uses_stdin_receipts(self) -> None:
        result = subprocess.run(
            ["python3", "scripts/v249/runtime_transition.py", "--help"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_host_adapter_cli_has_launch_and_read_only_key_verification(self) -> None:
        result = subprocess.run(
            ["python3", "scripts/v249/runtime_host_adapter.py", "--help"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
