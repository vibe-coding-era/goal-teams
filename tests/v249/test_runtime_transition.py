...
from scripts.v249 import runtime_transition
...
ROUTE_ID = "V249-ROUTE-LARGE-RELEASE"
HANDOFF_NONCE = "nonce-v249-controller-handoff-000001"
...
def _prepare_observer_inputs(root: Path) -> tuple[Path, Path, Path]:
    ...
    current_paths = {
        "references/current/generations/V2.49/core.md": b"core\n",
        "references/current/generations/V2.49/prompt-manifest.json": b"",
        "references/profiles/goal-teams-self-release-v2.49.md": b"profile\n",
        "references/release-profiles/v2.49.json": b"{}\n",
        "references/current/generations/V2.49/contracts/release-route-manifest.json": b"{}\n",
        "references/current/generations/V2.49/contracts/release-command-manifest.json": b"{}\n",
    }
    execution_paths = {
        "scripts/checks/check-v249.py": b"checker\n",
        "scripts/v249/runtime_host_adapter.py": b"host-adapter\n",
        "scripts/v249/runtime_transition.py": b"observer\n",
    }
    schema_paths = {
    ...
    prompt = {
        "schema_version": "goal-teams-prompt-manifest-v2.49",
        "generation_id": "V2.49",
        "manifest_state": "active_current",
        "routes": {
            ROUTE_ID: {
                "workflow_phase": "release",
                "ordered_refs": ["references/current/generations/V2.49/core.md"],
            }
        },
    }
    current_paths[
    ...
    activation = {
        "schema_version": "goal-teams-activation-manifest-v2.49",
        "generation_id": "V2.49",
        "generation_state": "active",
        "identity": {
            "loaded_runtime_product_version": "V2.49",
            "route_contract_schema_version": "goal-teams-project-route-v2.49",
            "target_policy_generation": "V2.49",
        },
        "root_sets": entries,
        "prompt_manifest_path": "references/current/generations/V2.49/prompt-manifest.json",
    }
    activation["manifest_payload_sha256"] = _activation_payload_sha256(activation)
    activation_path = "references/current/generations/V2.49/activation-manifest.json"
    ...
    route = {
        "generation_id": "V2.49",
        "route_id": ROUTE_ID,
        "loaded_paths": ["references/current/generations/V2.49/core.md"],
        "path_digests": {
            "references/current/generations/V2.49/core.md": hashlib.sha256(
                b"core\n"
            ).hexdigest()
        },
        "legacy_intersection": [],
...
def _launch(handoff: dict, adapter_path: Path) -> dict:
    value = {
        ...
        "new_run_id": "V249-RUNTIME-RUN-0001",
...
class TestV249RuntimeTransition(unittest.TestCase):
    ...
    def test_released_receipt_binds_signed_handoff_launch_and_current_inputs(self) -> None:
        ...
        self.assertEqual(
            "V249-RUNTIME-RUN-0001", value["runtime_launch_receipt"]["new_run_id"]
        )
        self.assertEqual("V2.49", value["loaded_runtime_product_version"])
    ...
    def test_forged_signature_is_rejected_by_fixed_owner_verifier(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            verdict = validate_controller_handoff(
                ...
                expected_source_commit=SOURCE,
                expected_source_tree=TREE,
                expected_authorization_id="AUTH-V249-TEST",
        ...
        self.assertIn("E_V249_CONTROLLER_HANDOFF_SIGNATURE", verdict["errors"])
    ...
    def test_missing_handoff_or_launch_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            value = _observe(root)
            for field, error in (
                ("controller_handoff_receipt", "E_V249_CONTROLLER_HANDOFF_REQUIRED"),
                ("runtime_launch_receipt", "E_V249_RUNTIME_LAUNCH_REQUIRED"),
            ):
    ...
    def test_nonce_pid_parent_host_execution_and_run_drift_fail_closed(self) -> None:
        ...
        for field, replacement in drift_cases:
            with self.subTest(field=field), tempfile.TemporaryDirectory() as directory:
                ...
                verdict = self._validate(value, root)
                self.assertFalse(verdict["ok"])
                self.assertIn("E_V249_RUNTIME_LAUNCH_LINEAGE", verdict["errors"])
    ...
    def test_expired_or_identity_drifted_handoff_fails_closed(self) -> None:
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V249_CONTROLLER_HANDOFF_EXPIRED", verdict["errors"])
    ...
    def test_handoff_source_authorization_installed_state_and_signer_drift_fail(self) -> None:
        cases = (
            ...
            (
                lambda payload: payload["installed_v248_current_state"].__setitem__(
                    "tag", "v2.47"
                ),
    ...
    def test_raw_lineage_parameters_and_fields_are_forbidden(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            verdict = self._validate(value, root)
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V249_RUNTIME_TRANSITION_RAW_LINEAGE", verdict["errors"])
    ...
    def test_unknown_top_level_field_cannot_be_resealed_into_s0(self) -> None:
        ...
        self.assertIn(
            "E_V249_RUNTIME_TRANSITION_SCHEMA_FIELDS", verdict["errors"]
        )
    ...
    def test_schema_self_expansion_cannot_reseal_an_unknown_field_into_s0(self) -> None:
        ...
        self.assertFalse(verdict["may_enter_s0"])
        self.assertIn(
            "E_V249_RUNTIME_TRANSITION_MEMBER_DIGEST:"
            "schemas/v2.49/runtime-transition-receipt.schema.json",
            verdict["errors"],
        )
    ...
    def test_current_route_and_adapter_drift_still_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            value = _observe(root)
            (root / "references/current/generations/V2.49/core.md").write_text(
                "drift\n", encoding="utf-8"
            )
            verdict = self._validate(value, root)
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V249_RUNTIME_TRANSITION_CURRENT_DIGEST", verdict["errors"])
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            verdict = self._validate(value, root)
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V249_RUNTIME_TRANSITION_ADAPTER_CODE_DIGEST", verdict["errors"])
