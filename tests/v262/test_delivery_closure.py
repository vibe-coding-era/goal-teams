...
from scripts.v262.role_projections import check_role_projections
...
def load_package_checker():
    ...
    spec = importlib.util.spec_from_file_location("check_package_manifest_v262", path)
...
class TestV262DeliveryClosure(unittest.TestCase):
    def test_role_projection_plan_and_generated_assets_are_closed(self) -> None:
        verdict = check_role_projections(ROOT)
        self.assertTrue(verdict["ok"], verdict)
        self.assertEqual(
            {
                "references/compatibility/v2.62/projections/codex/subagents/goal-lead.toml",
                "references/compatibility/v2.62/projections/codex/subagents/goal-reviewer.toml",
                "references/compatibility/v2.62/projections/claude-code/.claude/agents/goal-lead.md",
                "references/compatibility/v2.62/projections/claude-code/.claude/agents/goal-reviewer.md",
            },
            set(verdict["projected_paths"]),
        )

    def test_activation_contains_v262_compatibility_execution_and_schema(self) -> None:
        ...
        for required in (
            "references/compatibility/v2.62/manifest.json",
            "references/compatibility/v2.62/role-projections.json",
            "schemas/v2.62/compatibility-manifest.schema.json",
            "scripts/v262/compatibility.py",
            "scripts/v262/role_projections.py",
            "tests/v262/test_delivery_closure.py",
        ):
            self.assertIn(required, allowlist)

    def test_default_package_is_a_valid_v262_current_closure(self) -> None:
    ...
    def test_non_readme_current_identity_surfaces_are_v262(self) -> None:
        expected = {
            "SKILL.md": ("Goal Teams V2.62",),
            ".agents/skills/goal-teams/SKILL.md": ("V2.62",),
            "RULES.md": ("V2.62",),
            "goal-teams.md": ("V2.62",),
            "AGENTS.md": ("产品版本：`V2.62`",),
            "agents/openai.yaml": ("Goal Teams V2.62",),
        }
