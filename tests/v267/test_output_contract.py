...
SHA_D = "d" * 64
SHA_E = "e" * 64
FIXTURE = "tests/v267/fixtures/dashboard"
...
def _target():
    try:
        return importlib.import_module("scripts.v267.output_dashboard")
    except ModuleNotFoundError as exc:
        if exc.name == "scripts.v267" or str(exc.name).startswith("scripts.v267."):
            raise AssertionError("E_TEST_V267_OUTPUT_TARGET_MISSING") from exc
        raise
...
class TestV267OutputContract(unittest.TestCase):
    def test_skill_and_rules_declare_v267_output_control(self) -> None:
        self.assertEqual("V2.67", (ROOT / "VERSION").read_text().strip())
        skill = (ROOT / "SKILL.md").read_text(encoding="utf-8")
        rules = (ROOT / "RULES.md").read_text(encoding="utf-8")
        task_state = (
            ROOT
            / "references/current/generations/V2.67/contracts/task-state.md"
        ).read_text(encoding="utf-8")
        output_contract = (
            ROOT
            / "references/current/generations/V2.67/contracts/output-dashboard.md"
        ).read_text(encoding="utf-8")
    ...
    def test_dashboard_only_shows_active_parent_and_subtask_rows(self) -> None:
        ...
        self.assertIn("E_V267_OUTPUT_ACTIVE_ROWS", verdict["errors"])
    ...
    def test_context_entries_are_links_and_codebase_is_project_name_only(self) -> None:
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_PROJECT_MEMORY", verdict["errors"])

        invalid = _view()
        invalid["context"]["core_rules"][0]["href"] = "{Skill链接}"  # type: ignore[index]
        verdict = target.validate_dashboard(invalid, loop_decision="continue", repo_root=ROOT)
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_LINK", verdict["errors"])
    ...
    def test_loop_heading_and_pdca_lines_are_exact(self) -> None:
        ...
        verdict = target.validate_dashboard(invalid, loop_decision="continue", repo_root=ROOT)
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_LOOP_ROUND", verdict["errors"])
    ...
    def test_unbound_or_preview_success_fails_closed(self) -> None:
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_PREVIEW_STATE", verdict["errors"])
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_BINDING", verdict["errors"])
    ...
    def test_execution_facts_match_state_evidence_and_ready_layers(self) -> None:
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_STATE_MISMATCH", verdict["errors"])
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_EVIDENCE_MISMATCH", verdict["errors"])
        ...
        self.assertFalse(verdict["ok"])
        self.assertIn("E_V267_OUTPUT_READY_LAYERS", verdict["errors"])
    ...
    def test_v267_schema_is_strict_and_matches_renderer(self) -> None:
        try:
            from jsonschema import Draft202012Validator
        except ImportError as exc:  # pragma: no cover - environment gate
            self.fail(f"E_TEST_V267_JSONSCHEMA_UNAVAILABLE:{exc}")
        schema = json.loads(
            (ROOT / "schemas/v2.67/output-dashboard.schema.json").read_text(encoding="utf-8")
        )
