...
TARGET = "V2.67"
PREDECESSOR = "V2.66"
GEN = ROOT / "references/current/generations/V2.67"
PROTECTED_READMES = {
...
class TestV267VersionCandidate(unittest.TestCase):
    ...
    def test_active_selects_v267_while_published_release_stays_predecessor(self) -> None:
    ...
    def test_v267_rule_and_prompt_close_output_owner(self) -> None:
        rule = json.loads((GEN / "rule-manifest.json").read_text())
        prompt = json.loads((GEN / "prompt-manifest.json").read_text())
        path = "references/current/generations/V2.67/contracts/output-dashboard.md"
        ...
        self.assertEqual("CONTRACT-OUTPUT-DASHBOARD-V267", owner["owner_id"])
    ...
    def test_activation_reuses_exact_v265_execution_without_legacy_overlap(self) -> None:
        ...
        self.assertIn("scripts/v267/output_dashboard.py", members)
        self.assertIn("schemas/v2.67/output-dashboard.schema.json", members)
        self.assertIn("tests/v267/test_output_contract.py", members)
        for forbidden in (
    ...
    def test_package_manifest_selects_policy_adapters_and_exact_shared_runtime(self) -> None:
        ...
        for marker in (
            "product V2.67, core policy V2.5, Graph execution contract V2.65, legacy data schema V2.3",
            "prefix references/current/generations/V2.67/",
            "prefix references/compatibility/v2.67/",
            "references/profiles/goal-teams-self-release-v2.67.md",
            "references/release-profiles/v2.67.json",
            "prefix scripts/v267/",
            "prefix schemas/v2.67/",
            "prefix tests/v267/",
        ):
    ...
    def test_profiles_and_public_projection_remain_truthful(self) -> None:
        ...
        if published["product_version"] == PREDECESSOR:
            ...
            self.assertEqual("v2.67", published["release_identity"]["tag"])
        ...
        self.assertIn("## V2.67 Development Candidate", (ROOT / "CHANGELOG.md").read_text())

    def test_generated_subagents_are_v267(self) -> None:
        paths = sorted((ROOT / "subagents").glob("goal-*.toml"))
        self.assertGreater(len(paths), 10)
        for path in paths:
            body = path.read_text(encoding="utf-8")
            self.assertIn('# common_prefix_generation = "V2.67"', body, path.name)
            self.assertIn("Goal Teams V2.67", body, path.name)
