...
class TestV263ActivationLifecycleHardening(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_activate_rejects_non_rfc3339_timestamp_before_pointer_write(self) -> None:
        ...
        prepared = self.root / "references/current/generations/V2.63/activation-manifest.json"
    ...
    def test_prepare_rejects_inconsistent_active_pointer_semantics_without_writes(self) -> None:
        protected = tuple(
            pathlib.Path(f"references/current/generations/V2.63/{name}")
            for name in (
                "rule-manifest.json",
    ...
    def test_activate_reverifies_prepared_members_immediately_before_write(self) -> None:
        ...
        def load_then_mutate(*args, **kwargs):
            ...
            if calls == 1:
                member = self.root / "references/current/generations/V2.63/core.md"
