...
def _load_checker():
    spec = importlib.util.spec_from_file_location(
        "check_package_manifest_v268_candidate_closure", CHECKER
    )
...
class TestV268PackageCandidateClosure(unittest.TestCase):
    @classmethod
    ...
    def _fixture(
        ...
        temporary = tempfile.TemporaryDirectory(
            prefix="v268-package-candidate-", dir=DEVELOPS
        )
    ...
    def test_candidate_closure_uses_explicit_identity_across_active_phases(
        ...
        try:
            ...
            self.assertIn(active["generation_id"], {"V2.67", "V2.68"})
            self.assertEqual(
                [],
