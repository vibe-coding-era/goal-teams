"""V2.63 candidate projection over the published V2.62 release."""
...
class TestV263CandidateProjection(unittest.TestCase):
    def test_release_current_is_strict_candidate_or_strict_published_projection(self) -> None:
        ...
        self.assertIn(manifest.get("product_version"), {"V2.62", "V2.63"})
        ...
        if manifest["product_version"] == "V2.62":
            self.assertEqual(
                {
            ...
            self.assertEqual("v2.62", manifest["release_identity"]["tag"])
            self.assertEqual(367112913, manifest["release_identity"]["release_id"])
            self.assertTrue(readme.startswith("# Goal Teams V2.62 Release\n"))
            self.assertIn("V2.63 is an unpublished development candidate", readme)
        else:
    ...
    def test_release_projection_is_not_an_activation_or_runtime_static_input(self) -> None:
        ...
        rule = refresh._refreshed_rule_manifest(paths, "V2.63")
        prompt = refresh._refreshed_prompt_manifest(paths, "V2.63")
