"""V2.63 OKF package-manifest runtime identity contracts."""
...
MODERN_RUNTIME_VERSIONS = ("V2.50", "V2.52", "V2.6", "V2.62", "V2.63")
ROOT = Path(__file__).resolve().parents[2]


class V263OkfPackageManifestTests(unittest.TestCase):
    def _staged_tree(
        self,
        root: Path,
        version: str,
    ...
    def test_v263_current_only_package_does_not_require_the_v23_checker(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            manifest = self._build(root, "V2.63", include_v23=False)
        ...
        for require_complete_package in (False, True):
            for version in ("V2.51", None):
                with (
                    ...
                    root = Path(td)
                    manifest = self._build(root, "V2.63")
                    if version is None:
                        (root / "VERSION").unlink()
                    else:
