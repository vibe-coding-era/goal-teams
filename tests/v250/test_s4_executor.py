...
VERSION = "V2.50"
TAG = "v2.50"
RELEASE_BODY = "Goal Teams V2.50. See release/current/README.md in the tagged source."
...
def make_release_tree(root: Path) -> tuple[Path, list[dict[str, object]]]:
    profile = root / "references/release-profiles/v2.50.json"
        ...
        json.dumps(
            {
                ...
                "status": "active",
                "tag": TAG,
                "release_title": "Goal Teams V2.50",
                "release_body": RELEASE_BODY,
                "tag_message": "Goal Teams V2.50",
            },
            sort_keys=True,
        ),
        encoding="utf-8",
...
class FakeBackend:
    ...
    def create_draft_release(
        ...
        self.release = {
            ...
            "draft": True,
            "prerelease": False,
            "html_url": "https://github.com/vibe-coding-era/goal-teams/releases/tag/v2.50",
            "assets": [],
        }
    ...
    def publish_release(self, repository: str, tag: str) -> None:
        ...
        self.release["html_url"] = (
            "https://github.com/vibe-coding-era/goal-teams/releases/tag/v2.50"
        )
...
class TestV250S4Executor(unittest.TestCase):
    ...
    def test_default_control_validator_uses_downloaded_receipt_paths(self) -> None:
        ...
        receipt_root = Path("/portable/release/versions/V2.50/_receipts")
    ...
    def test_existing_exact_draft_resumes_without_second_create(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            backend.tag = {
                ...
                "annotated": True,
                "message": "Goal Teams V2.50",
            }
            backend.release = {
                "id": 363687225,
                "tag_name": TAG,
                "name": "Goal Teams V2.50",
                "body": RELEASE_BODY,
                "draft": True,
                "prerelease": False,
                "html_url": (
...
class TestV250CommandBackend(unittest.TestCase):
    def test_tag_creation_uses_annotated_local_tag_and_ssh_origin_push(self) -> None:
        ...
        backend.create_annotated_tag(TAG, SOURCE, "Goal Teams V2.50")
        backend.push_tag(TAG, "6" * 40)

        self.assertEqual(
            ["git", "tag", "-a", TAG, SOURCE, "-m", "Goal Teams V2.50"],
            runner.argv[0],
        )
        self.assertEqual(
            ["git", "push", "origin", f"{'6' * 40}:refs/tags/v2.50"],
            runner.argv[1],
        )
    ...
    def test_every_gh_command_pins_canonical_host_and_repository(self) -> None:
        response = s4_executor.CommandResult(
            0,
            'HTTP/2.0 200 OK\n\n'
            '{"id":249,"tag_name":"v2.50","name":"Goal Teams V2.50",'
            '"body":"Goal Teams V2.50. See release/current/README.md in the tagged source.",'
    ...
    def test_annotated_tag_readback_requires_exact_message(self) -> None:
        ...
        self.assertEqual("Goal Teams V2.50", value["message"])
        self.assertEqual(
        ...
        wrong = tag_object.replace("Goal Teams V2.50\n", "wrong title\n")
        backend = s4_executor.CommandBackend(
            Path.cwd(),
            RecordingRunner(
                [
    ...
    def test_local_tag_probe_only_treats_exit_one_as_absent(self) -> None:
        ...
        self.assertIsNone(backend.read_local_tag(TAG))
        self.assertEqual(
            ["git", "show-ref", "--verify", "--quiet", "refs/tags/v2.50"],
