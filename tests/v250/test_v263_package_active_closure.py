...
def _load_checker():
    spec = importlib.util.spec_from_file_location(
        "check_package_manifest_v268_active_closure", CHECKER
    )
...
class TestV268PackageActiveClosure(unittest.TestCase):
    @classmethod
    ...
    def _v268_cutover_fixture(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        temporary = tempfile.TemporaryDirectory(
            prefix="v268-package-active-closure-", dir=DEVELOPS
        )
        root = Path(temporary.name)
        activation = copy.deepcopy(
            json.loads((ROOT / V268_ACTIVATION_PATH).read_text(encoding="utf-8"))
        )
        ...
        activation_file = root / V268_ACTIVATION_PATH
            ...
            json.dumps(
                {
                    ...
                    "generation_id": "V2.68",
                    "activation_manifest": V268_ACTIVATION_PATH,
    ...
    def test_same_package_selection_passes_after_v268_active_cutover(self) -> None:
        temporary, root = self._v268_cutover_fixture()
        try:
            self.assertEqual(
    ...
    def test_active_activation_and_generation_digests_are_exact(self) -> None:
        ...
        try:
            ...
            activation = json.loads(
                (root / V268_ACTIVATION_PATH).read_text(encoding="utf-8")
            )
    ...
    def test_repository_validation_uses_active_not_version_candidate(self) -> None:
        ...
        if active["generation_id"] in {"V2.65", "V2.67"}:
