...
def _load_checker():
    spec = importlib.util.spec_from_file_location(
        "check_package_manifest_v263_active_closure", CHECKER
    )
...
class TestV263PackageActiveClosure(unittest.TestCase):
    @classmethod
    ...
    def _v263_cutover_fixture(self) -> tuple[tempfile.TemporaryDirectory[str], Path]:
        temporary = tempfile.TemporaryDirectory(
            prefix="v263-package-active-closure-", dir=DEVELOPS
        )
        root = Path(temporary.name)
        activation = copy.deepcopy(
            json.loads((ROOT / V263_ACTIVATION_PATH).read_text(encoding="utf-8"))
        )
        ...
        activation_file = root / V263_ACTIVATION_PATH
            ...
            json.dumps(
                {
                    ...
                    "generation_id": "V2.63",
                    "activation_manifest": V263_ACTIVATION_PATH,
    ...
    def test_package_selection_fails_closed_for_pre_cutover_v262_active(self) -> None:
        temporary = tempfile.TemporaryDirectory(
            prefix="v262-package-active-closure-", dir=DEVELOPS
        )
        try:
            root = Path(temporary.name)
            active_file = root / ACTIVE_PATH
    ...
    def test_same_package_selection_passes_after_v263_active_cutover(self) -> None:
        temporary, root = self._v263_cutover_fixture()
        try:
            self.assertEqual(
    ...
    def test_active_activation_and_generation_digests_are_exact(self) -> None:
        ...
        try:
            ...
            activation = json.loads(
                (root / V263_ACTIVATION_PATH).read_text(encoding="utf-8")
            )
