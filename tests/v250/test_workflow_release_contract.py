...
class TestReleaseWorkflowSequence(unittest.TestCase):
    def test_released_runtime_adapter_binds_external_handoff_and_fresh_child(self) -> None:
        ...
        self.assertIn("Materialize the externally issued V2.50 controller handoff", workflow)
    ...
    def test_same_release_root_and_receipts_are_persisted_for_s4_continuation(self) -> None:
        ...
        for relative in (
            "release/versions/V2.51/_artifacts/goal-teams-V2.51.tar.gz",
            "release/versions/V2.51/_artifacts/SHA256SUMS",
            "release/versions/V2.51/_release.json",
            "release/versions/V2.51/_files.sha256",
            "release/versions/V2.51/_receipts/",
        ):
        ...
        self.assertIn("path: release/versions/V2.51/_diagnostics/", workflow)
    ...
    def test_release_order_is_consistent_in_owner_projections(self) -> None:
        ...
        self.assertEqual("release/versions/V2.51", continuation["release_root"])
        self.assertEqual(4, continuation["public_asset_count"])
        self.assertEqual(0, continuation["downstream_rebuild_invocation_limit"])

        profile = json.loads(text("references/release-profiles/v2.51.json"))
...
class TestBoundaryReceiptMode(unittest.TestCase):
    ...
    def test_boundary_accepts_only_the_repository_release_versions_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (expected / "V2.51").mkdir(parents=True)
            alternate = root / "alternate"
            (alternate / "V2.51").mkdir(parents=True)
            with mock.patch.object(repository_boundary, "ROOT", root):
                self.assertEqual(
                    (expected / "V2.51").resolve(),
