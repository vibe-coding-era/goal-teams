...
def _prepare_observer_inputs(root: Path) -> tuple[Path, Path, Path]:
    ...
    current_paths = {
        "references/current/generations/V2.6/core.md": b"core\n",
        "references/current/generations/V2.6/prompt-manifest.json": b"",
        "references/profiles/goal-teams-self-release-v2.6.md": b"profile\n",
        "references/release-profiles/v2.6.json": b"{}\n",
        "references/current/generations/V2.6/contracts/release-route-manifest.json": b"{}\n",
        "references/current/generations/V2.6/contracts/release-command-manifest.json": b"{}\n",
    }
    execution_paths = {
    ...
    prompt = {
        ...
        "routes": {
            ROUTE_ID: {
                ...
                "ordered_refs": ["references/current/generations/V2.6/core.md"],
            }
        },
    }
    current_paths[
    ...
    activation = {
        ...
        "root_sets": entries,
        "prompt_manifest_path": "references/current/generations/V2.6/prompt-manifest.json",
    }
    activation["manifest_payload_sha256"] = _activation_payload_sha256(activation)
    activation_path = "references/current/generations/V2.6/activation-manifest.json"
    ...
    route = {
        ...
        "path_digests": {
            "references/current/generations/V2.6/core.md": hashlib.sha256(
                b"core\n"
            ).hexdigest()
        },
        "legacy_intersection": [],
...
class TestV250RuntimeTransition(unittest.TestCase):
    ...
    def test_released_receipt_binds_signed_handoff_launch_and_current_inputs(self) -> None:
        ...
        self.assertEqual("V2.6", value["loaded_runtime_product_version"])
    ...
    def test_handoff_source_authorization_installed_state_and_signer_drift_fail(self) -> None:
        cases = (
            ...
            (
                lambda payload: payload["installed_v252_current_state"].__setitem__(
                    "tag", "v2.47"
                ),
    ...
    def test_current_route_and_adapter_drift_still_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            value = _observe(root)
            (root / "references/current/generations/V2.6/core.md").write_text(
                "drift\n", encoding="utf-8"
            )
            verdict = self._validate(value, root)
        self.assertFalse(verdict["ok"])
