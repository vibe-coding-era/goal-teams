...
from scripts.v250.generation_runtime import GenerationLoadError
from scripts.v250.replay_runner import load_replay_manifest
...
class TestV250ReplayBinding(unittest.TestCase):
    ...
    def test_replay_manifest_and_allowlist_cannot_be_rewritten_without_active_update(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                "scripts.v250.replay_runner.load_generation", return_value=active_binding
            ):
                with self.assertRaises(GenerationLoadError) as caught:
                    load_replay_manifest(root)

            self.assertEqual("E_V250_REPLAY_ACTIVE_BINDING", caught.exception.code)
