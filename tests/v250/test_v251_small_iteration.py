...
class TestV251SmallIteration(unittest.TestCase):
    ...
    def test_product_identity_and_current_generation_are_v268(self) -> None:
        self.assertEqual("V2.68", (ROOT / "VERSION").read_text(encoding="utf-8").strip())
        active = json.loads(
    ...
    def test_mit_license_grants_open_source_rights(self) -> None:
        license_text = (ROOT / "LICENSE").read_text(encoding="utf-8")
        self.assertIn("MIT License", license_text)
        self.assertIn("Permission is hereby granted, free of charge", license_text)
        self.assertIn("肉山@TGO 杭州", license_text)
        self.assertNotIn("prior written permission", license_text)
        self.assertNotIn("not an Open Source license", license_text)

        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        readme_en = (ROOT / "README.en.md").read_text(encoding="utf-8")
        self.assertIn("MIT License", readme)
        self.assertIn("MIT License", readme_en)
