"""Shared immutable identities and modules for V2.67 Current release tests."""
...
from scripts.v267 import release_flow, runtime_transition, s4_executor
from scripts.v267.repository_boundary import build_boundary_receipt
...
TARGET_VERSION = "V2.67"
TARGET_TAG = "v2.67"
TARGET_BRANCH = "codex/develop-v2.67"
PREDECESSOR_VERSION = "V2.66"
PREDECESSOR_TAG = "v2.66"
GENERATION_ROOT = ROOT / "references/current/generations/V2.67"
RELEASE_SCHEMA_ROOT = ROOT / "schemas/v2.67"
RELEASE_PROFILE_PATH = ROOT / "references/release-profiles/v2.67.json"
SOURCE = "1" * 40
TREE = "2" * 40
...
HANDOFF_NONCE = "nonce-v267-controller-handoff-000001"
...
class InactiveCandidateFixture:
    """Build a phase-independent V2.67 candidate over active V2.66."""
    ...
    def __enter__(self) -> CandidateFixture:
        ...
        self._temporary = tempfile.TemporaryDirectory(
            prefix="v267-inactive-candidate-", dir=develops
        )
        ...
        predecessor_path = fixture_root / V266_ACTIVATION_PATH
        ...
        if result.returncode != 0:
            raise AssertionError(
                "failed to build V2.67 inactive candidate fixture: "
                + hashlib.sha256(result.stderr.encode("utf-8")).hexdigest()
                + ": "
                + result.stderr.strip()
            )

        activation_path = fixture_root / V267_ACTIVATION_PATH
        ...
        return CandidateFixture(
            root=fixture_root,
            activation_path=V267_ACTIVATION_PATH,
...
def _prepare_v267_observer_inputs(root: Path) -> tuple[Path, Path, Path, Path, Path]:
    bootstrap_paths = {
    ...
    core_path = "references/current/generations/V2.67/core.md"
    core_raw = b"core\n"
    facts_source = {
        "schema_version": "goal-teams-project-route-facts-source-v2.67",
    ...
    derived_route = derive_route(project_route_facts, generation_id="V2.67")
    owner = {
        "owner_id": "core",
        "path": core_path,
    ...
    rule_manifest_path = (
        "references/current/generations/V2.67/rule-manifest.json"
    )
    current_paths = {
        core_path: core_raw,
        rule_manifest_path: _runtime_json_bytes(rule_manifest),
        "references/current/generations/V2.67/prompt-manifest.json": b"",
        "references/profiles/goal-teams-self-release-v2.67.md": b"profile\n",
        "references/release-profiles/v2.67.json": b"{}\n",
        "references/current/generations/V2.67/contracts/release-route-manifest.json": b"{}\n",
        "references/current/generations/V2.67/contracts/release-command-manifest.json": b"{}\n",
        "references/current/generations/V2.67/contracts/predecessor-release-identity.json": _runtime_json_bytes(
            {
                "schema_version": "goal-teams-predecessor-release-identity-v2.67",
                "generation_id": "V2.67",
                "predecessor_product_version": "V2.66",
                "release_identity": PUBLISHED_V266_IDENTITY,
    ...
    execution_paths = {
        "scripts/checks/check-v267.py": b"checker\n",
        "scripts/v267/release_identity.py": b"release-identity\n",
        "scripts/v267/runtime_host_adapter.py": b"host-adapter\n",
        "scripts/v267/runtime_transition.py": b"observer\n",
    }
    schema_paths = {
    ...
    # of the V2.67 runtime or predecessor-controller identity closure.
    _runtime_write(
        root,
    ...
    activation = {
        ...
        "identity": {
            ...
            "target_policy_generation": "V2.67",
        },
        "root_sets": entries,
        "rule_manifest_path": rule_manifest_path,
        "prompt_manifest_path": "references/current/generations/V2.67/prompt-manifest.json",
    ...
    activation_path = "references/current/generations/V2.67/activation-manifest.json"
...
def _v267_handoff(authorization_path: Path) -> dict:
    ...
    signed_payload = {
        ...
        "installed_v266_current_state": {
            "source_commit": PUBLISHED_V266_IDENTITY["source_commit"],
            "source_tree": PUBLISHED_V266_IDENTITY["source_tree"],
            "tag": PUBLISHED_V266_IDENTITY["tag"],
            "release_id": PUBLISHED_V266_IDENTITY["release_id"],
        },
    ...
    installed = signed_payload["installed_v266_current_state"]
...
def _v267_launch(handoff: dict, adapter_path: Path) -> dict:
    value = {
        ...
        "new_run_id": "V267-RUNTIME-RUN-0001",
...
def observe_runtime_transition(root: Path) -> dict:
    (
        ...
        authorization_path,
        adapter_path,
    ) = _prepare_v267_observer_inputs(root)
    handoff = _v267_handoff(authorization_path)
    launch = _v267_launch(handoff, adapter_path)
