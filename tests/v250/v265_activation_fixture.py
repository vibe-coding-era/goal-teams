"""Phase-independent V2.65 activation lifecycle test fixture.

The source repository may already point at V2.65.  Lifecycle tests must still
exercise the V2.63 -> prepared V2.65 -> active V2.65 transition, so every test
...
V263_ACTIVATED_AT = "2026-08-13T19:07:54+08:00"
...
class V265ActivationFixture:
    """Own one isolated repository with an explicit V2.63 base identity."""
    ...
    def restore_v263_base(self) -> bytes:
        ...
        if (
            ...
            raise AssertionError("V2.63 fixture activation identity is invalid")

        activation_target = self.root / V263_ACTIVATION
        ...
        if (
            ...
            raise AssertionError("V2.63 fixture exact readback failed")
    ...
    def prepare_v265(self) -> subprocess.CompletedProcess[str]:
        """Rebuild prepared V2.65 assets from the explicit V2.63 base."""

        self.restore_v263_base()
        ...
        if result.returncode == 0:
            prepared_raw = (self.root / V265_ACTIVATION).read_bytes()
            prepared = json.loads(prepared_raw.decode("utf-8"))
            if (
                prepared.get("generation_id") != "V2.65"
                or prepared.get("generation_state") != "active"
                or (self.root / ACTIVE).read_bytes() != self.base_active_raw
            ):
                raise AssertionError("V2.65 prepared fixture identity is invalid")
