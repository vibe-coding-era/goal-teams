"""V2.66 release-only identity and manifest closure Red denominator."""
...
class TestV266ReleaseManifestClosure(unittest.TestCase):
    def _json(self, relative: str) -> dict[str, object]:
        path = ROOT / relative
        self.assertTrue(path.is_file(), f"E_TEST_V266_RELEASE_FILE_MISSING:{relative}")
    ...
    def test_release_profile_and_predecessor_identity_are_registered(self) -> None:
        profile_path = ROOT / "references/release-profiles/v2.66.json"
        self.assertTrue(
            profile_path.is_file(), "E_TEST_V266_RELEASE_PROFILE_MISSING"
        )
        try:
            profile = release_config.release_config(TARGET)
        except ValueError as exc:
            self.fail(f"E_TEST_V266_RELEASE_PROFILE_UNREGISTERED:{exc}")
        ...
        self.assertEqual(PUBLISHED_V265_IDENTITY, predecessor["release_identity"])
        self.assertEqual(
            _canonical_sha256(PUBLISHED_V265_IDENTITY),
            predecessor["release_identity_sha256"],
        )

    def test_release_manifests_define_exact_v266_s0_s4_contract(self) -> None:
        public = self._json(
        ...
        self.assertEqual(V266_ASSETS, [item["name"] for item in public["assets"]])
        self.assertEqual("release/versions/V2.66", public["source_root"])
        ...
        self.assertEqual(["tests/v250", "tests/v266"], denominator["test_roots"])
        self.assertEqual(
    ...
    def test_current_projection_is_v266_candidate_or_final_without_prebuild(self) -> None:
        ...
        if current.get("product_version") == PREDECESSOR:
            ...
            self.assertEqual(PUBLISHED_V265_IDENTITY, current["release_identity"])
        else:
            self.assertEqual(TARGET, current.get("product_version"))
            self.assertEqual(
                "goal-teams-release-manifest-v2.66", current.get("schema_version")
            )
            self.assertEqual("v2.66", current["release_identity"]["tag"])
            self.assertEqual("published", current["release_identity"]["state"])

        self.assertFalse(
            (ROOT / "release/versions/V2.66").exists(),
            "Development must not prebuild the formal V2.66 S2 snapshot",
        )
