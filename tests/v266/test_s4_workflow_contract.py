"""V2.66 S2-S4 and phase-aware workflow Red denominator."""
...
class TestV266S4WorkflowContract(unittest.TestCase):
    def _module(self, name: str) -> ModuleType:
        try:
            return importlib.import_module(name)
        except ModuleNotFoundError as exc:
            self.fail(f"E_TEST_V266_MODULE_MISSING:{name}:{exc}")
    ...
    def test_s2_single_build_s3_large_only_and_s4_identity(self) -> None:
        flow = self._module("scripts.v266.release_flow")
        s4 = self._module("scripts.v266.s4_executor")
        ...
        self.assertEqual("v2.66", s4.TAG)
        self.assertEqual("Goal Teams V2.66", s4.TITLE)
        self.assertEqual(ASSETS, s4.CANONICAL_ASSET_NAMES)
        self.assertEqual(
            Path("references/release-profiles/v2.66.json"),
            s4.RELEASE_PROFILE_RELATIVE,
        )
        self.assertTrue(s4.CANONICAL_RELEASE_URL.endswith("/tag/v2.66"))

    def test_v265_predecessor_cannot_reuse_v266_current_release_flow(self) -> None:
    ...
    def test_workflows_are_v266_and_keep_external_s4_outside_actions(self) -> None:
        check = (ROOT / ".github/workflows/check.yml").read_text(encoding="utf-8")
        release = (ROOT / ".github/workflows/release-gate.yml").read_text(
            encoding="utf-8"
        )

        self.assertIn("Active V2.66 pre-release exact Development gate", check)
        self.assertIn("--generation-id V2.66", check)
        self.assertIn("tests.v266.", check)
        self.assertNotIn("tests.v265.", check)

        self.assertIn("Goal Teams V2.66 phase-aware verification", release)
        self.assertIn("codex/develop-v2.66", release)
        self.assertIn("Installed V2.65 host-issued V2.66 controller handoff", release)
        self.assertIn('"V2.66"', release)
        self.assertIn("--version V2.66", release)
        self.assertIn("release/versions/V2.66", release)
        self.assertIn("goal-teams-V2.66.tar.gz", release)
        ...
        self.assertNotIn("scripts/v266/s4_executor.py execute", release)
        self.assertNotIn("tests.v265.", release)
