"""V2.66 fresh runtime transition and S0 predecessor identity Red denominator."""
...
class TestV266ReleaseRuntimeTransition(unittest.TestCase):
    def _runtime(self) -> ModuleType:
        try:
            return importlib.import_module("scripts.v266.runtime_transition")
        except ModuleNotFoundError as exc:
            self.fail(f"E_TEST_V266_RUNTIME_MODULE_MISSING:{exc}")
    ...
    def _handoff(
        ...
        payload = {
            ...
            "nonce": "nonce-v266-controller-handoff-000001",
            "issued_at": "2026-08-26T08:00:00+00:00",
            "expires_at": "2026-08-26T08:10:00+00:00",
            "installed_v265_current_state": self._installed_state(runtime, identity),
    ...
    def test_runtime_and_schema_bind_installed_v265_to_loaded_v266(self) -> None:
        ...
        self.assertEqual("v2.65", runtime.PREVIOUS_CONTROLLER_RELEASE_TAG)
        self.assertEqual(
            "goal-teams-v2.66-controller-handoff", runtime.HANDOFF_SIGNATURE_NAMESPACE
        )
        self.assertEqual(
            "references/current/generations/V2.66/contracts/"
            "predecessor-release-identity.json",
        ...
        schema_path = ROOT / "schemas/v2.66/runtime-transition-receipt.schema.json"
        self.assertTrue(schema_path.is_file(), "E_TEST_V266_RUNTIME_SCHEMA_MISSING")
        ...
        self.assertIn("installed_v265_current_state", payload["required"])
        self.assertNotIn("installed_v263_current_state", payload["properties"])
        self.assertEqual(
            {"const": "v2.65"},
            payload["properties"]["installed_v265_current_state"]["properties"][
                "tag"
            ],
        )
        self.assertEqual(
    ...
    def test_exact_published_v265_handoff_passes_signature_boundary(self) -> None:
        runtime = self._runtime()
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._write_predecessor_contract(runtime, root, PUBLISHED_V265_IDENTITY)
            handoff = self._handoff(runtime, PUBLISHED_V265_IDENTITY)
    ...
    def test_resealed_v263_or_tampered_v265_controller_is_rejected(self) -> None:
        runtime = self._runtime()
        for case in ("v263", "release_id", "source_commit", "state_sha256"):
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                ...
                installed = payload["installed_v265_current_state"]
                if case == "v263":
