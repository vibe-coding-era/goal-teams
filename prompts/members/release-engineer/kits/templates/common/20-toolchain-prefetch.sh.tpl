...
TOOLCHAIN_ACTION={{TOOLCHAIN_PREFETCH_ACTION_Q}}
TOOLCHAIN_RECEIPT={{TOOLCHAIN_PREFETCH_RECEIPT_Q}}
...
/usr/bin/python3 -c 'import json,sys; p,action,plan,host,manifest,execution=sys.argv[1:]; d=json.load(open(p,encoding="utf-8")); fields={"action_id","action_manifest_sha256","dependency_bundle_digest","execution_id","full_test_execution_count","host_attestation","host_executable_sha256","network_policy","observed_at","plan_digest","schema_version","status"}; digest=lambda v:isinstance(v,str) and len(v)==64 and all(c in "0123456789abcdef" for c in v); ok=set(d)==fields and d["schema_version"]=="goal-teams-toolchain-action-receipt-v2.45" and d["action_id"]==action and d["status"]=="passed" and d["execution_id"]==execution and d["plan_digest"]==plan and d["host_executable_sha256"]==host and d["action_manifest_sha256"]==manifest and d["network_policy"]=="prefetch_only" and d["full_test_execution_count"]==0 and digest(d["dependency_bundle_digest"]) and isinstance(d["host_attestation"],dict) and isinstance(d["observed_at"],str) and bool(d["observed_at"]); raise SystemExit(0 if ok else "invalid toolchain prefetch receipt")' "$TOOLCHAIN_RECEIPT" "$TOOLCHAIN_ACTION" "$GOAL_TEAMS_RELEASE_PLAN_DIGEST" "$TOOLCHAIN_HOST_SHA256" "$ACTION_MANIFEST_SHA256" "$GOAL_TEAMS_RELEASE_EXECUTION_ID"
