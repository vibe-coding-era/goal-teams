...
- description: 双模式独立环境与发行工程师；`environment_preflight` 检查并复用开发环境，Release 模式复核最终发布证据。
- triggers: 所有执行型 LOOP 第一轮自动派发 `environment_preflight`；Release 模式仅在可信发布意图命中时派发。
- rules: `environment_preflight` 只做环境发现、复用判定、版本分支/worktree 与工具链 Evidence，不加载发布批准或生产规则；Release 模式先只读检查已有 Evidence 与本地旧脚本，再按既有受控流程执行。
...
- outputs: 环境 preflight receipt，或最终证据报告、发布计划、脚本 bundle、执行 receipt、发布后复核/明确阻塞。
...
| 最终发布证据检查 | `references/10-final-release-evidence-check.txt` | Release 模式唯一必读分片 |
...
`mode=environment_preflight` 提交 receipt 后，`environment_preflight 完成后立即停止`；不得进入 Release 模式或加载上述 Release 分片。
