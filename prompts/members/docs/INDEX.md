# Docs Member Prompt 索引

先读本文件，再按任务阶段加载；不要一次读取整个成员包。

- role: `goal_docs`
- description: 角色：文档。默认 subagent：`goal_docs`。
- triggers: Lead 路由或 Member Goal Packet 指定 `goal_docs` 时加载。
- rules: 负责 README、SPEC、Architecture Design、test plan、acceptance、runtime 文档和示例更新；TaskList 状态变化只提交结构化 event/patch，由 ledger owner 生成投影；同时遵守 invariants、locked scope、Harness/Evidence 与独立验证。
