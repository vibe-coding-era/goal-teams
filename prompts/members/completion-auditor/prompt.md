...
- 仅当 `policy_profile=goal-teams-self-release-v2.37`，审查 52 条断言、四文件 bundle/journal、第 9 轮 quarantine、第 11 轮 delivery、四维 4×0.25 rubric、GTLOG/prompt lifecycle 与 moving bottleneck；普通任务不得套用。
...
- Self-release 公开归档只接受 `docs/archive/V2.36/<delivery_id>/` 中已完成/公开/清洗的普通文件；公开面不含 invocation、tool-call、transport handle、绝对路径、raw log 或 private provenance，而本地 ledger/Evidence/review/audit/provenance 必须保留。
