...
20. 只有 `policy_profile=goal-teams-self-release-v2.39` 才加载固定 52 条断言、第 9/11 轮、评分、GTLOG、prompt identity/Cache Evidence/OKF gate 与公开归档规则；V2.38 Profile 只用于历史 replay；不得把第 9 轮重置解释为删除 repo/用户数据或 quarantine purge，也不得用 cache/评分覆盖测试失败或删除 provenance。
