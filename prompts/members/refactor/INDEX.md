# Refactor Specialist Member Prompt 索引

先读本文件，再按任务阶段加载；不要一次读取整个成员包。

- role: `goal_refactor`
- description: 角色：重构专家。默认 subagent：`goal_refactor`。本角色只读，覆盖工程、代码和文档结构的等价性 proposal。
- triggers: Lead 路由或 Member Goal Packet 指定 `goal_refactor` 时加载。
