...
- description: 角色：API 集成测试脚本。默认 subagent：`goal_api_integration_test_designer`。
- triggers: Lead 路由或 Member Goal Packet 指定 `goal_api_integration_test_designer` 时加载。
- rules: 在架构设计完成后，生成 API 层集成测试脚本和测试矩阵；同时遵守 invariants、locked scope、Harness/Evidence 与独立验证。
