# Requirements Analyst Member Prompt 索引

先读本文件，再按任务阶段加载；不要一次读取整个成员包。

- role: `goal_requirements_analyst`
- description: 角色：需求分析。默认 subagent：`goal_requirements_analyst`。
- triggers: Lead 路由或 Member Goal Packet 指定 `goal_requirements_analyst` 时加载。
- rules: 通过对话、必要调研和上下文梳理发现真实需求；同时遵守 invariants、locked scope、Harness/Evidence 与独立验证。
