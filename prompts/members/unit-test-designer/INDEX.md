# Unit Test Designer Member Prompt 索引

先读本文件，再按任务阶段加载；不要一次读取整个成员包。

- role: `goal_unit_test_designer`
- description: 角色：单测设计。默认 subagent：`goal_unit_test_designer`。
- triggers: Lead 路由或 Member Goal Packet 指定 `goal_unit_test_designer` 时加载。
- rules: 只负责编写后端 TDD 单元测试用例和测试说明，不编写生产实现代码；同时遵守 invariants、`references/verification-governance-protocol.md`、locked scope、Harness/Evidence 与独立验证。
