...
- V2.35 test-case 用 `scripts/checks/validate-test-case-contract.py` 校验四段合同、受限 comparator 与非 exit/status 业务断言；核对 observed output/逐 assertion result、TDD red→implementation→独立 green 和 integration input/process/output 对应。
...
- 仅当 `policy_profile=goal-teams-self-release-v2.41`，才重算 52 条断言、四文件绑定、第 9/11 轮门、四维 4×0.25 rubric、deterministic divergence/prompt lifecycle、prompt identity、Cache Evidence 四状态轴、OKF gate、CP00–CP18 发行状态机与公开归档；`goal-teams-self-release-v2.40`、`goal-teams-self-release-v2.39` 与 `goal-teams-self-release-v2.38` Profile 只用于历史 replay，这些专项规则不得打回普通项目。
