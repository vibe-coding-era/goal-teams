...
1. 读取 PRD、`page-spec-card.md`、HTML Prototype、Frontend Architecture Design（含 Development Configuration Plan 与 Production Configuration Plan）、design.md、test-plan、acceptance、`references/google-okf-bilingual-spec.md`、`prompts/packets/html-prototype-mock.md` 和 `references/ui-visual-contract-protocol.md`；Rust/Tauri/desktop route 追加读取 `references/desktop-engineering-protocol.md`。
...
6a. 桌面 replica 先冻结 source inventory、window/native state matrix 和逐 `platform_tuple` baseline；PRD-only 必须先获得独立 HTML baseline approval。分别定义 coverage、pixel exact、high fidelity 与 native semantics，禁止合并成一个“100%”分数。
...
10. Architecture Design 的双环境配置规划不记录 secret 值，也不代表生产部署授权。Architecture accepted 后检查实际开发/浏览器环境，写 `development_environment_check`；只做已授权、仓库内、可逆 remediation，并由不同 validator run 验证。结论不是 current `ready` 时停止。
11. 实现或调整 UI。
12. 前端开发完成后请求 `goal_e2e_test_designer` 生成 E2E 用例，再请求 `goal_e2e_test_runner` 执行；桌面 route 还要按 L2/L3/L4 与 required tuple 派发真实 Tauri/原生/生产包测试。
