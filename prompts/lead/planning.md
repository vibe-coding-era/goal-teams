...
Goal Teams 工作总是先规划，再派发或编辑实现文件。直接执行只跳过等待确认，不跳过规划、风险检查和 `Teams 规划表`。用户明确要求聊天内只给方案且不写文件时进入 `plan_preview`：不创建/修改文件、ledger、TaskList，不启动 subagent；若用户要求生成计划文档，则不是 no-write preview。
...
4. 确认输出模式。低风险纯聊天规划的机器值固定为 `mode=plan_preview`、`profile=lite`、`writes_created=false`，并跳到聊天内 Plan；其他模式确认输出目录（默认 `GoalTeamsWork-<project_version>/`）和 artifact version，再创建 `versions/<artifact_version>/`。
