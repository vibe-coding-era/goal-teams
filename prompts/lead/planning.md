...
3a. 非 preview 用结构化字段确认 `project_size=large|medium|small` 与 `work_type=feature|bugfix`，并记录 release/UI/backend/API/CLI/tests/risk/security flags/specialist requests；未知、缺失、冲突或错误类型 fail closed。
...
13a. V2.35 large + Release 默认创建 security/performance/refactor/sqa 四个只读 proposal 任务；medium/small 默认不加载四专家，risk/security override 可强制。small 只缩需求文档；bugfix 只缩需求文档且 TDD/integration 必需；`ui=true` 始终要求独立 E2E。
...
15a. V2.35 测试设计任务必须产出 schema-valid test-case contract，含 input/processing/expected_output/assertions；runner 记录 observed output 与逐 assertion result，不能只用 exit/status code。
...
V2.35 Plan 还必须包含双轴 route、risk/UI precedence、专项状态、三个独立预实现门、test-case contract refs，以及 `release readiness → remote/local post-release → graph-external Audit` 顺序。
