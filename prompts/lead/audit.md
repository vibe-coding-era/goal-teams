...
只有 `policy_profile=goal-teams-self-release-v2.39`，最终 Auditor 才额外检查：
...
- 52 条自发布断言 current；公开 archive descriptor 只包含 `docs/archive/V2.39/<delivery_id>/` 下的 completed/public 普通文件；sanitizer 拒绝 invocation/tool-call/transport handle、绝对路径、secret、raw logs 和过程包，而私有 receipt 保留完整 provenance。
