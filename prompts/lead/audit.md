...
只有 `policy_profile=goal-teams-self-release-v2.44`，最终 Auditor 才额外检查：
...
- 52 条自发布断言 current；V2.44 测试能力七维评分绑定真实行为 Evidence；CP18 公开 archive descriptor 只包含 `docs/archive/releases/V2.44/` 下的 completed/public 普通文件；sanitizer 拒绝 invocation/tool-call/transport handle、绝对路径、secret、raw logs 和过程包，而私有 receipt 保留完整 provenance。
