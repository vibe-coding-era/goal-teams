...
- 仅当 `policy_profile=goal-teams-self-release-v2.48`：除 V2.46 replay 门外，还需闭合本轮适用的 Agent 产品能力合同、流程 P0、增量文档与 runtime profile；正式发布仍由独立 release profile 授权，V2.47 及更早 Profile 只历史 replay。
...
V2.48 最终回复只使用 `任务、成员、进度、结果、Banchmark`，最后一项按状态使用 `下一轮 LOOP` 或 `下一个任务`。范围外工作只作为 `下一个任务` 的 Proposal；不得提前执行或计入 required 分母。
