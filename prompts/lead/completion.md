...
- 仅当 `policy_profile=goal-teams-self-release-v2.39`：52 条断言 current，四文件一致，第 9 轮 reset 只隔离预授权 disposable candidate，第 11 轮才 delivery/achieved，四维评分不覆盖测试，并且 sanitizer 后副本只进入 `docs/archive/V2.39/<delivery_id>/`、私有 provenance 完整保留。
- V2.39 cache 任务分别记录 structural/host/live/request-hit 四状态轴、route-static identity、manifest status/scope、宿主可用时的 runtime digests、observer parser/identity/coverage 与 raw hash；无授权 live probe 为 `not_authorized`，无 request 事件时 hit rate 为 null/unavailable，cache 指标不得替代完成 Evidence。
