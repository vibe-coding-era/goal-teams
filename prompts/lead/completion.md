...
- 仅当 `policy_profile=goal-teams-self-release-v2.37`：52 条断言 current，四文件一致，第 9 轮 reset 只隔离预授权 disposable candidate，第 11 轮才 delivery/achieved，四维评分不覆盖测试，并且 sanitizer 后副本只进入 `docs/archive/V2.36/<delivery_id>/`、私有 provenance 完整保留。
