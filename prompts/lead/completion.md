...
- 仅当 `policy_profile=goal-teams-self-release-v2.44`：52 条断言 current，四文件一致，第 9 轮 reset 只隔离预授权 disposable candidate，第 11 轮才 delivery/achieved，四维评分不覆盖测试，V2.44 测试能力 100 分绑定七维 required checks、问题账本和真实行为 Evidence，并且 sanitizer 后副本只进入 `docs/archive/V2.44/<delivery_id>/`、私有 provenance 完整保留；`goal-teams-self-release-v2.43`、`goal-teams-self-release-v2.42`、`goal-teams-self-release-v2.41`、`goal-teams-self-release-v2.40`、`goal-teams-self-release-v2.39` 与 `goal-teams-self-release-v2.38` 只用于历史 replay。
- V2.44 cache 任务分别记录 structural/host/live/request-hit 四状态轴、route-static identity、manifest status/scope、宿主可用时的 runtime digests、observer parser/identity/coverage 与 raw hash；Tokens/Cache 命中率缺可信 usage Evidence 时写 `未获取到` / `Unavailable`，无授权 live probe 为 `not_authorized`，cache 指标不得替代完成 Evidence。
