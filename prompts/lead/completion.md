...
- 已按 `references/engineering-metrics-protocol.md` 生成当前 run 的 `versions/<artifact_version>/metrics/engineering-metrics.md`；它是可解析的 OKF Concept Document，包含四列完整指标表以及十二项指标的公式、分子、分母、排除项、历史聚合、可用状态和 Evidence refs，且算法 digest 与 `references/engineering-metrics-manifest.json` 一致。
- 工程指标只作观测，不替代 `SPEC -> Harness -> Evidence -> Audit`。缺少可信遥测或观察窗未结束时保留 `unavailable`、`pending`、`not_applicable` 或 `insufficient_sample` 及原因，不得写成零；这些状态本身不阻塞业务 Done Criteria。
...
- 仅当 `policy_profile=goal-teams-self-release-v2.45`：52 条断言 current，四文件一致，第 9 轮 reset 只隔离预授权 disposable candidate，第 11 轮才 delivery/achieved，四维评分不覆盖测试，V2.44 测试能力 100 分作为兼容合同绑定七维 required checks、问题账本和真实行为 Evidence，独立 Release Engineer 未进入主路由，并且 sanitizer 后副本只进入 `docs/archive/releases/V2.45/`、私有 provenance 完整保留；`goal-teams-self-release-v2.44`、`goal-teams-self-release-v2.43`、`goal-teams-self-release-v2.42`、`goal-teams-self-release-v2.41`、`goal-teams-self-release-v2.40`、`goal-teams-self-release-v2.39` 与 `goal-teams-self-release-v2.38` 只用于历史 replay。
- V2.45 cache 任务分别记录 structural/host/live/request-hit 四状态轴、route-static identity、manifest status/scope、宿主可用时的 runtime digests、observer parser/identity/coverage 与 raw hash；Tokens/Cache 命中率缺可信 usage Evidence 时写 `未获取到` / `Unavailable`，无授权 live probe 为 `not_authorized`，cache 指标不得替代完成 Evidence。
