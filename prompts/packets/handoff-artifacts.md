...
| `development_environment_check` | 第一轮 baseline 与 Architecture/依赖变化后的 targeted revalidation | 独立 `goal_release_engineer` 的 `environment_preflight` 模式 | Goal Lead 或 `goal_reviewer`（必须与实现/检查 run 不同） | 所有执行 LOOP 第一轮写轻量 preflight；Medium/Large 或用户指定时写正式检查 | `spec/development-environment-check.md`、reuse/create decision、versioned branch/worktree、Architecture/workspace/tool exact hash、argv/cwd/log、current Evidence；只有 `ready` 开实现门 |
...
| `incremental_document_fragment` | V2.47 过程文档的 revision-bound 增量片段 | 当前文档 Owner | `goal_reviewer` 或下游接收者 | 过程文档发生变化时 | document/fragment ID、base/new revision、section、operation、content ref/hash、actor、timestamp |
| `scope_change_proposal` | 当前 required TaskList 外的新任务方案 | 发现者 | Goal Lead + 用户选择 | 发现新范围时只登记 Proposal | source task、理由、建议 scope/AC/风险/成本；`required=false`、`dispatch_forbidden=true` |
| `engineering_metrics_report` | V2.43 十二项工程指标 sidecar、机器摘要与用户 OKF 报告 | Goal Lead、`goal_docs` 或确定性 calculator | `goal_completion_auditor`（不同 run） | 任务完成或 Benchmark 收尾时 | `metrics/metric-events.jsonl`、`metrics/metric-summary.json`、`metrics/engineering-metrics.md`；12 项齐全、schema 有效、manifest bytes digest、current run binding、Evidence refs、OKF 检查和未采集不写零 |
...
| `iteration_state_bundle` | Self-release 四文件 LOOP 状态与 transaction/reconcile receipt | Goal Lead / 唯一 state writer | `goal_qa` 或 `goal_completion_auditor`（不同 run） | 仅当前 `goal-teams-self-release-v2.45` 每个 committed revision 登记；V2.44/V2.43/V2.42/V2.41/V2.40/V2.39/V2.38 只读 replay | `feature_list.json`、`progress.md`、`contract.md`、`log.md`、journal/receipt hash、marker-last/CAS/reconcile Evidence |
| `public_completion_doc` | Self-release 审计完成且清除调用痕迹的公开交付文档 | `goal_docs` 或 Goal Lead | `goal_reviewer` 与最终 `goal_completion_auditor` | 仅 self-release 进入 `docs/archive/V2.44/<delivery_id>/` 前写入 | accepted source hash、sanitizer report、public manifest/tree digest、private provenance receipt；不包含 invocation/tool-call/transport/raw log |
...
8. `scope_change_proposal` 不得改变当前 required 分母。Lead 先完成用户指定任务，再展示 Proposal；只有用户选择后才能创建新的 required task、Owner/Validator/Harness 并进入下一轮。
9. 过程文档只追加 `incremental_document_fragment`；最终文档由 reducer/compiler 从冻结 prefix 重建，不得双写。
