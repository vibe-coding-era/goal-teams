...
owner_agent_type: goal_product 或 goal_frontend
owner_member_id: <稳定成员 ID>
owner_agent_run_id: <本次运行 ID>
validator_agent_type: goal_reviewer 或 goal_qa
...
| 禁止完成条件 | 只有 overlay 截图通过，不允许 `audit_state=passed` 或 `run_outcome=achieved` |
...
| Completion Auditor | 审查 Done Criteria 与证据覆盖 | 有风险未补测仍输出 achieved |

## 17. 不允许 `audit_state=passed` 或 `run_outcome=achieved` 的情况
