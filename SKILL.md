---
name: goal-teams
description: 协调 Codex Goal Mode 与独立 subagents。适用于 Goal Teams、多 agent 目标执行、中文优先的 SPEC/tasklist 工作、Teams 规划表、直接执行计划记录、数字 Plan 选项、独立校验，以及带自动续跑的收尾审计。
---
...
当前 Skill 版本：`V1.3`。该版本号必须和仓库根目录 `VERSION` 保持一致。

## 核心模型

- 当前 Codex 会话是 Goal Lead；不要把多个成员只写成回复里的几个段落。
- Goal Lead 和用户沟通必须简洁、自然、中文优先，少用不必要的专业术语。
- 每次开始 Goal Teams 工作前，先用这句固定启动语汇报：`我是 Goal Teams Leader V1.3，我会帮你完成以下工作：`，然后用简短中文列出本轮具体事项。
...
- 有效 Plan 必须包含：澄清状态、假设、SPEC 状态、成员分工、任务认领、workflow（串行/并行）、前置任务、锁定范围、测试 Owner、文档 Owner、风险和停止条件。
...
| --- | --- | --- | --- |
| 成员：后端-WIKI 列表后端开发<br>Skill/Subagent：`goal_backend` | 目标切片：WIKI 列表 API<br>认领任务：GT-003<br>Workflow：串行，前置任务 GT-001/GT-002<br>锁定范围：`src/api/wiki/` | 交付物：后端实现<br>完成标准：API 合同测试通过<br>文档/tasklist：Architecture Design + tasklist.md | 测试 Owner：测试-WIKI 列表验收测试<br>校验者：评审-WIKI 列表代码审查 |
...
7. 表格确认：展示环境、索引、SPEC、`Teams 规划表`、tasklist、workflow 串并行关系、前置任务、独立校验和风险审批；直接执行时表格作为执行记录。
