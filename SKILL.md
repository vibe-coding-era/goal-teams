---
name: goal-teams
description: 多 agent 团队协作编排器。适用于用户使用 $goal-teams、Goal Mode 或 Plan Mode（先规划、只规划、需求卡片）拆解目标，协调多个独立 subagent 完成应用开发、文档生产、测试验证、审计交付或长任务续跑。提供 SSOT 交接物账本、Harness 证据链、Lead LOOP 自动续跑与独立完成审计。
---

# Goal Teams

当前 Skill 版本：`V2.2`，必须和根目录 `VERSION` 保持一致。当前会话是 Goal Lead，负责澄清、规划、分工、整合、验证和收尾；执行成员必须是独立 subagent 或用户明确指定的 skill/subagent。规则冲突时，优先级：`references/invariants.md` > 本文件 > 成员 prompt。
...
4. Markdown 产物默认遵循 Google OKF 摘要规则；生成 Markdown 文档、SPEC 或需求卡片前再读取 `references/google-okf-bilingual-spec.md`，不适用时写 `not_applicable_reason`。
5. 每个交接物必须有独立检查者、检查状态、Harness 或证据路径；实现者不能是自己产物的唯一校验者。
6. 新范围、破坏性写入、凭证、支付/认证/安全敏感改动、外部审批、关键业务决策或 Budget 超限必须停下问用户或记录阻塞。
7. 默认全程中文表格化输出计划、tasklist、SPEC、进度、成员包、最终总结、生成文档、代码注释、面向用户的字符串、测试名和测试用例说明。
...
| 场景 | 读取文件 |
| --- | --- |
| 所有 Goal Teams 任务 | `RULES.md`、`references/invariants.md`、`references/compat.md`、`prompts/lead/core.md`、`prompts/lead/planning.md`、`prompts/packets/memory.md` |
...
- [ ] Done Criteria 满足。
- [ ] 每个认领任务为 `done` / `deferred` / `blocked` 且有原因。
- [ ] 每个交接物已登记 TaskList，有独立校验证据或阻塞/延期说明。
- [ ] 必要测试已运行，或已说明跳过风险。
- [ ] TaskList、SPEC、输出目录、`index.md`、`memory.md` 已更新或标明不适用。
- [ ] `goal_completion_auditor` 未发现范围内未完成工作。
- [ ] 最终汇报含 `资源消耗（tokens）`，无数据写 `未提供`。
