...
`V2.46` 是唯一 active profile；`V2.45`、`V2.44` 与 `V2.40` 保留协议、前序验证和错误码
...
V2.46 CP05 的 `host_acceptance` 使用发行 Profile 冻结的 Ed25519 公钥验证。
`key_id` 必须等于 32-byte 原始公钥的 SHA-256；`signature` 是固定版本域
`goal-teams/v2.46/cp05/host-acceptance/ed25519/v1`、一个 NUL byte 和移除自身
...
`CP05.workflow_approve` helper 同样不得为 V2.46 生成 caller-forged exact
...
V2.46 `current-route-receipt.json` 的原始字节摘要、self-release policy 与冻结
...
canonical title/body 均由 frozen release-engine profile 唯一派生；V2.46 分别是
`Goal Teams V2.46`、`Goal Teams V2.46` 与
`Goal Teams V2.46. See release/current/README.md in the tagged source.`，
...
V2.46 active profile 的可执行主链是 `start(CP00) → promote(CP01) → doctor(CP02 前必须通过) → promote(CP02–CP08) → prepare(CP09–CP10) → promote(CP11–CP17) → close(CP18)`。CP01 校验 base main 上 V2.45 已发布投影与 candidate 上 V2.46 投影的连续性，并绑定 frozen release-engine profile；不再要求 legacy-root 恢复。V2.45/V2.44/V2.40 历史回放仍保留前序与 recovery 验证，但不能触发外部写入。`status` 是任意非终态阶段的只读观测，不写 state、不触发外部动作。`recover` 只处理当前已持久化 intent：重读或采纳 exact readback；若必须重放外部写入，还必须提供 `resume_external_writes=true`、原 operation authorization 和 `GOAL_TEAMS_RELEASE_WRITE=1`。
...
cd "$CANONICAL_ROOT/develops/v2.46-verification-governance"
"$PYTHON_BIN" scripts/release/release.py start --input "$CANONICAL_ROOT/docs/release-state/V2.46/start.json"
"$PYTHON_BIN" scripts/release/release.py promote --input "$CANONICAL_ROOT/docs/release-state/V2.46/promote-cp01.json"
"$PYTHON_BIN" scripts/release/release.py doctor --input "$CANONICAL_ROOT/docs/release-state/V2.46/doctor.json"
"$PYTHON_BIN" scripts/release/release.py promote --input "$CANONICAL_ROOT/docs/release-state/V2.46/promote-current-checkpoint.json"
"$PYTHON_BIN" scripts/release/release.py prepare --input "$CANONICAL_ROOT/docs/release-state/V2.46/prepare.json"
"$PYTHON_BIN" scripts/release/release.py status --input "$CANONICAL_ROOT/docs/release-state/V2.46/status.json"
"$PYTHON_BIN" scripts/release/release.py recover --input "$CANONICAL_ROOT/docs/release-state/V2.46/recover.json"

cd "$CANONICAL_ROOT"
"$PYTHON_BIN" scripts/release/release.py close --input "$CANONICAL_ROOT/docs/release-state/V2.46/close.json"
```

所有 active JSON envelope 的 `state_path` 必须是 canonical root 下 `docs/release-state/V2.46/promotion-state.json` 的绝对路径；不得使用会从 candidate cwd 解析到 `develops/v2.46-verification-governance/docs/` 的相对路径，也不得在实际 JSON 中保留 `<canonical-root>` 一类未解析占位值。`start` 从 candidate worktree 执行，但一开始就把 state 写进 canonical root `docs/`，以便移除 candidate 后由 canonical root 上的 `close` 继续读取同一 state。
...
V2.46 active envelope 必须从当前 state 中提取 intent/CAS，并额外携带 frozen
