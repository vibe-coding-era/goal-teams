...
| `Engineering Metrics` | 以统一算法观测任务与 Benchmark 运行质量 | `metrics/metric-events.jsonl`、`metric-summary.json`、OKF `engineering-metrics.md` |
...
### V2.43 工程指标与 Benchmark 报告

普通任务和 Benchmark 共用 `references/engineering-metrics-protocol.md`、`references/engineering-metrics-manifest.json` 与同一个确定性 calculator。每个 run 的版本目录写入：
...
Benchmark run record 的 `engineering_metrics` 至少包含 `schema_version`、`current`、`previous`、`recent`、`recent_sample_count` 和 `report_path`。Benchmark `summary.json`、Markdown report 和用户 OKF 报告都读取同一 `metric-summary.json`，不得各自计算 FPAR、LCC、HER、SAR、CPAC、DER、RRR、CWR、SDI、RFR、ARCR 或 MRT。

默认近期窗口是当前 run 之前最近 20 个可比较样本；Benchmark comparison key 为 `scenario_id + execution_mode + rubric_digest + model_and_config_identity + metric_schema_version`。比例类指标用 pooled numerator/denominator，`pending`、`unavailable`、`not_applicable` 不作为零进入平均值，少于 5 个样本标记 `insufficient_sample` 和 `n=N`。

用户工程指标报告必须是自包含 OKF Concept Document：四列表格第一列依次显示“缩写 — 英文全称 — 中文名称”，其余三列为本次、上一次和近期平均值；正文还要投影十二项指标的公式、分子、分母、排除项、历史规则、可用状态与 current Evidence refs。指标仅作观察维度，不替代 `SPEC -> Harness -> Evidence -> Audit`，V2.43 第一版不自动进入 Benchmark 100 分评分或 Release Gate。
...
收尾后按 `references/engineering-metrics-protocol.md` 生成并校验 `versions/<artifact_version>/metrics/engineering-metrics.md`。用户可见最终回复不复制完整指标表或算法正文，只提供该 OKF 报告的可点击链接并提醒打开查看；报告未真实生成时明确说明原因。指标为 `pending`、`unavailable` 或 `not_applicable` 不等于任务失败，也不能替代完成 Evidence。
...
7. 当前成员认领任务行。
8. 当前任务的 Harness Contract；如使用 Benchmark，加载对应 `benchmarks/` 任务包索引和报告；候选收尾时加载工程指标 manifest、summary 与报告协议。
```
...
| 工程指标/完成报告 | `references/engineering-metrics-protocol.md`、`references/engineering-metrics-manifest.json`、当前 run 的 `metrics/metric-summary.json` 与 `engineering-metrics.md` |
