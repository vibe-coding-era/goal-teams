...
| 阶段 | 输入 | 动作 | 输出 | 证据 | 自动续跑 |
| --- | --- | --- | --- | --- | --- |
| Build | Requirement Specification Card、PRD、Architecture Design、tasklist、Member Goal Packet | 成员按 locked_scope 完成实现、文档或模板改动 | 工作区变更、构建候选说明、变更摘要 | diff、文件清单、成员输出契约 | 仅限已确认范围内的非敏感修复 |
...
- Goal Lead 确认版本目录、tasklist、Owner、locked_scope 和 Harness Contract。
...
- `tasklist.md`：每个任务写明 Harness Contract、证据路径和 safety gate。
