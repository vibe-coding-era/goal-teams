...
| 文件或对象 | 作用 | 建议位置 | 写入时机 |
| --- | --- | --- | --- |
| `harness.yaml` | 单任务或单版本的验证契约 | `.codex/goal-teams/versions/<version>/harness.yaml` 或任务目录 | Plan 阶段创建，执行中更新 |
| `evidence.jsonl` | 追加式证据日志 | `.codex/goal-teams/versions/<version>/evidence.jsonl` | 每次检查、人工确认、阻塞、失败后追加 |
| `pipeline-state.json` | 当前研发流水线快照 | `.codex/goal-teams/versions/<version>/pipeline-state.json` | Lead 整合、门禁变化、收尾审计后更新 |
...
- `version` 使用 Goal Teams 版本目录名，例如 `V1.8`。
...
scope:
  allowed:
    ...
    - .codex/goal-teams/versions/V1.8/
  forbidden:
    - SKILL.md
    - README.md
    - README.en.md
...
artifact_checks:
  - path: references/goal-teams-automation-protocol.md
    must_contain:
      ...
      - failure_report
      - approval_gate
  - path: .codex/goal-teams/versions/V1.8/spec/
    must_exist:
...
evidence:
  evidence_file: .codex/goal-teams/versions/V1.8/evidence.jsonl
  ...
  "tasks": [
    {
      ...
      "harness_ref": ".codex/goal-teams/versions/V1.8/spec/test-plan.md#harness-contract",
...
- `tasklist.md` 记录 Owner、状态、Locked Scope 和完成标准。
