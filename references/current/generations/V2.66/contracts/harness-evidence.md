...
tags: [goal-teams, v2.66, harness, evidence]
...
- `GitChangeReceipt` 自动记录 baseline commit/tree、tracked patch/name-status 与 untracked exact-set；解释性手写清单只能引用 receipt，不能成为范围 Oracle。
...
- `GT263-EVIDENCE-GIT-CHANGE`: 真实差异必须从 Git 自动采集并与 locked scope、TaskExactSet 和 source identity 比较；越界路径、漏报 untracked 或 baseline 漂移使 Evidence invalid/stale。
- `GT263-EVIDENCE-BLOCKER`: blocker 单独绑定外部依赖、受影响 Task 与 DAG 后继闭包；解除 blocker 产生新回执，不通过内部重做覆盖等待事实。
