...
description: 定义 V2.66 可执行 Graph 的编译、调度、权限、恢复与证据边界。
tags: [goal-teams, v2.66, graph-engineering, runtime]
...
- `function_id`: `FUNCTION-GRAPH-ENGINEERING-V265`
- `purpose`: 把 Requirement/TaskExactSet 编译为有类型端口、条件、权限、重试、恢复和完成语义的可执行 Graph；区分静态 DAG、宿主执行与真实业务证明。
- `execution_asset_generation`: `V2.65`；V2.66 只改变 Product/Policy 输出控制，不复制或冒充新的 Graph Runtime。
