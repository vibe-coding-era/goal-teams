# Goal Teams V2.52 Current Generation

本目录是 V2.52 Current generation 的人类导航投影。规范语义只存在于 Owner Markdown；`rule-manifest.json` 与本文件不得覆盖 Owner。
...
- `.github/workflows/*` 是仓库侧、不可安装的 CI 控制面，明确不属于 generation root；它由 exact source/tree、V2.52 release profile 与 command contract 单独绑定，不能表述为 ACTIVE 全仓绑定。
