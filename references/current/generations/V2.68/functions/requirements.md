...
tags: [goal-teams, v2.68, requirements, acceptance]
...
- 用户只授权 PRD / SPEC 等文档写入时，按 `specification_delivery` 的 prepare → 写入 → 真实审核 → observe → render 交付；文档完成不等于工程需求冻结、TaskExactSet 或 Development 准入，不为显示看板而造工程实体。
