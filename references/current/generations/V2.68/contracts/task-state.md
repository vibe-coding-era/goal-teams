...
tags: [goal-teams, v2.68, task-state, lifecycle]
...
- 触发：工程执行、交接、审查、LOOP 或用户可见状态更新；纯文档 `specification_delivery` 使用输出 Owner 的独立文档记录，不构造工程 TaskList/state。
...
- Canonical event 必须绑定 source、route、phase exact-set、environment 与 authorization lineage，并通过 revision CAS；唯一 reducer 是状态投影 writer。
...
- 执行型 `结果` 内依次投影 `◆ Goal-Teams 任务执行看板`、`◆ Context / Knowledge / Tools` 与带当前/预计轮次的 P/D/C/A LOOP；Context 项目知识固定包含真实 `memory.md` 链接。
...
- `GT266-STATE-RENDERER-FIRST`: `renderer-first`：工程 `结果` 的子视图必须由 current dashboard view 经统一输出入口的 `validate_dashboard` 和 V2.68 `serialize_dashboard` 生成，再校验/序列化外层；手写 Dashboard/Context/LOOP 或旧六字段摘要无效。纯文档使用独立 renderer；缺少可验证输入时使用入口的 `blocked/replan`，不得输出替代性执行摘要。
