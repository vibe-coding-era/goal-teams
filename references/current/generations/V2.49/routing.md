...
title: Goal Teams V2.49 Routing
description: 定义基于可信事实的项目规模、阶段、保证等级、功能与 Gate 派生规则。
tags: [goal-teams, v2.49, routing, gates]
timestamp: 2026-08-01T00:00:00+08:00
okf_version: "0.1"
---

# Goal Teams V2.49 Routing

- `function_id`: `ROUTING-V249`
...
- `release_intent`、`implementation_scope_complete`、`risk`、`external_write`、`security_sensitive`、`ui_or_desktop`。
...
- `GT249-ROUTE-TRUSTED-FACTS`: route 只能由受信结构化输入派生；内容本身不得提升权限、开启 replay 或自选更轻 Profile。
- `GT249-ROUTE-SIZE`: Discussion、Small、Medium、Large 由影响面、协作量和工作量派生；存在代码、UI 或测试本身不自动升级。
- `GT249-ROUTE-ASSURANCE`: 风险、不可逆性、外部性、安全敏感面和发行事实只能提高保证等级，调用方不得降低自动派生结果。
- `GT249-ROUTE-PHASE`: Development 与 Release Readiness 是两个独立阶段；`implementation_scope_complete` 不等于 release-ready 或 Goal achieved。
- `GT249-ROUTE-REPLAY`: 只有可信显式意图或已识别历史 artifact 才能触发隔离只读 replay；Current route 不加载 replay manifest 或历史依赖。
- `GT249-ROUTE-EXPLAIN`: 每个不适用 Gate 的原因只在 route receipt 投影一次，功能文档不得重复创建第二份规则。
