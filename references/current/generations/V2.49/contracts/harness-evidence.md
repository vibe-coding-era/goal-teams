...
tags: [goal-teams, v2.49, harness, evidence]
...
- `contract_id`: `CONTRACT-HARNESS-EVIDENCE-V249`
...
- `GT249-HARNESS-ASSERTION`: 每个 required item 必须有可执行 assertion、业务 Oracle、observed-result extractor 和失败 artifact 位置。
- `GT249-HARNESS-EXECUTION`: Run Receipt 绑定 exact argv、cwd、source、test、environment、runner、attempts 和 raw artifact digest；discovery 必须大于零。
- `GT249-EVIDENCE-BINDING`: Current Evidence 必须绑定 exact source、route、contract、Harness、environment 和 actor；任一输入漂移立即 stale。
- `GT249-EVIDENCE-VALIDITY`: Evidence state 只能由完整性、真实性和 binding 校验派生；未执行为 not_run，无法执行为 blocked，执行失败为 failed，不可信才 invalid。
- `GT249-EVIDENCE-SIDE-EFFECT`: 外部副作用分别保留 intent、execute-once、exact readback、cleanup/cleanup verification、idempotency 或 reconciliation Evidence。
- `GT249-EVIDENCE-APPEND-ONLY`: 首个失败、重试、flake、修复和 revalidation 追加记录，不覆盖历史事实或降低分母。
