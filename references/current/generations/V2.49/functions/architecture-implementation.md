...
tags: [goal-teams, v2.49, architecture, implementation, tdd]
...
- `function_id`: `FUNCTION-ARCHITECTURE-IMPLEMENTATION-V249`
...
- `GT249-ARCH-TRIGGER`: 跨组件、接口、数据、依赖、迁移、运行时或恢复行为变化必须先形成足够架构边界；局部低风险修改可使用轻量决策记录。
- `GT249-ARCH-ENVIRONMENT`: 开发、测试与生产环境假设分别记录；生产环境规划不等于部署、凭证或生产写入授权。
- `GT249-IMPL-TDD`: 行为实现使用同一不可变 TestCase 的先失败后通过链；测试或 case 漂移必须建立新 revision。
- `GT249-IMPL-INCREMENTAL`: 开发过程的阻断性验证只覆盖适用 TDD 与受影响面增量测试，并绑定当前 source 与 environment digest。
- `GT249-IMPL-IDENTITY`: 实现状态与 Evidence 必须绑定 exact source snapshot；任何代码、依赖、配置或迁移输入变化使旧回执 stale。
