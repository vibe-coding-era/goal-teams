...
tags: [goal-teams, v2.49, agent-runtime, tools]
...
- `function_id`: `FUNCTION-AGENT-RUNTIME-V249`
...
- `GT249-RUNTIME-CAPABILITY`: 每项 runtime/tool 能力必须区分 documented、discovered、executed 和 verified；未实测能力不得宣称可用。
- `GT249-RUNTIME-PROMPT`: Prompt plan 必须绑定 active generation、route、ordered refs、byte count 和 digest；未命中的功能不得加载。
- `GT249-RUNTIME-DISPATCH`: 成员派发绑定稳定身份、locked scope、合同、交付物和停止条件；成员不得自行扩大范围或自证完成。
- `GT249-RUNTIME-ASSURANCE`: actor assurance 与 relationship 分轴记录；同模型、同控制器或相关成员复核不能标为外部独立。
- `GT249-RUNTIME-TRANSITION`: active generation 改变后必须建立 fresh-process transition receipt；任一 source、bootstrap、generation、prompt、route 或 checker digest 漂移使其 stale。
