...
tags: [goal-teams, v2.67, release, github, install]
...
- Release entry 额外要求 Development exact-set 已 accepted、GitChangeReceipt 与 frozen source/tree 一致、外部 blocker 后继闭包为空；Runtime 与业务验证状态仍独立，不用工程完成冒充。
...
- `GT263-RELEASE-GIT-SCOPE`: tracked diff 与 untracked exact-set 必须由 Git 自动采集并匹配锁定 scope；手写清单不能掩盖越界，任何 source/tree 或 exact-set 变化使 readiness 回执 stale。
