...
tags: [goal-teams, v2.62, ui, desktop, e2e]
...
- `function_id`: `FUNCTION-UI-DESKTOP-V250`
...
- `GT250-UI-TRIGGER`: 只有实际 UI/交互影响才加载页面与 E2E 规则；存在前端文件本身不构成触发事实。
- `GT250-UI-STATE-MATRIX`: 页面规格必须覆盖正常、加载、空、错误、禁用、权限和关键 overlay 状态，并把 AC 映射到可观察断言。
- `GT250-UI-E2E`: E2E 必须通过真实交互与回读验证行为，不能只检查静态文本、模拟器或生成文件。
- `GT250-UI-PIXEL`: 只有视觉复刻 route 才要求基线截图、组件视觉锁、像素差和平台归一化 Evidence。
- `GT250-DESKTOP-CAPABILITY`: Browser、Computer Use、Playwright 和桌面控制能力分别按当前宿主观测报告；资料或 manifest 不等于实机能力。
