...
- V2.44 的 Portable Core 可被不同 Agent 运行时采用；`references/agent-runtime-capability-contract.md` 是能力、adapter 与降级边界的 SSOT。Codex 的 `$goal-teams`、`CODEX_HOME`、`.codex`、TOML subagent 是 Codex Adapter，不是通用核心的前置条件。
...
- `feature_list.json` / `progress.md` / `contract.md` / `log.md` 是 legacy 自发布控制平面，不取代 V2.3 ledger、TaskList reducer、Harness、Evidence 或 Completion Audit。当前只由 `goal-teams-self-release-v2.44` 加载；V2.43/V2.42/V2.41/V2.40/V2.39/V2.38 Profile 只读 replay，不是通用 core 默认文件集。
...
- `references/profiles/goal-teams-self-release-v2.44.md` / `policy_profile=goal-teams-self-release-v2.44` 只在可信 adapter 验证目标为 Goal Teams 仓库、产品版本 `V2.44` 且任务类型 `goal_teams_self_release` 时加载；V2.36/V2.37/V2.38/V2.39/V2.40/V2.41/V2.42/V2.43 Profile 仅保留历史 replay，不作为当前路由。
