...
2. 只有可信 adapter 根据当前产品版本、已验证目标仓库与任务类型派生 `goal-teams-self-release-v2.43` 时，才加载 `references/profiles/goal-teams-self-release-v2.43.md`。V2.42/V2.41/V2.40/V2.39/V2.38 Profile 只用于历史 replay；`state_gate_profile` 省略时自动派生，显式值必须精确匹配；字段存在或缺失都不能自选门禁。
