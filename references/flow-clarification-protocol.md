...
2. 中型项目（内部规模：medium；原中迭代流程）如下：
   ...
   -> 生成 TDD、代码和 API/模块集成
   -> 完成增量测试和 P0 冒烟；最终阶段询问用户是否全量回归，再进行独立 Review
...
3. 大型系统（内部规模：large；原大迭代流程）如下：
   ...
   -> 独立 Review、LOOP 补缺、最终重新执行完整全量回归、发布就绪检查和完成审计
...
flowchart TD
    A[用户目标] --> B[LLM 流程初判\nProposal: 小 / 中 / 大]
    B --> C{用户确认流程}
    C -->|1 小型需求/BugFix| S[需求或 BugFix -> 实现 -> 增量测试 + P0 冒烟 -> 独立复核]
    C -->|2 中型项目| M[需求与影响分析 -> 实现 -> 增量 + P0 -> 用户选择是否全量 -> Review]
    C -->|3 大型系统| L[完整规格 -> 实现 -> 增量 + P0 -> 最终完整全量 -> Evidence/审计]
...
三种流程的测试策略以 `references/flow-test-strategy-manifest.json` 为机器 SSOT。中型流程的全量回归选择是最终阶段的新用户决定，不得在启动澄清时预先伪造；大型流程的最终全量 run 不复用前序增量或 P0 的通过结果。
