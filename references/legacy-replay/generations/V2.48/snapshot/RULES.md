...
只输出 `任务、成员、进度、结果、Banchmark、下一轮 LOOP 或下一个任务`，不输出内部推理；状态仍须可验证，未运行/不可用写 `not_run/未获取到`。

## V2.48 流程测试、增量 SSOT、运行时兼容与 Agent 产品开发
...
5. API/E2E 测试能力评分必须读取 `references/testing-capability-manifest.json`，并以 V2.44 三类机器交接物、风险覆盖分母、真实行为 Benchmark 和 append-only issue Evidence 为依据；缺失、未运行、阻塞、不可用或只提供说明/退出码不计分。
6. 规则或合同变化后，历史通过结果仍按原绑定有效；当前不适用写 `stale|retest_required`，新增项写 `not_run`，实际失败写 `failed`，只有证据不可信才写 `invalid`。
7. V2.47 流程测试、增量文档和新范围分别读取对应 manifest/protocol 与 `scope_change_proposal`，细节不复制进稳定前缀。
8. Agent 产品任务按 `references/agent-development/INDEX.md` 渐进加载；产品资料的 `documented`、`unknown`、`not_found` 不等于当前运行时适配、实机执行或验收 Evidence。外部工具、Browser、Computer Use、Playwright、凭证和外部写入仍受上层授权与独立验证约束。
