...
title: Goal Teams Self Release V2.37
description: 只用于 Goal Teams 仓库 V2.37 自发布的附加断言、恢复、评分与公开归档协议。
tags: [goal-teams, self-release, v2.37, policy-profile, okf]
timestamp: 2026-07-12T00:00:00+08:00
okf_version: "0.1"
---

# Goal Teams Self Release V2.37

机器名：`goal-teams-self-release-v2.37`。

本 Profile 扩展 `goal-teams-core-v2.5`，只适用于可信 adapter 已验证目标仓库为 Goal Teams、产品版本为 `V2.37` 且任务类型为 `goal_teams_self_release` 的发布任务。普通业务项目、普通 Goal Teams 使用、仓库内非发布维护都不得加载本文件或继承其固定轮次和归档布局。
...
- 只有 sanitizer 后的 completed/public 普通文件可进入 `docs/archive/V2.37/<delivery_id>/`；公开面不得包含 invocation/tool-call、transport handle、绝对路径、raw log、secret 或 private provenance。
