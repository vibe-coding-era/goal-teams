...
6. CP17 的唯一顺序是 `main_promote -> release_publish -> published_asset_download -> actual_install -> post_release_ci -> independent_audit`。main 先在 remote lock 内执行 exact compare-and-swap；只有 exact main 才可 publish 已验证 Draft。
7. CP18 只有在 immutable Release、永久 main/tag protection、published-asset 安装、post-release CI 与独立 Audit 全部通过后才能写 `CLOSED`。外层 release host 必须重算归档 SSOT；候选侧 V2.36 `completion-audit` 必须精确返回 `E_V236_HOST_ADAPTER_REQUIRED`，该拒绝证明 self-acceptance 边界仍为 fail closed，不能被解释为 Completion 成功。
