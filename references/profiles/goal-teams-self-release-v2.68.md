...
title: Goal Teams Self Release V2.68
description: Goal Teams V2.68 的 Current/Replay 隔离、两阶段测试门禁与简化 Skill 发行规则。
tags: [goal-teams, self-release, v2.68, policy-profile]
timestamp: 2026-08-01T00:00:00+08:00
okf_version: "0.1"
---

# Goal Teams Self Release V2.68

机器名：`goal-teams-self-release-v2.68`。该 Profile 只在 V2.68 Current generation、
可信 route receipt 和 `goal_teams_self_release` 共同命中时加载。V2.67 是已发布 predecessor；
V2.63 及更早发行规则只能由显式 Replay route 加载，不能进入 V2.68 默认闭包。
...
8. 跨 job 续接只允许使用绑定 exact released SHA 的官方 Actions artifact；其中封存 `release/versions/V2.68` 的同一四资产及 S1/S2/integrity/boundary/S3/release-control/plan receipts。后续 S4 必须下载回同一 release root，重建调用数为零。

## Authorization、transport 与 runtime

- 本机真实 V2.67 安装必须由 `scripts/v268/installed_predecessor.py` 逐文件只读捕获，再绑定本次授权投影紧凑 observation；缺失、越界、额外文件或摘要错误均拒绝，禁止以仓库 identity 合成已安装状态。
- Actions 接收 typed observation 输入并写入官方 continuation artifact，保持 I1/correlated；它没有独立读取用户本机文件。既有 observation captured_at 与本次 fresh launch 的短期 handoff 时钟分开。
...
`下一轮 LOOP` 或 `下一个任务`。每轮 `进度` 包含当前轮次/总轮次；终局 `结果` 在 Benchmark
之外包含基于 Evidence 的 `LOOP 改进建议`。不得输出合并字段、额外一级字段或内部推理过程。
不固定输出运行身份短指纹；运行身份只进入机器 receipt 和诊断 Evidence。

执行型 `结果` 依次投影 digest-bound `◆ Goal-Teams 任务执行看板`、`◆ Context / Knowledge / Tools`
和 `◆ LOOP：第 n 轮 / 预计 m 轮`。项目知识包含真实 `memory.md` 链接；LOOP 使用 P/D/C/A 四行，
C 链接 `Banchmark.md`，A 链接 `loop-review.md`。preview/example 不得冒充 current 执行事实。
