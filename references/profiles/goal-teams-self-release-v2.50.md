...
title: Goal Teams Self Release V2.50
description: Goal Teams V2.50 的 Current/Replay 隔离、两阶段测试门禁与简化 Skill 发行规则。
tags: [goal-teams, self-release, v2.50, policy-profile]
timestamp: 2026-08-01T00:00:00+08:00
okf_version: "0.1"
---

# Goal Teams Self Release V2.50

机器名：`goal-teams-self-release-v2.50`。该 Profile 只在 V2.50 Current generation、
可信 route receipt 和 `goal_teams_self_release` 共同命中时加载。V2.48 及更早发行规则只能由
显式 Replay route 加载，不能进入 V2.50 默认闭包。
...
8. 跨 job 续接只允许使用绑定 exact released SHA 的官方 Actions artifact；其中封存 `release/versions/V2.50` 的同一四资产及 S1/S2/integrity/boundary/S3/release-control/plan receipts。后续 S4 必须下载回同一 release root，重建调用数为零。
