#!/usr/bin/env python3
"""Pure V2.68 release routing and receipt-chain helpers.
...
dedicated V2.68 runners.  A consistent receipt chain is evidence correlation;
...
from scripts.v268.runtime_transition import (
...
SECURITY_REVIEW_RUNNER_PATH = "scripts/checks/run-v268-release-security-review.py"
PUBLIC_ASSET_NAMES = {
...
V268_SECURITY_CORE_TARGET_PATHS = frozenset(
    {
        ...
        "schemas/v2.68/output-request.schema.json",
...
def _load_security_required_target_paths() -> frozenset[str]:
    """Load the activation-bound V2.68 review denominator fail closed.

    The security runner and release-flow validator must consume one exact
    manifest-owned target set.  A clean released worktree then binds this
    filesystem projection to the same Git object read by
    :func:`_security_review_git_snapshot`.
    """

    path = REPO_ROOT / V250_SECURITY_CONTRACT_PATH
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RuntimeError("E_V250_SECURITY_REVIEW_MANIFEST") from exc
    targets = value.get("review_targets") if isinstance(value, dict) else None
    if (
        value.get("schema_version")
        != "goal-teams-v2.68-release-security-review-v2"
        or value.get("generation_id") != CURRENT_RELEASE_VERSION
        or not isinstance(targets, list)
        or not targets
    ):
        raise RuntimeError("E_V250_SECURITY_REVIEW_MANIFEST")
    paths: list[str] = []
    for target in targets:
        target_path = target.get("path") if isinstance(target, dict) else None
        pure = PurePosixPath(target_path) if isinstance(target_path, str) else None
        if (
            pure is None
            or not target_path
            or "\\" in target_path
            ...
            or pure.is_absolute()
            or pure.as_posix() != target_path
            or any(part in {"", ".", ".."} for part in pure.parts)
            or target_path in paths
        ):
            raise RuntimeError("E_V250_SECURITY_REVIEW_MANIFEST")
        paths.append(target_path)
    result = frozenset(paths)
    if not V268_SECURITY_CORE_TARGET_PATHS.issubset(result):
        raise RuntimeError("E_V250_SECURITY_REVIEW_DENOMINATOR")
    return result


V250_SECURITY_REQUIRED_TARGET_PATHS = _load_security_required_target_paths()


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.68 receipt bindings."""
...
def derive_release_plan(route: dict[str, Any]) -> dict[str, Any]:
    """Derive V2.68 S0-S4 without executing child processes."""
...
def build_s2_receipt(
    *,
    ...
    if {asset.get("name") for asset in asset_rows} != PUBLIC_ASSET_NAMES:
        raise ValueError("the exact V2.68 four-asset set is required")
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.68-s2-receipt-v1",
...
def _validate_full_regression_receipt(
    ...
    if (
        ...
        or denominator.get("generation_id") != "V2.68"
        ...
        or denominator.get("test_roots") != ["tests/v250", "tests/v268"]
        ...
        != "references/current/generations/V2.68/contracts/predecessor-release-identity.json"
        or denominator.get("test_pattern") != "test_*.py"
        or denominator.get("contract_path")
        != "references/current/generations/V2.68/contracts/release-command-manifest.json"
    ...
    for item in files if isinstance(files, list) else []:
        ...
        path_is_current_test = (
            ...
            and pure_path.parts[:2]
            in (("tests", "v250"), ("tests", "v268"))
...
def _validate_security_review_receipt(
    ...
    if (
        ...
        or reviewer.get("runner_path") != SECURITY_REVIEW_RUNNER_PATH
    ...
    runner_file = reviewed_by_path.get(SECURITY_REVIEW_RUNNER_PATH, {})
    ...
    denominator_valid = bool(
        ...
        == "V250-RELEASE-SECURITY-IMPLEMENTATION"
        and denominator.get("generation_id") == "V2.68"
        ...
        == "references/current/generations/V2.68/contracts/release-security-review-manifest.json"
...
def build_s0_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.68-s0-receipt-v1",
            "gate_id": "s0_identity",
            "source_commit": source_commit,
            "source_tree": source_tree,
...
def build_s1_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.68-s1-receipt-v1",
...
def validate_project_start_authorization(
    ...
    *,
    ...
    if version != CURRENT_RELEASE_VERSION:
        errors.append("E_V250_RELEASE_VERSION")
    value = receipt if isinstance(receipt, dict) else {}
    if not isinstance(receipt, dict):
...
def build_release_control_receipt(
    *,
    ...
    if version != CURRENT_RELEASE_VERSION:
        raise ValueError("E_V250_RELEASE_VERSION")
    ...
    s4_preflight = _seal_receipt(
        {
            "schema_version": "goal-teams-v2.68-s4-preflight-receipt-v1",
    ...
    control: dict[str, Any] = {
        "schema_version": "goal-teams-v2.68-release-control-receipt-v1",
        "repository": repository,
        "version": version,
...
def validate_release_control_receipt(
    ...
    *,
    ...
    value = receipt if isinstance(receipt, dict) else {}
    errors: list[str] = []
    if expected_version != CURRENT_RELEASE_VERSION:
        errors.append("E_V250_RELEASE_VERSION")
    if not isinstance(receipt, dict):
        return {
            "ok": False,
    ...
    if value.get("schema_version") != "goal-teams-v2.68-release-control-receipt-v1":
    ...
    if (
        ...
        != "goal-teams-v2.68-external-anchor-validation-v1"
