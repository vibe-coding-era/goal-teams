#!/usr/bin/env python3
"""Observe and validate V2.68 fresh-runtime transition receipts.
...
from scripts.v268.release_identity import (  # noqa: E402
    ACTIVE_PATH,
    ...
    PREDECESSOR_RELEASE_TAG as PREVIOUS_CONTROLLER_RELEASE_TAG,
    RELEASE_COMMAND_MANIFEST_PATH,
    RELEASE_PROFILE_PATH,
    RELEASE_ROUTE_MANIFEST_PATH,
    REPOSITORY,
    RUNTIME_TRANSITION_SCHEMA_PATH,
    TARGET_PRODUCT_VERSION as LOADED_RUNTIME_PRODUCT_VERSION,
...
ROUTE_BY_STAGE_AND_SIZE = {
    ...
    # V2.68 has no separate Small Release prompt route.  Small Release uses
...
def _load_route_context(
    *,
    ...
    if (
        ...
        or active.get("generation_id") != LOADED_RUNTIME_PRODUCT_VERSION
    ...
    if (
        ...
        or activation.get("generation_id") != LOADED_RUNTIME_PRODUCT_VERSION
    ...
    if (
        ...
        or identity.get("target_policy_generation") != LOADED_RUNTIME_PRODUCT_VERSION
    ...
    if (
        ...
        or prompt_manifest.get("generation_id") != LOADED_RUNTIME_PRODUCT_VERSION
    ...
    if not isinstance(legacy, dict):
        raise ValueError("E_V250_RUNTIME_TRANSITION_ROUTE")
    runtime_generation = {
        "generation_id": LOADED_RUNTIME_PRODUCT_VERSION,
    ...
    if (
        route_receipt.get("generation_id") != LOADED_RUNTIME_PRODUCT_VERSION
...
def _load_authorization(
    ...
    if (
        ...
        or value.get("version") != LOADED_RUNTIME_PRODUCT_VERSION
...
def _verify_handoff_signature(
    ...
    """Verify the V2.67 host-issued V2.68 handoff with the pinned owner key."""
...
def _load_previous_controller_release_identity(root: Path) -> dict[str, Any]:
    """Load the activation-bound published V2.67 identity fail closed."""
...
def build_authorized_local_predecessor_observation(
    *,
    ...
    predecessor_observation: Mapping[str, Any] | None,
    root: Path = ROOT,
    ...
    external SSH-signature requirement, while retaining exact published V2.67
    state and the one-time user authorization bindings.
    """

    from scripts.v268.installed_predecessor import validate_installed_predecessor

    identity = _load_previous_controller_release_identity(root)
    observed = validate_installed_predecessor(
        predecessor_observation, authorization=authorization, expected_identity=identity,
    )
    if not observed.get("ok"):
        raise ValueError("E_V268_PREDECESSOR_OBSERVATION_REQUIRED")
    ...
    payload = {
        ...
        "previous_run_id": "V267-INSTALLED-STATE-" + installed["state_sha256"][:16],
        "nonce": uuid.uuid4().hex,
        "issued_at": issued.isoformat(timespec="seconds"),
        "expires_at": expires,
        "installed_v267_current_state": installed,
        "installed_predecessor_observation": dict(predecessor_observation),
        "observation_source": "authorized_local_installed_v267_state",
    }
    return {
...
def validate_controller_handoff(
    ...
    *,
    ...
    """Validate the installed V2.67 host-issued V2.68 handoff."""
    ...
    expected_payload_fields = {
        ...
        "issued_at",
        "expires_at",
        "installed_v267_current_state",
        "installed_predecessor_observation",
    }
    if local_observation:
    ...
    installed = payload.get("installed_v267_current_state")
    ...
    observed = validate_installed_predecessor(
        payload.get("installed_predecessor_observation"),
        expected_identity=release_identity if expected_installed is not None else {},
        expected_authorization_id=payload.get("authorization_id"),
        expected_authorization_intent_sha256=payload.get("authorization_intent_sha256"),
    )
    _append_if(errors, not observed.get("ok"), "E_V268_PREDECESSOR_OBSERVATION_REQUIRED")
    if observed.get("ok"):
        try:
            captured = _parse_timestamp(payload["installed_predecessor_observation"]["captured_at"], "E_V268_PREDECESSOR_CAPTURE_TIME")
            launched = _parse_timestamp(payload.get("issued_at"), "E_V268_PREDECESSOR_CAPTURE_TIME")
            _append_if(errors, captured > launched, "E_V268_PREDECESSOR_CAPTURE_TIME")
        except ValueError:
    ...
    if local_observation:
        _append_if(
            ...
            != "authorized_local_installed_v267_state",
...
def validate_transition(
    ...
    *,
    ...
    _append_if(
        errors,
        value.get("generation_id") != LOADED_RUNTIME_PRODUCT_VERSION
