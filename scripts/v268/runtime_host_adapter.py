#!/usr/bin/env python3
"""External host adapter for the V2.67 -> V2.68 runtime handoff.

This module can derive an authorization-bound local predecessor observation
from the exact installed V2.67 state. It never reads a private key or claims
external independence; it launches a fresh V2.68 child with ``Popen``,
...
KNOWN_RUNTIME_CHILD_ERROR_CODES = frozenset(
    {
        "E_V268_PREDECESSOR_OBSERVATION_REQUIRED",
        "E_V268_PREDECESSOR_CAPTURE_TIME",
...
def launch_runtime_transition(
    *,
    ...
    controller_handoff_receipt: Mapping[str, Any],
    host_execution_id: str,
    predecessor_observation_receipt: Mapping[str, Any] | None = None,
    root: Path = ROOT,
    ...
    if controller_handoff_receipt is None:
        controller_handoff_receipt = build_authorized_local_predecessor_observation(
            ...
            predecessor_observation=predecessor_observation_receipt,
            root=root,
        )
...
def _read_json(path: Path) -> object:
    from scripts.v268.installed_predecessor import _read_file, strict_json

    return strict_json(_read_file(path, max_bytes=16 * 1024 * 1024)[0])
...
def parse_args() -> argparse.Namespace:
    ...
    launch.add_argument("--predecessor-observation-receipt", type=Path, required=True)
...
def main() -> int:
    ...
    try:
        ...
        result = launch_runtime_transition(
            ...
            predecessor_observation_receipt=_read_json(args.predecessor_observation_receipt),
        )
