#!/usr/bin/env python3
"""Execute and reconcile the V2.68 S4 publish/install contract.
...
TAG = "v2.68"
TITLE = "Goal Teams V2.68"
CANONICAL_REMOTE = "git@github.com:vibe-coding-era/goal-teams.git"
CANONICAL_RELEASE_URL = (
    "https://github.com/vibe-coding-era/goal-teams/releases/tag/v2.68"
)
DRAFT_RELEASE_URL_RE = re.compile(
    r"^https://github\.com/vibe-coding-era/goal-teams/releases/tag/"
    r"untagged-[0-9a-f]{20,64}$"
)
RELEASE_PROFILE_RELATIVE = Path("references/release-profiles/v2.68.json")
SCHEMA_PATH = ROOT / "schemas" / "v2.68" / "release-control.schema.json"
RUNTIME_TRANSITION_SCHEMA_PATH = (
    ROOT / "schemas" / "v2.68" / "runtime-transition-receipt.schema.json"
)
RUNTIME_TRANSITION_SCHEMA_ID = (
    "https://goal-teams.local/schemas/v2.68/"
    "runtime-transition-receipt.schema.json"
)
SCHEMA_REF = (
    "https://goal-teams.local/schemas/v2.68/release-control.schema.json"
...
JOURNAL_LAYOUT = (
    ("tag_local_create", "tag_local_create", "refs/tags/v2.68", "local_repository"),
    ("tag_push", "tag_push", "refs/tags/v2.68", "external_service"),
    ("release_create", "release_create", "v2.68", "external_service"),
    (
    ...
    (
        "asset_upload:goal-teams-V2.68.tar.gz",
        "asset_upload",
        "goal-teams-V2.68.tar.gz",
        "external_service",
    ),
    ("release_publish", "release_publish", "v2.68", "external_service"),
...
def derive_resume_operation_ids(
    ...
    for entry in journal:
        if not isinstance(entry, Mapping):
            raise ValueError("E_V268_S4_RESUME_JOURNAL")
...
def _prepare_formal_install_temp_root(repository_root: Path) -> Path:
    ...
    try:
        ...
        raise S4ExecutionError("E_V268_S4_INSTALL_TMPDIR_UNSAFE") from exc


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.68 receipts."""
...
class CommandBackend:
    ...
    def install(self, bundle: Path, identity_path: Path) -> None:
        ...
        if self.install_temp_root is not None and self.install_temp_root != temp_root:
            raise S4ExecutionError("E_V268_S4_INSTALL_TMPDIR_UNSAFE")
        _checked(
            self.runner,
            [
                "/usr/bin/env",
...
def _default_prewrite_boundary_validator(
    *,
    ...
    module = _load_module(
        ...
        repository_root / "scripts/v268/repository_boundary.py",
    )
    try:
...
def _verify_remote_asset(
    *,
    ...
    if readback_temp_root != expected_temp_root:
        raise S4ExecutionError("E_V268_S4_INSTALL_TMPDIR_UNSAFE")
    with tempfile.TemporaryDirectory(
        prefix="goal-teams-v268-asset-readback-", dir=readback_temp_root
    ) as temp:
        _download_one_exact(
            backend=backend,
            name=name,
...
def _base_outcome(
    *,
    ...
    return {
        "schema_version": "goal-teams-v2.68-s4-outcome-receipt-v2",
    ...
    *,
    ...
    try:
        ...
        if download_temp_root != install_temp_root:
            raise S4ExecutionError("E_V268_S4_INSTALL_TMPDIR_UNSAFE")
        with tempfile.TemporaryDirectory(
            prefix="goal-teams-v268-s4-", dir=download_temp_root
        ) as temp:
            work = Path(temp)
