...
def validate_v250_current(args: argparse.Namespace, product: str) -> None:
    ...
    if product == "V2.68" and identity.get("execution_asset_generation") != "V2.65":
        fail("V2.68 activation must bind the V2.65 Graph execution contract")
    ...
    if published == product:
        ...
        candidate_states = (
            ...
            if product in {"V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}
            else {"v250_release_readiness"}
        )
        unchanged_published_predecessor = (
            product in {"V2.65", "V2.66", "V2.67", "V2.68"}
...
def main() -> None:
    ...
    if product in {"V2.62", "V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}:
        validate_v250_current(args, product)
        execution = "v2.50+v2.65-graph" if product in {"V2.66", "V2.67", "V2.68"} else "v2.50"
        print(
