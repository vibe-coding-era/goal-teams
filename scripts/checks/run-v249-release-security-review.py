#!/usr/bin/env python3
"""Audit the exact V2.49 released implementation in a fresh process.
...
EXPECTED_ASSERTION_IDS = (
    "V249-SEC-EXACT-GIT-IDENTITY",
    "V249-SEC-DENOMINATOR-COMPLETE",
    "V249-SEC-GIT-OBJECT-FILESYSTEM",
    "V249-SEC-PATH-SYMLINK-MODE",
    "V249-SEC-DEPENDENCY-SURFACE",
    "V249-SEC-SECRET-NEGATIVE-SCAN",
    "V249-SEC-DANGEROUS-OPERATION-ALLOWLIST",
    "V249-SEC-COMMAND-EXECUTION-BOUNDARIES",
    "V249-SEC-GITHUB-GIT-SSH-ONLY",
    "V249-SEC-WORKFLOW-ACTION-PINS",
    "V249-SEC-S2-SECURITY-SEPARATION",
    "V249-SEC-FRESH-CORRELATED-PROCESS",
)
...
def _git_text(root: Path, *args: str) -> str:
    ...
    if result.returncode != 0:
        raise SecurityReviewError("E_V249_SECURITY_GIT_IDENTITY")
...
def _git_bytes(root: Path, *args: str) -> bytes:
    ...
    if result.returncode != 0:
        raise SecurityReviewError("E_V249_SECURITY_GIT_OBJECT")
    return bytes(result.stdout)


def _safe_path(value: object) -> str:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_PATH")
...
def _ls_tree(root: Path, source_commit: str) -> dict[str, dict[str, str]]:
    ...
    for record in raw.split(b"\x00"):
        ...
        try:
            ...
            raise SecurityReviewError("E_V249_SECURITY_GIT_TREE") from exc
        if path in entries:
            raise SecurityReviewError("E_V249_SECURITY_GIT_TREE")
...
def _verify_exact_identity(
    ...
    if COMMIT_RE.fullmatch(source_commit) is None or COMMIT_RE.fullmatch(source_tree) is None:
        raise SecurityReviewError("E_V249_SECURITY_IDENTITY")
    observed_root = Path(_git_text(root, "rev-parse", "--show-toplevel")).resolve()
    if observed_root != root.resolve():
        raise SecurityReviewError("E_V249_SECURITY_WORKTREE_ROOT")
    ...
    if head_commit != source_commit:
        raise SecurityReviewError("E_V249_SECURITY_COMMIT_DRIFT")
    if head_tree != source_tree:
        raise SecurityReviewError("E_V249_SECURITY_TREE_DRIFT")
    ...
    if status or diff.returncode != 0 or cached.returncode != 0:
        raise SecurityReviewError("E_V249_SECURITY_WORKTREE_DIRTY")
...
def _frozen_file(
    ...
    if not isinstance(entry, Mapping):
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_MISSING")
    if entry.get("type") != "blob" or GIT_MODE_RE.fullmatch(str(entry.get("mode", ""))) is None:
        raise SecurityReviewError("E_V249_SECURITY_TARGET_TYPE")
    frozen = _git_bytes(root, "show", f"{source_commit}:{safe}")
    filesystem = root / safe
    try:
        metadata = filesystem.lstat()
    except OSError as exc:
        raise SecurityReviewError("E_V249_SECURITY_TARGET_MISSING") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise SecurityReviewError("E_V249_SECURITY_TARGET_SYMLINK")
    try:
        filesystem.resolve().relative_to(root.resolve())
    except (OSError, ValueError) as exc:
        raise SecurityReviewError("E_V249_SECURITY_TARGET_ESCAPE") from exc
    current = filesystem.read_bytes()
    if current != frozen:
        raise SecurityReviewError("E_V249_SECURITY_TARGET_CONTENT_DRIFT")
...
def _load_manifest(
    ...
    try:
        ...
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_CONTRACT") from exc
    if not isinstance(value, dict):
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_CONTRACT")
    return value, _sha256(frozen)


def _validate_manifest(manifest: Mapping[str, Any]) -> list[dict[str, Any]]:
    if manifest.get("schema_version") != "goal-teams-v2.49-release-security-review-v2":
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_SCHEMA")
    if manifest.get("denominator_id") != "V249-RELEASE-SECURITY-IMPLEMENTATION":
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_ID")
    if manifest.get("unknown_or_missing_policy") != "fail_closed":
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_POLICY")
    if tuple(manifest.get("assertion_denominator", [])) != EXPECTED_ASSERTION_IDS:
        raise SecurityReviewError("E_V249_SECURITY_ASSERTION_DENOMINATOR")
    targets = manifest.get("review_targets")
    if not isinstance(targets, list) or not targets:
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_EMPTY")
    ...
    for target in targets:
        if not isinstance(target, dict) or set(target) != {"path", "content_kind", "categories"}:
            raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_TARGET")
        ...
        if categories != sorted(set(categories)) or not set(categories) <= REQUIRED_CATEGORIES:
            raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_CATEGORY")
        seen.add(path)
    ...
    if seen != MANDATORY_REVIEW_TARGETS or observed_categories != REQUIRED_CATEGORIES:
        raise SecurityReviewError("E_V249_SECURITY_DENOMINATOR_INCOMPLETE")
    ...
    if not isinstance(declared, list) or declared != sorted(set(declared)):
        raise SecurityReviewError("E_V249_SECURITY_DEPENDENCY_CONTRACT")
    ...
    if not isinstance(dangerous, dict) or set(dangerous) != {
        ...
        raise SecurityReviewError("E_V249_SECURITY_DANGEROUS_ALLOWLIST")
...
def _collect_reviewed_files(
    ...
    for target in targets:
        ...
        try:
            ...
            raise SecurityReviewError("E_V249_SECURITY_TARGET_ENCODING") from exc
        if "\x00" in text:
            raise SecurityReviewError("E_V249_SECURITY_TARGET_ENCODING")
...
def _python_source(text: str, kind: str) -> str:
    ...
    if marker not in text or not text.endswith("\nPY\n"):
        raise SecurityReviewError("E_V249_SECURITY_INSTALLER_HEREDOC")
    ...
    *,
    ...
    for target in targets:
        ...
        try:
            ...
            raise SecurityReviewError("E_V249_SECURITY_PYTHON_SYNTAX") from exc
        names: set[str] = set()
...
def run_review(
    *,
    ...
    if not reviewer_id or not review_run_id or orchestrator_pid < 1:
        raise SecurityReviewError("E_V249_SECURITY_REVIEWER_IDENTITY")
    root = root.resolve()
    ...
    command_contract = json.loads(texts["references/current/generations/V2.49/contracts/release-command-manifest.json"])
    ...
    assertions = [
        _assertion("V249-SEC-EXACT-GIT-IDENTITY", True, **identity),
        _assertion("V249-SEC-DENOMINATOR-COMPLETE", len(targets) == len(MANDATORY_REVIEW_TARGETS), target_count=len(targets), required_target_count=len(MANDATORY_REVIEW_TARGETS)),
        _assertion("V249-SEC-GIT-OBJECT-FILESYSTEM", all(item["git_object_matches_filesystem"] for item in reviewed_files), reviewed_file_count=len(reviewed_files)),
        _assertion("V249-SEC-PATH-SYMLINK-MODE", all(not item["symlink"] and GIT_MODE_RE.fullmatch(item["git_mode"]) for item in reviewed_files), symlink_count=sum(bool(item["symlink"]) for item in reviewed_files)),
        _assertion("V249-SEC-DEPENDENCY-SURFACE", dependency["passed"], dependency_file_count=len(dependency["discovered_dependency_files"]), python_import_count=len(dependency["python_imports"]), unknown_import_count=dependency["unknown_import_count"]),
        _assertion("V249-SEC-SECRET-NEGATIVE-SCAN", secrets["passed"], scanned_file_count=secrets["scanned_file_count"], rule_count=secrets["rule_count"], finding_count=secrets["finding_count"]),
        _assertion("V249-SEC-DANGEROUS-OPERATION-ALLOWLIST", dangerous["passed"], observed_count=dangerous["observed_count"], allowed_count=dangerous["allowed_count"], inventory_sha256=dangerous["inventory_sha256"], finding_count=len(dangerous["findings"])),
        _assertion("V249-SEC-COMMAND-EXECUTION-BOUNDARIES", commands["passed"], subprocess_call_count=commands["subprocess_call_count"], finding_count=len(commands["findings"])),
        _assertion("V249-SEC-GITHUB-GIT-SSH-ONLY", git_ssh["passed"], workflow_checkout_count=git_ssh["workflow_checkout_count"], finding_count=len(git_ssh["findings"])),
        _assertion("V249-SEC-WORKFLOW-ACTION-PINS", workflows["passed"], action_count=len(workflows["action_pins"]), finding_count=len(workflows["findings"])),
        _assertion("V249-SEC-S2-SECURITY-SEPARATION", s2_separation, s2_security_check_invocation_limit=s2.get("security_check_invocation_limit"), s1_release_security_review_invocation_count=1),
        _assertion("V249-SEC-FRESH-CORRELATED-PROCESS", fresh_process, runner_pid=os.getpid(), orchestrator_pid=orchestrator_pid, actor_assurance="I1", actor_relationship="correlated"),
    ]
    findings = sorted(
    ...
    receipt: dict[str, Any] = {
        "schema_version": "goal-teams-v2.49-release-gate-receipt-v1",
        ...
        "reviewer_identity": {
            ...
            "runner_sha256": next(item["sha256"] for item in reviewed_files if item["path"] == "scripts/checks/run-v249-release-security-review.py"),
        },
        "source_commit": source_commit,
        "source_tree": source_tree,
