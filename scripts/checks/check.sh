...
if [[ "$(<VERSION)" != "V2.51" ]]; then
  echo "This Current checker only accepts VERSION=V2.51." >&2
  exit 2
fi

"$PYTHON_BIN" scripts/checks/validate-v250-generation.py --generation-id V2.51
...
if [[ "$PHASE" == "development" ]]; then
  ...
  "$PYTHON_BIN" scripts/checks/check-v250.py \
    ...
    --stage candidate
  echo "Goal Teams V2.51 development checks passed (${CHECK_MODE})."
  exit 0
fi
...
echo "Goal Teams V2.51 final regression and release security review passed (${CHECK_MODE})."
