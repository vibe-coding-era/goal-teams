#!/usr/bin/env python3
"""Validate V2.49 owner projections, route closures, budgets, and replay isolation."""
...
from scripts.v249.generation_runtime import GenerationLoadError, load_generation, resolve_repo_file
from scripts.v249.replay_runner import load_replay_manifest, run_replay
from scripts.v249.route_closure import RouteClosureError, compile_route_closure


RULE_RE = re.compile(r"^- \x60(GT249-[A-Z0-9-]+)\x60:", re.MULTILINE)


def validate_generation(repo_root: Path | str, generation_id: str = "V2.49") -> dict[str, Any]:
    ...
    if not isinstance(owners, list) or not owners:
        errors.append("E_V249_RULE_OWNERS_EMPTY")
    else:
        for owner in owners:
            if not isinstance(owner, dict):
                errors.append("E_V249_RULE_OWNER_INVALID")
                continue
            owner_id = owner.get("owner_id")
            path = owner.get("path")
            if not isinstance(owner_id, str) or owner_id in owner_ids:
                errors.append("E_V249_RULE_OWNER_DUPLICATE")
            else:
                owner_ids.add(owner_id)
            if not isinstance(path, str) or path in owner_paths:
                errors.append("E_V249_RULE_OWNER_PATH_DUPLICATE")
                continue
            ...
            try:
                ...
                errors.append("E_V249_RULE_OWNER_UNREADABLE")
                continue
            ...
            if not isinstance(projected_rules, list) or source_rules != set(projected_rules):
                errors.append("E_V249_RULE_PROJECTION_DRIFT")
                continue
        ...
        if duplicate_rules:
            errors.append("E_V249_RULE_ID_DUPLICATE")

    routes = prompt_manifest.get("routes", {})
    if not isinstance(routes, dict) or not routes:
        errors.append("E_V249_ROUTES_EMPTY")
    else:
    ...
    try:
        ...
        for version in replay_versions:
            ...
            if denied.get("status") != "replay_unavailable":
                errors.append("E_V249_REPLAY_IMPLICITLY_ENABLED")
            ...
            if explicit.get("status") not in {"historical_passed", "historical_failed", "replay_unavailable"}:
                errors.append("E_V249_REPLAY_OUTPUT")
            if explicit.get("current_completion_eligible") is not False:
                errors.append("E_V249_REPLAY_CURRENT_LEAK")
    except GenerationLoadError as exc:
        errors.append(exc.code)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--generation-id", default="V2.49")
...
def main() -> int:
    ...
    if args.json:
        ...
        print(
            f"V2.49 generation validation passed: "
            f"{result['route_count']} routes, {result['rule_count']} unique rules"
        )
    else:
        print("V2.49 generation validation failed: " + ", ".join(result["errors"]))
