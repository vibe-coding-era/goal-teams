#!/usr/bin/env python3
"""V2.49 route-aware development and final-release checker."""
...
CONTRACT_ROOT = ROOT / "references/current/generations/V2.49/contracts"
...
def validate_contracts() -> dict[str, Any]:
    required = {
        "release-route-manifest.json": "goal-teams-v2.49-release-route-v1",
        "release-command-manifest.json": "goal-teams-v2.49-release-command-manifest-v1",
        "release-security-review-manifest.json": "goal-teams-v2.49-release-security-review-v2",
        "public-asset-map.json": "goal-teams-v2.49-public-asset-map-v1",
    }
    errors: list[str] = []
    ...
    for name, schema_version in required.items():
        try:
            ...
            errors.append(f"E_V249_CONTRACT:{name}:{type(exc).__name__}")
            continue
        values[name] = value
        if value.get("schema_version") != schema_version:
            errors.append(f"E_V249_CONTRACT_SCHEMA:{name}")
    ...
    if assets.get("asset_count") != 4 or observed_assets != expected_assets:
        errors.append("E_V249_PUBLIC_ASSET_MAP")
    ...
    if (
        ...
        errors.append("E_V249_CURRENT_DENOMINATOR_CONTRACT")
    return {
        "ok": not errors,
        "passed": not errors,
        "errors": errors,
...
def _git_text(*args: str) -> str:
    ...
    if result.returncode != 0:
        raise ValueError("E_V249_RELEASE_GIT_IDENTITY")
...
def _current_test_denominator(
    ...
    *,
    ...
    tracked = _git_text(
        "ls-tree", "-r", "--name-only", source_commit, "--", "tests/v249"
    ).splitlines()
    paths = sorted(
        path
        for path in tracked
        if path.startswith("tests/v249/")
        and Path(path).name.startswith("test_")
        and path.endswith(".py")
    )
    filesystem_paths = sorted(
        path.relative_to(ROOT).as_posix()
        for path in (ROOT / "tests/v249").rglob("test_*.py")
        if path.is_file() and not path.is_symlink()
    )
    if not paths or paths != filesystem_paths:
        raise ValueError("E_V249_CURRENT_DENOMINATOR_FILE_DRIFT")
    files: list[dict[str, str]] = []
    for relative in paths:
        frozen = _git_bytes("show", f"{source_commit}:{relative}")
        current = (ROOT / relative).read_bytes()
        if current != frozen:
            raise ValueError("E_V249_CURRENT_DENOMINATOR_CONTENT_DRIFT")
        files.append(
...
def run_full_regression(
    ...
    *,
    ...
    """Run the complete frozen V2.49 Current-generation denominator once."""
    ...
    receipt: dict[str, Any] = {
        ...
        "run_id": f"V249-CURRENT-FULL-{source_commit[:12]}-{uuid.uuid4().hex}",
...
def run_release_security_review(
    ...
    *,
    ...
    review_run_id = f"V249-SECURITY-{source_commit[:12]}-{uuid.uuid4().hex}"
    argv = [
        sys.executable,
        ...
        source_tree,
        "--reviewer-id",
        "goal-teams-v249-release-implementation-security-reviewer",
        "--review-run-id",
        review_run_id,
        "--orchestrator-pid",
        str(os.getpid()),
    ...
    try:
        ...
        raise ValueError("E_V249_SECURITY_REVIEW_OUTPUT") from exc
    if not isinstance(receipt, dict):
        raise ValueError("E_V249_SECURITY_REVIEW_OUTPUT")
    if result.returncode != 0:
        return receipt
    if (
        ...
        raise ValueError("E_V249_SECURITY_REVIEW_PROCESS_BINDING")
    return receipt
...
def main() -> int:
    ...
    release_flow = _load_module(
        "_goalteams_v249_release_flow", ROOT / "scripts/v249/release_flow.py"
    )
    runtime_transition = _load_module(
        "_goalteams_v249_runtime_transition",
        ROOT / "scripts/v249/runtime_transition.py",
    )
    route = {
    ...
    if args.phase == "release":
        try:
            ...
            if args.released_runtime_receipt is None:
                raise ValueError("E_V249_RELEASED_RUNTIME_RECEIPT_REQUIRED")
            expected_host_execution_id = getattr(
                args, "expected_host_execution_id", None
            )
            if not expected_host_execution_id:
                raise ValueError("E_V249_HOST_EXECUTION_ID_REQUIRED")
            ...
            if not released_runtime_validation.get("may_enter_s0"):
                raise ValueError("E_V249_RELEASED_RUNTIME_S0_REQUIRED")
            ...
            release_gate_binding = {
                "ok": False,
                "passed": False,
                "errors": [str(exc) or f"E_V249_RELEASE_GATE_RUNNER:{type(exc).__name__}"],
            }
    ...
    payload: dict[str, Any] = {
        "schema_version": "goal-teams-v2.49-check-result-v1",
        "passed": passed,
        "s1_passed": s1_passed,
