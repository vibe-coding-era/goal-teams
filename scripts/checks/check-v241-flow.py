#!/usr/bin/env python3
"""Fail-closed checks for the V2.41 flow contract on the V2.45 runtime."""
...
def read(relative: str) -> str:
    path = ROOT / relative
    if not path.is_file():
        raise SystemExit(f"[FAIL] missing required V2.45 file: {relative}")
...
def require_markers(relative: str, *markers: str) -> None:
    ...
    for marker in markers:
        if marker not in text:
            raise SystemExit(f"[FAIL] {relative} missing V2.45 marker: {marker}")


def main() -> None:
    if read("VERSION").strip() != "V2.45":
        raise SystemExit("[FAIL] VERSION must be V2.45")
    require_markers(
    ...
    require_markers(
        "RULES.md",
        "V2.45 流程澄清、V2.44 测试能力与运行时兼容",
        "Proposal",
        "确认前",
        "生产环境规划不等于部署授权",
    )
    print("V2.45 flow clarification checks passed.")
