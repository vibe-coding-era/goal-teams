...
side effect. ``plan`` is read-only. V2.67 ``validate`` checks the same asset
...
with predecessor contracts or assets.
"""
...
ACTIVE_SIMPLE_VERSION = "V2.68"
SINGLE_BUILD_VERSIONS = frozenset({"V2.49", "V2.50", "V2.52", "V2.6", "V2.62", "V2.63", "V2.65", "V2.66", "V2.67", "V2.68"})
SHARED_FLOW_PREDECESSORS = frozenset({"V2.50", "V2.52", "V2.6", "V2.65", "V2.66", "V2.67"})
...
def _version_digits(version: str) -> str:
    if version not in SINGLE_BUILD_VERSIONS:
        raise SkillReleaseError(
            ...
            "release flow is defined only for registered single-build versions",
            version=version,
        )
...
def _protocol_digits(version: str) -> str:
    ...
    return "250" if version in {"V2.52", "V2.6", "V2.62", "V2.63", "V2.65"} else _version_digits(version)


def _protocol_lower(version: str) -> str:
    return "v2.50" if version in {"V2.52", "V2.6", "V2.62", "V2.63", "V2.65"} else _version_lower(version)
...
def runtime_static_input_paths(version: str) -> tuple[str, ...]:
    ...
    release_identity_paths = (
        ...
        if version in {"V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}
        else ()
    )
    release_identity_module_paths = (
        (f"scripts/v{_version_digits(version)}/release_identity.py",)
    ...
    ) + release_identity_paths + release_identity_module_paths + (
        f"scripts/checks/check-v{digits}.py",
        f"scripts/v{digits}/runtime_host_adapter.py",
        f"scripts/v{digits}/runtime_transition.py",
    ) + (
        ("scripts/v268/installed_predecessor.py", "schemas/v2.68/installed-predecessor-observation.schema.json")
        if version == "V2.68" else ()
    )
...
V250_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.65")
...
V263_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.65")
V265_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.65")
V266_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.66")
V267_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.67")
V268_RUNTIME_STATIC_INPUT_PATHS = runtime_static_input_paths("V2.68")
...
_ROUTE_RECEIPT_INDEX = V249_CONTINUATION_FORMAL_RECEIPTS.index(
    "release-route-receipt.json"
)
...
V267_CONTINUATION_ASSET_NAMES = continuation_asset_names("V2.67")
V268_CONTINUATION_ASSET_NAMES = continuation_asset_names("V2.68")
V268_CONTINUATION_FORMAL_RECEIPTS = (
    V267_CONTINUATION_FORMAL_RECEIPTS + ("installed-predecessor-observation.json",)
)
V268_CONTINUATION_PHASE_ORDER = (
    V267_CONTINUATION_PHASE_ORDER[:2] + ("installed_predecessor",) + V267_CONTINUATION_PHASE_ORDER[2:]
)


def continuation_formal_receipts(version: str) -> tuple[str, ...]:
    """Return the exact formal receipt set without mutating predecessor contracts."""

    _version_digits(version)
    return (
        V268_CONTINUATION_FORMAL_RECEIPTS
        if version == "V2.68"
        else V267_CONTINUATION_FORMAL_RECEIPTS
        if version in {"V2.67", "V2.68"}
...
def _release_flow_module(version: str) -> ModuleType:
    ...
    if version in {"V2.50", "V2.52", "V2.6", "V2.62", "V2.63"}:
        return _v250_release_flow_module()
    if version == "V2.66":
        return _load_module(
            "_goal_teams_v266_release_flow",
            _release_flow_path(version),
        )
    if version == "V2.68":
        return _load_module(
            "_goal_teams_v268_release_flow",
            _release_flow_path(version),
        )
    _version_digits(version)
...
def _validate_v249_external_anchors(
    *,
    ...
    if version in {"V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}:
        ...
        expected_roots_by_version = {
            "V2.63": ["tests/v250", "tests/v263"],
            "V2.65": ["tests/v250", "tests/v265"],
            "V2.66": ["tests/v250", "tests/v266"],
            "V2.67": ["tests/v250", "tests/v267"],
            "V2.68": ["tests/v250", "tests/v268"],
        }
        if expected_test_roots != expected_roots_by_version[version]:
            raise SkillReleaseError(
                _version_error(version, "CURRENT_DENOMINATOR_EXTERNAL_ANCHOR"),
                "Current test roots differ from the release contract",
                command="preflight",
                source_commit=commit,
            )
    else:
    ...
    if (
        ...
        or (
            version in {"V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}
...
def _validate_external_anchors(version: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if version in {"V2.50", "V2.52", "V2.6", "V2.62", "V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}:
...
def validate_v250_s4_control(
    ...
    *,
    ...
    if version not in {"V2.50", "V2.52", "V2.6", "V2.62", "V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}:
        return {
            "ok": False,
            "passed": False,
...
def _checkpoint_gate_errors(
    *,
    ...
    phase_order = (
        V268_CONTINUATION_PHASE_ORDER
        if version == "V2.68"
        else V267_CONTINUATION_PHASE_ORDER
        if version == "V2.67"
...
def _v249_checkpoint_receipt_binding_errors(
    ...
    *,
    ...
    if version == "V2.68":
        handoff = runtime.get("controller_handoff_receipt", {})
        payload = handoff.get("signed_payload", {}) if isinstance(handoff, Mapping) else {}
        if payload.get("installed_predecessor_observation") != receipt_values.get("installed-predecessor-observation.json"):
            errors.append(_version_error(version, "CHECKPOINT_PREDECESSOR_BINDING"))
    return errors
...
def _v263_checkpoint_route_triplet_binding_errors(
    receipt_values: Mapping[str, Mapping[str, Any]],
    receipt_records: Mapping[str, Mapping[str, Any]],
    *,
    version: str,
) -> list[str]:
    """Bind a route-aware continuation bundle to the triplet used by S0."""

    if version not in {"V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}:
        return []
    facts_name = "release-route-facts.json"
    derived_name = "release-route-derived.json"
    closure_name = "release-route-receipt.json"
    runtime = receipt_values.get("released-runtime-transition.json", {})
    facts = receipt_values.get(facts_name, {})
    derived = receipt_values.get(derived_name, {})
    closure = receipt_values.get(closure_name, {})
    if not all(
        isinstance(value, Mapping)
        for value in (runtime, facts, derived, closure)
    ):
        return ["E_V263_CONTINUATION_ROUTE_TRIPLET_BINDING"]
    record_digests = {
        name: receipt_records.get(name, {}).get("sha256")
        for name in (facts_name, derived_name, closure_name)
    }
    if (
        runtime.get("route_facts_receipt_sha256")
        != record_digests[facts_name]
        or runtime.get("derived_route_receipt_sha256")
        != record_digests[derived_name]
        or runtime.get("route_receipt_sha256")
        != record_digests[closure_name]
        or runtime.get("project_route_facts_sha256")
        != facts.get("project_route_facts_sha256")
        or runtime.get("derived_route_sha256")
        != derived.get("receipt_sha256")
        or runtime.get("derived_route_sha256")
        != closure.get("derived_route_sha256")
        or runtime.get("route_closure_digest")
        != closure.get("closure_digest")
        or runtime.get("route_id") != derived.get("route_id")
        or runtime.get("route_id") != closure.get("route_id")
    ):
        return ["E_V263_CONTINUATION_ROUTE_TRIPLET_BINDING"]
    return []
...
def build_v249_continuation_checkpoint(
    ...
    *,
    ...
    lower = _version_lower(version)
    release_flow = _release_flow_module(version)
    formal_receipt_names = continuation_formal_receipts(version)
    ...
    for name in formal_receipt_names:
    ...
    required_receipts_present = (
        set(receipt_values) == set(formal_receipt_names)
    )
    ...
    if ready_candidate and not errors:
        ...
        errors.extend(
            _v263_checkpoint_route_triplet_binding_errors(
                receipt_values,
                observed_outputs,
                version=version,
            )
...
def validate_v249_continuation_checkpoint(
    ...
    *,
    ...
    if actual_receipt_names != set(formal_receipt_names) | {
        "_checkpoint.json"
    }:
        errors.append(_version_error(version, "CONTINUATION_RECEIPT_SET"))
    actual_receipt_records: dict[str, dict[str, Any]] = {}
    for name in formal_receipt_names:
        record = _checkpoint_file_record(receipt_root / name)
        if record is not None:
            actual_receipt_records[name] = record
    ...
    try:
        if args.command == "plan":
            ...
            checkpoint_builder = (
                ...
                if args.version in {"V2.50", "V2.52", "V2.6", "V2.62", "V2.63", "V2.65", "V2.66", "V2.67", "V2.68"}
