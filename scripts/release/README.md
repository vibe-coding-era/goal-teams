...
V2.50 开发阶段只运行 TDD 与受影响面增量检查，不进入 S0–S4。只有 exact released
...
```bash
python3 scripts/release/skill_release.py plan --version V2.50 --commit <40-hex>
...
`skill_release.py validate` 校验同一 asset set；不要对 V2.50 调用兼容命令 `verify`。
...
以下 `release.py` 与 CP00–CP18 内容是 V2.46 governed 兼容路径，不是 V2.50
...
- `release_config.py`：只加载 Git-tracked 闭集 profile；V2.50 是候选 `skill_simple` profile，V2.48 保持已安装基线直到 atomic cutover，V2.46 保留 governed replay engine。
