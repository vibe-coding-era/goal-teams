...
- `release_config.py`：只加载 Git-tracked 闭集 profile；V2.46 为 active，V2.45/V2.44/V2.40 为 historical replay 且不能执行 external write。
...
V2.46 active 主链是 `start(CP00) → promote(CP01) → doctor(CP02 前必须通过) → promote(CP02–CP08) → prepare(CP09–CP10) → promote(CP11–CP17) → close(CP18)`。`status` 是只读旁路，`recover` 是中断恢复旁路，两者都不是需要顺序执行的新 checkpoint。
