...
- `release_config.py`：只加载 Git-tracked 闭集 profile；V2.44 为 active，V2.40 为 historical replay 且不能执行 external write。
...
V2.44 固定公开资产只有 `goal-teams-V2.44.tar.gz`、`SHA256SUMS`、`_release.json` 和 `_files.sha256`。完整门禁见 `references/release-packaging-protocol.md`；顺序为 candidate exact-SHA CI → remote lock → tag → verified Draft → exact main CAS → publish-last → published asset install/audit。
...
| 阶段 | 公开命令 | 语义 |
| --- | --- | --- |
| CP00 | `"$PYTHON_BIN" scripts/release/release.py start --input <start.json>` | 从 active profile 的 `develops/v2.44` candidate 创建 state，冻结 scope/profile 并完成 CP00；state 必须写入 canonical root `docs/` |
...
V2.44 active 主链是 `start(CP00) → promote(CP01) → doctor(CP02 前必须通过) → promote(CP02–CP08) → prepare(CP09–CP10) → promote(CP11–CP17) → close(CP18)`。`status` 是只读旁路，`recover` 是中断恢复旁路，两者都不是需要顺序执行的新 checkpoint。

实际 envelope 的 `state_path` 必须是 canonical root 下 `docs/release-state/V2.44/promotion-state.json` 的绝对路径，不能写 candidate-relative `docs/...`，也不能保留未解析占位值。`start` 从 candidate worktree 运行并把 state 直接写入 canonical root `docs/`；`close` 从 canonical root 运行并读取同一 state。JSON envelope、绝对路径命令示例和 CP00–CP18 细表见 `references/release-packaging-protocol.md`。
