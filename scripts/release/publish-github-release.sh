...
[[ "$COMMON_DIR" = /* ]] || COMMON_DIR="$SOURCE_ROOT/$COMMON_DIR"
...
if git -C "$SOURCE_ROOT" rev-parse -q --verify "refs/tags/$TAG" >/dev/null; then
  ...
  git -C "$SOURCE_ROOT" tag -a "$TAG" "$COMMIT" -m "Goal Teams $VERSION"
fi
git -C "$SOURCE_ROOT" push origin "refs/tags/$TAG"
...
EVIDENCE="$WORKSPACE_ROOT/docs/archive/releases/$VERSION/release-evidence"
mkdir -p "$EVIDENCE"
