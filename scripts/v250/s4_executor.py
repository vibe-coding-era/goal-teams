#!/usr/bin/env python3
"""Execute and reconcile the V2.6 S4 publish/install contract.
...
RELEASE_PROFILE_RELATIVE = Path("references/release-profiles/v2.6.json")
...
JOURNAL_LAYOUT = (
    ("tag_local_create", "tag_local_create", "refs/tags/v2.6", "local_repository"),
    ("tag_push", "tag_push", "refs/tags/v2.6", "external_service"),
    ("release_create", "release_create", "v2.6", "external_service"),
    (
    ...
    (
        "asset_upload:goal-teams-V2.6.tar.gz",
        "asset_upload",
        "goal-teams-V2.6.tar.gz",
        "external_service",
    ),
    ("release_publish", "release_publish", "v2.6", "external_service"),
    ("install", "install", "canonical CODEX_HOME", "local_install"),
)


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.6 receipts."""
...
def _base_outcome(
    *,
    ...
    return {
        "schema_version": "goal-teams-v2.6-s4-outcome-receipt-v2",
