#!/usr/bin/env python3
"""Deterministic Google OKF conformance primitives for Goal Teams V2.50 Current.
...
def parse_okf_document(path: str | Path, *, max_bytes: int = 65536) -> dict[str, Any]:
    """Safely parse the V2.50 Current-supported OKF Markdown subset."""
...
def build_package_manifest(
    ...
    builder_version = "V2.50" if product_version == "V2.50" else "V2.39"
    runtime_path = (
        "scripts/v250/okf_conformance.py"
        if product_version == "V2.50"
    ...
    if is_package:
        if require_complete_package:
            ...
            expected_builder_version = (
                "V2.50" if installed_product_version == "V2.50" else "V2.39"
            )
            expected_top_fields = {
                "schema_version",
                "canonicalization",
