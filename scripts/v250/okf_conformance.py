#!/usr/bin/env python3
"""Deterministic Google OKF conformance primitives for Goal Teams V2.52 Current.
...
def parse_okf_document(path: str | Path, *, max_bytes: int = 65536) -> dict[str, Any]:
    """Safely parse the V2.52 Current-supported OKF Markdown subset."""
...
def build_package_manifest(
    ...
    builder_version = "V2.52" if product_version == "V2.52" else "V2.39"
    runtime_path = (
        "scripts/v250/okf_conformance.py"
        if product_version == "V2.52"
    ...
    if is_package:
        if require_complete_package:
            ...
            expected_builder_version = (
                "V2.52" if installed_product_version == "V2.52" else "V2.39"
            )
            expected_top_fields = {
                "schema_version",
                "canonicalization",
