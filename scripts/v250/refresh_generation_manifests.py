...
def _package_selected_paths() -> set[str]:
    """Expand the Current package manifest to its exact repository path set."""
...
def _refreshed_activation(
    ...
    bootstrap = _existing_paths(value, "bootstrap")
    current = _existing_paths(value, "current")
    ...
    current.update({paths["profile"].as_posix(), paths["release_profile"].as_posix()})
    ...
    schemas.update(_glob_files(f"{paths['product_schemas'].as_posix()}/*.json"))
    root_sets = {
...
def main() -> int:
    ...
    if (args.write or args.prepare_active or args.activate) and not args.base_activation_sha256:
        parser.error("--write/--prepare-active/--activate require --base-activation-sha256")
    if (args.prepare_active or args.activate) and not args.base_active_sha256:
        parser.error("--prepare-active/--activate require --base-active-sha256")
    ...
    if args.write or args.prepare_active or args.activate:
...
def _run_locked(args: argparse.Namespace, paths: dict[str, Path]) -> int:
    ...
    rule = _refreshed_rule_manifest(paths, args.generation_id)
    prompt = _refreshed_prompt_manifest(paths, args.generation_id)
    if args.generation_id == "V2.63":
        from scripts.v250.semantic_closure import validate_route_controls

        for route_id, route in prompt["routes"].items():
            workflow_phase = route.get("workflow_phase")
            if workflow_phase == "startup":
                semantic_phase = "development"
            else:
                semantic_phase = workflow_phase
    ...
    prompt["manifest_state"] = "active_current" if state == "active" else "inactive_candidate"
    activation = _refreshed_activation(
        paths, args.generation_id, args.predecessor, state, rule, prompt
    )
    expected = {
        paths["rule"]: rule,
        paths["prompt"]: prompt,
