"""V2.62 current-generation test-gate primitives.
...
TG_ERRORS = {
    "TG00": "E_V250_TG00_SCOPE",
    "TG01": "E_V250_TG01_CONTRACT",
    "TG02": "E_V250_TG02_IDENTITY",
    "TG03": "E_V250_TG03_ENVIRONMENT",
    "TG04": "E_V250_TG04_EXECUTION",
    "TG05": "E_V250_TG05_BUSINESS_ORACLE",
    "TG06": "E_V250_TG06_SIDE_EFFECT",
    "TG07": "E_V250_TG07_EVIDENCE",
    "TG08": "E_V250_TG08_REVIEW",
}
SIDE_EFFECT_FACETS = (
    "observation",
    "exact_readback",
...
def _error(*codes: str, **extra: Any) -> dict[str, Any]:
    return {
        "ok": False,
        "error_code": codes[0] if codes else "E_V250_TEST_GATE",
...
def derive_gate_plan(facts: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if size not in {"small", "medium", "large"}:
        return _error("E_V250_ROUTE_SIZE")
    if phase not in {"development", "release_readiness", "release"}:
        return _error("E_V250_WORKFLOW_PHASE")
    ...
    gates["s3"] = _gate(
        ...
        run_outcome="not_run",
        reason=None if s3_required else "not_required_by_v250_policy",
    )
    gates["s3"].update(
...
def build_tdd_chain(
    ...
    case: dict[str, Any] = {
        "schema_version": "goal-teams-test-case-v2.50",
    ...
    review: dict[str, Any] = {
        ...
        "validator_identity": {
            "validator_id": "v250-test-gate",
            "validator_digest": _text_digest("v250-test-gate"),
    ...
    return {
        "schema_version": "goal-teams-test-chain-v2.50",
...
def validate_test_chain(chain: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(denominator, dict) or not isinstance(case, dict):
        return _error("E_V250_TEST_CHAIN_SHAPE")
    ...
    if denominator.get("digest") != _canonical_digest(denominator):
        errors.extend(("E_V250_TG00_DENOMINATOR", "E_V250_DIGEST_MISMATCH"))
    if case.get("denominator_id") != denominator.get("denominator_id") or case.get(
        "denominator_digest"
    ) != denominator.get("digest"):
        errors.append("E_V250_CHAIN_BINDING")
    if case.get("case_digest") != _canonical_digest(case, "case_digest"):
        errors.extend(("E_V250_TG01_CONTRACT", "E_V250_DIGEST_MISMATCH"))

    for run in runs:
        if not isinstance(run, dict):
            errors.append("E_V250_TG04_EXECUTION")
            continue
        ...
        if run.get("case_digest") != case.get("case_digest") or run.get(
            ...
            errors.append("E_V250_TDD_CASE_DRIFT")
    ...
    if not isinstance(tg_results, list) or [item.get("tg_id") for item in tg_results] != list(TG_IDS):
        errors.append("E_V250_TG08_ASSURANCE")
    elif any(
...
def validate_current_route_closure(
    ...
    *,
    ...
    if replay_version is None and intersection:
        return _error(
            "E_V250_GENERATION_REPLAY_LEAK", legacy_intersection=intersection
        )
...
def derive_installer_route(
    ...
    *,
    ...
    return {
        ...
        "s3_process_invocation_count": 0,
        "child_argv": [],
        "not_required_reason": None if required else "not_required_by_v250_policy",
    }
...
def validate_git_transport(remote_url: str) -> dict[str, Any]:
    ...
    if not is_ssh:
        return _error("E_V250_GIT_TRANSPORT_NOT_SSH")
...
def validate_subagent_assurance(
    *,
    ...
    if reviewer_run_id in set(delivery_run_ids):
        return _error("E_V250_TG08_ASSURANCE", reason="reviewer_not_independent")
    missing = sorted(set(required_actor_assurances) - set(provided_actor_assurances))
    if missing:
        return _error("E_V250_TG08_ASSURANCE", missing_actor_assurances=missing)
