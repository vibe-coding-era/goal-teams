...
def compile_route_closure(
    ...
    if generation.get("generation_id") in {"V2.63", "V2.65", "V2.66", "V2.67"}:
        raise RouteClosureError(
...
def _compile_route_closure(
    ...
    *,
    ...
    if generation_id not in {"V2.62", "V2.63", "V2.65", "V2.66", "V2.67"}:
    ...
    loaded_paths = _ordered_paths(
        route.get("ordered_refs"), reject_duplicates=generation_id in {"V2.63", "V2.65", "V2.66", "V2.67"}
    )
...
def _validate_derived_route_receipt(
    ...
    generation_id = generation.get("generation_id")
    if (
        generation_id not in {"V2.63", "V2.65", "V2.66", "V2.67"}
