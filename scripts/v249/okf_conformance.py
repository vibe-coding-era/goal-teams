#!/usr/bin/env python3
"""Deterministic Google OKF conformance primitives for Goal Teams V2.49 Current.
...
def classify_path(path: str | Path, policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if candidate.startswith("references/legacy-replay/generations/"):
        return {
            "class": "B",
            "rule_id": "B_FROZEN_LEGACY_SNAPSHOT",
            "contract_id": "raw-byte-frozen-historical-snapshot",
        }
...
def parse_okf_document(path: str | Path, *, max_bytes: int = 65536) -> dict[str, Any]:
    """Safely parse the V2.49 Current-supported OKF Markdown subset."""
...
def build_package_manifest(
    ...
    builder_version = "V2.49" if product_version == "V2.49" else "V2.39"
    runtime_path = (
        "scripts/v249/okf_conformance.py"
        if product_version == "V2.49"
    ...
    if is_package:
        if require_complete_package:
            ...
            expected_builder_version = (
                "V2.49" if installed_product_version == "V2.49" else "V2.39"
            )
            expected_top_fields = {
                "schema_version",
                "canonicalization",
