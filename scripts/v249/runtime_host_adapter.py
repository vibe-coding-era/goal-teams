#!/usr/bin/env python3
"""External host adapter for the V2.48 -> V2.49 runtime handoff.

This module never creates or signs a controller handoff.  It accepts the
externally issued V2.48 receipt, verifies the pinned owner key, launches a
fresh V2.49 child with ``Popen``, sends the launch contract only after the
child PID exists, and verifies the child's acknowledgement.
"""
...
def validate_github_key_readback(value: object) -> dict[str, Any]:
    ...
    if len(matching) != 1:
        errors.append("E_V249_GITHUB_OWNER_KEY_MISSING")
        public_key = None
    else:
    ...
    if (
        ...
        errors.append("E_V249_GITHUB_OWNER_KEY_DRIFT")
...
def build_runtime_launch_receipt(
    *,
    ...
    if not isinstance(host_execution_id, str) or not host_execution_id:
        raise ValueError("E_V249_RUNTIME_LAUNCH_LINEAGE")
    new_run_id = f"V249-RUNTIME-{uuid.uuid4().hex}"
...
def _validate_child_ack(
    ...
    *,
    ...
    if not isinstance(ack, dict):
        raise ValueError("E_V249_RUNTIME_CHILD_ACK")
...
def launch_runtime_transition(
    *,
    ...
    if not handoff_verdict.get("ok"):
        errors = handoff_verdict.get("errors") or ["E_V249_CONTROLLER_HANDOFF_REQUIRED"]
    ...
    if not isinstance(process.pid, int) or process.pid < 1:
        raise ValueError("E_V249_RUNTIME_CHILD_PID")
    ...
    if process.returncode != 0:
        raise ValueError(
            "E_V249_RUNTIME_CHILD_FAILED:"
            + hashlib.sha256(stderr.encode("utf-8")).hexdigest()
        )
    try:
        ack = json.loads(stdout)
    except json.JSONDecodeError as exc:
        raise ValueError("E_V249_RUNTIME_CHILD_ACK") from exc
    ...
    if not verdict.get("ok") or (stage == "released" and not verdict.get("may_enter_s0")):
        raise ValueError("E_V249_RUNTIME_CHILD_RECEIPT_INVALID")
    return receipt


def _read_json(path: Path) -> object:
    if path.is_symlink() or not path.is_file():
        raise ValueError("E_V249_RUNTIME_HOST_INPUT")
