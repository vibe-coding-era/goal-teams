#!/usr/bin/env python3
"""Pure V2.49 release routing and receipt-chain helpers.
...
dedicated V2.49 runners.  A consistent receipt chain is evidence correlation;
...
from scripts.v249.runtime_transition import (
...
V249_SECURITY_REQUIRED_TARGET_PATHS = frozenset(
    {
...
def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.49 receipt bindings."""
...
def derive_release_plan(route: dict[str, Any]) -> dict[str, Any]:
    """Derive V2.49 S0-S4 without executing child processes."""
    ...
    if size != "large" or route.get("release_intent") is not True:
        ...
        gates["s3"] = {
            ...
            "blocked_code": "E_V249_S3_S1_REQUIRED",
            "check_state": "blocked",
            "run_outcome": "blocked",
...
def build_s2_receipt(
    *,
    ...
    build_run_id: str = "V249-S2-SINGLE-BUILD",
) -> dict[str, Any]:
    ...
    if {asset.get("name") for asset in asset_rows} != PUBLIC_ASSET_NAMES:
        raise ValueError("the exact V2.49 four-asset set is required")
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.49-s2-receipt-v1",
...
def validate_s2_receipt(
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        errors.append("E_V249_S2_RECEIPT_MISSING")
    if not _valid_receipt_digest(value):
        errors.append("E_V249_S2_RECEIPT_DIGEST")
    if (
        ...
        errors.append("E_V249_S2_IDENTITY_DRIFT")
    ...
    if (
        ...
        errors.append("E_V249_S2_ASSET_SET_DRIFT")
    if (
        ...
        errors.append("E_V249_S2_NOT_CURRENT")
    if (
        value.get("second_build_comparison_attempted") is not False
        or value.get("reproducibility") != "not_verified_by_v249_policy"
        or value.get("s2_security_checks") != "not_run_by_v249_policy"
        or value.get("legacy_double_build_gate_loaded") is not False
        or value.get("legacy_s2_security_gate_loaded") is not False
    ):
        errors.append("E_V249_S2_POLICY_VIOLATION")
...
def _validate_full_regression_receipt(
    ...
    if not isinstance(receipt, dict) or value.get("gate_id") != "full_regression":
        return ["E_V249_RELEASE_GATE_MISSING"]
    if not _valid_receipt_digest(value):
        errors.append("E_V249_RELEASE_GATE_DIGEST")
    if value.get("source_commit") != source_commit or value.get("source_tree") != source_tree:
        errors.append("E_V249_RELEASE_GATE_STALE")
    if (
        value.get("check_state") != "passed"
        or value.get("run_outcome") != "passed"
        or value.get("evidence_state") != "current"
        ...
        errors.append("E_V249_RELEASE_GATE_NOT_CURRENT")
    denominator = value.get("denominator")
    if not isinstance(denominator, dict):
        errors.append("E_V249_CURRENT_DENOMINATOR_MISSING")
        return errors
    files = denominator.get("test_files")
    case_count = denominator.get("test_case_count")
    if (
        denominator.get("denominator_id") != "V249-CURRENT-GENERATION-FULL"
        or denominator.get("generation_id") != "V2.49"
        ...
        or denominator.get("test_root") != "tests/v249"
        ...
        or denominator.get("legacy_roots_excluded") != ["tests/v23"]
    ...
    if (
        ...
        errors.append("E_V249_CURRENT_FULL_RUN_CONTRACT")
    return errors
...
def _security_git_bytes(*args: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_OBJECT")
...
def _security_target_path(value: object) -> str:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_PATH")
...
def _security_review_git_snapshot(
    ...
    if (
        ...
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_IDENTITY")
    manifest_bytes = _security_git_bytes(
        "show", f"{source_commit}:{V249_SECURITY_CONTRACT_PATH}"
    )
    try:
        manifest = json.loads(manifest_bytes.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_MANIFEST") from exc
    targets = manifest.get("review_targets") if isinstance(manifest, dict) else None
    if (
        not isinstance(targets, list)
        or not targets
        or manifest.get("schema_version")
        != "goal-teams-v2.49-release-security-review-v2"
        or manifest.get("denominator_id")
        != "V249-RELEASE-SECURITY-IMPLEMENTATION"
        or manifest.get("unknown_or_missing_policy") != "fail_closed"
    ):
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_MANIFEST")
    ...
    if set(target_by_path) != V249_SECURITY_REQUIRED_TARGET_PATHS:
        raise ValueError("E_V249_SECURITY_REVIEW_GIT_DENOMINATOR")
    ...
    for record in raw_tree.split(b"\x00"):
        ...
        try:
            ...
            raise ValueError("E_V249_SECURITY_REVIEW_GIT_TREE") from exc
        if path in tree_entries:
            raise ValueError("E_V249_SECURITY_REVIEW_GIT_TREE")
        tree_entries[path] = {
            "mode": mode,
            "type": kind,
            "object_id": object_id,
...
def _validate_security_review_receipt(
    ...
    if (
        ...
        errors.append("E_V249_SECURITY_REVIEWER_IDENTITY")
    if (
        ...
        errors.append("E_V249_SECURITY_ASSURANCE")
    ...
    try:
        ...
        errors.append("E_V249_SECURITY_REVIEW_DENOMINATOR")
        expected_assertions = []
    ...
    denominator_valid = bool(
        ...
        == "V249-RELEASE-SECURITY-IMPLEMENTATION"
        and denominator.get("generation_id") == "V2.49"
    ...
    if (
        ...
        errors.append("E_V249_SECURITY_REVIEW_CONTRACT")
    return errors
...
def _runtime_transition_errors(
    ...
    return list(
        dict.fromkeys(
            ["E_V249_RELEASED_RUNTIME_S0_REQUIRED"]
            + (
...
def build_s0_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.49-s0-receipt-v1",
            "gate_id": "s0_identity",
            "source_commit": source_commit,
            "source_tree": source_tree,
...
def build_s1_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.49-s1-receipt-v1",
...
def validate_project_start_authorization(
    ...
    *,
    ...
    if (
        ...
        errors.append("E_V249_AUTHORIZATION_IDENTITY_DRIFT")
    ...
    if (
        ...
        errors.append("E_V249_AUTHORIZATION_ACTION_DRIFT")
    ...
    if (
        ...
        errors.append("E_V249_AUTHORIZATION_INTENT_DRIFT")
...
def _validate_s3(
    ...
    *,
    ...
    if not isinstance(receipt, dict) or not _valid_receipt_digest(value):
        return ["E_V249_S3_RECEIPT_MISSING"]
    identity_ok = (
    ...
    if not identity_ok:
        errors.append("E_V249_S3_IDENTITY_DRIFT")
    if project_size == "large":
        if (
            ...
            errors.append("E_V249_S3_NOT_CURRENT")
    else:
        if (
            ...
            errors.append("E_V249_S3_UNEXPECTED_INVOCATION")
    return errors
...
def _validate_boundary(
    ...
    *,
    ...
    value = receipt if isinstance(receipt, dict) else {}
    if not isinstance(receipt, dict):
        return ["E_V249_REPOSITORY_BOUNDARY_MISSING"]
    errors: list[str] = []
    if not _valid_receipt_digest(value):
        errors.append("E_V249_REPOSITORY_BOUNDARY_DIGEST")
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_STALE")
    if (
        value.get("check_state") != "passed"
        or value.get("run_outcome") != "passed"
        or value.get("evidence_state") != "current"
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_NOT_CURRENT")
    return errors
...
def build_release_control_receipt(
    *,
    ...
    s4_preflight = _seal_receipt(
        {
            "schema_version": "goal-teams-v2.49-s4-preflight-receipt-v1",
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        return {
            ...
            "passed": False,
            "publish_allowed": False,
            "errors": ["E_V249_RELEASE_CONTROL_REQUIRED"],
        }
    if value.get("schema_version") != "goal-teams-v2.49-release-control-receipt-v1":
        errors.append("E_V249_RELEASE_CONTROL_SCHEMA")
    if (
        ...
        errors.append("E_V249_RELEASE_CONTROL_DIGEST")
    if (
        ...
        errors.append("E_V249_RELEASE_CONTROL_IDENTITY_DRIFT")
    ...
    if value.get("intent_sha256") != auth.get("intent_sha256"):
        errors.append("E_V249_RELEASE_CONTROL_INTENT_DRIFT")
    ...
    if (
        ...
        errors.append("E_V249_S0_NOT_CURRENT")
    ...
    if (
        ...
        errors.append("E_V249_S1_NOT_CURRENT")
    ...
    if (
        ...
        errors.append("E_V249_SAME_ASSET_INTEGRITY_RECEIPT")
    errors.extend(
        _validate_s3(
            value.get("s3"),
    ...
    if (
        ...
        != "goal-teams-v2.49-external-anchor-validation-v1"
        ...
        errors.append("E_V249_EXTERNAL_ANCHOR_REQUIRED")
    ...
    if (
        ...
        errors.append("E_V249_S4_PREFLIGHT_NOT_CURRENT")
    errors = list(dict.fromkeys(errors))
    return {
        "ok": not errors,
        "passed": not errors,
