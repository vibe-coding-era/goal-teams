"""Compile and verify a V2.49 Current route closure."""
...
from scripts.v249.generation_runtime import canonical_json_digest, resolve_repo_file, sha256_bytes
...
def _deduplicate_paths(values: Any) -> list[str]:
    if not isinstance(values, list) or not values or not all(isinstance(value, str) and value for value in values):
        raise RouteClosureError("E_V249_ROUTE_REFS", "ordered_refs must be a non-empty string array")
    result: list[str] = []
    seen: set[str] = set()
    for value in values:
...
def compile_route_closure(
    ...
    if generation.get("generation_id") != "V2.49":
        raise RouteClosureError("E_V249_ROUTE_GENERATION", "V2.49 route compiler received another generation")
    if not generation.get("activation_digest_verified") or not generation.get("member_digests_verified"):
        raise RouteClosureError("E_V249_ROUTE_UNVERIFIED_GENERATION", "generation digests are not verified")
    ...
    if not isinstance(prompt_manifest, dict) or not isinstance(rule_manifest, dict):
        raise RouteClosureError("E_V249_ROUTE_MANIFEST", "prompt and rule manifests are required")
    routes = prompt_manifest.get("routes")
    if not isinstance(routes, dict) or route_id not in routes:
        raise RouteClosureError("E_V249_ROUTE_UNKNOWN", f"unknown route: {route_id}")
    route = routes[route_id]
    if not isinstance(route, dict):
        raise RouteClosureError("E_V249_ROUTE_SHAPE", f"invalid route object: {route_id}")

    loaded_paths = _deduplicate_paths(route.get("ordered_refs"))
    allowlist = generation.get("current_default_allowlist", [])
    if not isinstance(allowlist, list):
        raise RouteClosureError("E_V249_ROUTE_ALLOWLIST", "generation current allowlist is invalid")
    allowset = set(allowlist)
    unmanaged = sorted(path for path in loaded_paths if path not in allowset)
    if unmanaged:
        raise RouteClosureError("E_V249_ROUTE_UNMANAGED", "route loads unmanaged paths: " + ", ".join(unmanaged))
    ...
    for relative_path in loaded_paths:
        ...
        try:
            ...
            raise RouteClosureError("E_V249_ROUTE_NON_TEXT", f"rule file is not UTF-8: {relative_path}") from exc
    ...
    if legacy_intersection:
        raise RouteClosureError(
            "E_V249_ROUTE_LEGACY_REACHABLE",
            "Current route reaches Legacy: " + ", ".join(legacy_intersection),
        )

    owners = rule_manifest.get("owners")
    if not isinstance(owners, list):
        raise RouteClosureError("E_V249_RULE_INDEX", "rule manifest owners must be an array")
    ...
    for owner in owners:
        if not isinstance(owner, dict) or not isinstance(owner.get("path"), str):
            raise RouteClosureError("E_V249_RULE_OWNER", "invalid rule owner")
        owner_path = owner["path"]
        if owner_path in owner_by_path:
            raise RouteClosureError("E_V249_RULE_OWNER_DUPLICATE", f"duplicate owner path: {owner_path}")
    ...
    for path in loaded_paths:
        ...
        if owner is None:
            raise RouteClosureError("E_V249_ROUTE_WITHOUT_OWNER", f"route ref has no rule owner: {path}")
        if route_id not in owner.get("route_membership", []):
            raise RouteClosureError(
                "E_V249_ROUTE_MEMBERSHIP_DRIFT",
        ...
        if observed_digests[path] != expected_source:
            raise RouteClosureError("E_V249_OWNER_DIGEST_DRIFT", f"owner digest differs: {path}")
    ...
    if not budgets or loaded_bytes > min(budgets):
        raise RouteClosureError("E_V249_ROUTE_BUDGET", f"route {route_id} uses {loaded_bytes} bytes")
