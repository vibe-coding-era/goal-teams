...
from scripts.v249.generation_runtime import (
    GenerationLoadError,
...
def load_replay_manifest(repo_root: Path | str) -> dict[str, Any]:
    ...
    try:
        ...
        raise GenerationLoadError("E_V249_REPLAY_MANIFEST_JSON", "legacy replay manifest is invalid") from exc
    if not isinstance(manifest, dict):
        raise GenerationLoadError("E_V249_REPLAY_MANIFEST_SHAPE", "legacy replay manifest must be an object")
    ...
    if not isinstance(expected_raw_digest, str) or sha256_bytes(raw) != expected_raw_digest:
        raise GenerationLoadError(
            "E_V249_REPLAY_ACTIVE_BINDING",
            "legacy replay manifest raw digest differs from the ACTIVE activation binding",
        )
    if manifest.get("schema_version") != "goal-teams-legacy-replay-manifest-v2.49":
        raise GenerationLoadError("E_V249_REPLAY_SCHEMA", "unsupported replay manifest schema")
    if manifest.get("network_policy") != "deny" or manifest.get("write_policy") != "deny_all":
        raise GenerationLoadError("E_V249_REPLAY_ISOLATION", "replay must deny network and writes")
    if manifest.get("output_status_enum") != list(ALLOWED_STATUSES):
        raise GenerationLoadError("E_V249_REPLAY_OUTPUT", "replay status enum differs")
    ...
    if isolation != {
        ...
        raise GenerationLoadError("E_V249_REPLAY_ISOLATION", "replay isolation profile differs")
    ...
    if not isinstance(allowlist, list) or not all(isinstance(item, str) for item in allowlist):
        raise GenerationLoadError("E_V249_REPLAY_ALLOWLIST", "optional replay allowlist is invalid")
    if len(allowlist) != len(set(allowlist)):
        raise GenerationLoadError("E_V249_REPLAY_ALLOWLIST", "optional replay allowlist has duplicates")
    observed = canonical_json_digest(sorted(allowlist))
    if observed != manifest.get("optional_replay_allowlist_digest"):
        raise GenerationLoadError("E_V249_REPLAY_ALLOWLIST_DRIFT", "optional replay allowlist digest differs")
    if observed != generation.get("optional_replay_allowlist_digest"):
        raise GenerationLoadError(
            "E_V249_REPLAY_ALLOWLIST_ACTIVE_DRIFT",
    ...
    if not isinstance(replays, list) or not replays:
        raise GenerationLoadError("E_V249_REPLAY_EMPTY", "replay manifest has no entries")
    ...
    if len(versions) != len(replays) or len(versions) != len(set(versions)):
        raise GenerationLoadError("E_V249_REPLAY_VERSION", "replay versions must be unique objects")
    return manifest
...
def run_replay(
    ...
    *,
    ...
    if explicit_intent is not True and identified_artifact is not True:
        result["errors"].append("E_V249_REPLAY_EXPLICIT_INTENT_REQUIRED")
        return result
    try:
    ...
    if entry is None:
        result["errors"].append("E_V249_REPLAY_VERSION_UNAVAILABLE")
        return result
    if operation not in entry.get("supported_read_only_operations", []):
        result["errors"].append("E_V249_REPLAY_OPERATION_FORBIDDEN")
        return result
    if entry.get("network_policy") != "deny" or entry.get("write_policy") != "deny_all":
        result["errors"].append("E_V249_REPLAY_ISOLATION")
        return result
    if entry.get("isolation_profile") != "digest_only_no_subprocess":
        result["errors"].append("E_V249_REPLAY_SUBPROCESS_FORBIDDEN")
        return result
    ...
    if not isinstance(content_digests, list) or not content_digests:
        result["errors"].append("E_V249_REPLAY_CONTENT_EMPTY")
        return result
    seen: set[str] = set()
    for item in content_digests:
        if not isinstance(item, dict) or not isinstance(item.get("path"), str):
            result["errors"].append("E_V249_REPLAY_CONTENT_INVALID")
            continue
        relative_path = item["path"]
        if relative_path in seen:
            result["errors"].append("E_V249_REPLAY_CONTENT_DUPLICATE")
            continue
        seen.add(relative_path)
        if relative_path not in allowset:
            result["errors"].append("E_V249_REPLAY_PATH_UNMANAGED")
            continue
        try:
        ...
        if observed != item.get("sha256"):
            result["errors"].append("E_V249_REPLAY_CONTENT_DRIFT")
