#!/usr/bin/env python3
"""Observe and validate V2.49 fresh-runtime transition receipts.
...
HANDOFF_SCHEMA_VERSION = "goal-teams-v2.49-controller-handoff-receipt-v1"
LAUNCH_SCHEMA_VERSION = "goal-teams-v2.49-runtime-launch-receipt-v1"
CHILD_ACK_SCHEMA_VERSION = "goal-teams-v2.49-runtime-child-ack-v1"
...
HANDOFF_SIGNATURE_NAMESPACE = "goal-teams-v2.49-controller-handoff"
ACTIVE_PATH = "references/current/ACTIVE.json"
POLICY_PROFILE_PATH = "references/profiles/goal-teams-self-release-v2.49.md"
RELEASE_PROFILE_PATH = "references/release-profiles/v2.49.json"
...
ROUTE_BY_STAGE_AND_SIZE = {
    ("candidate", "discussion"): "V249-ROUTE-DISCUSSION",
    ("candidate", "small"): "V249-ROUTE-SMALL-DEVELOPMENT",
    ("candidate", "medium"): "V249-ROUTE-MEDIUM-DEVELOPMENT",
    ("candidate", "large"): "V249-ROUTE-LARGE-DEVELOPMENT",
    # V2.49 has no separate Small Release prompt route.  Small Release uses
    # the stricter Medium Release prompt closure rather than inventing one.
    ("released", "small"): "V249-ROUTE-MEDIUM-RELEASE",
    ("released", "medium"): "V249-ROUTE-MEDIUM-RELEASE",
    ("released", "large"): "V249-ROUTE-LARGE-RELEASE",
}
...
def _repo_file(root: Path, relative: str) -> Path:
    if (
        ...
        raise ValueError("E_V249_RUNTIME_TRANSITION_PATH")
    ...
    for part in pure.parts:
        ...
        if cursor.is_symlink():
            raise ValueError(f"E_V249_RUNTIME_TRANSITION_INPUT:{relative}")
    try:
        candidate.resolve().relative_to(root.resolve())
    except ValueError as exc:
        raise ValueError("E_V249_RUNTIME_TRANSITION_PATH") from exc
...
def _evidence_file(path: Path | str, root: Path) -> Path:
    ...
    if candidate.is_symlink() or not candidate.is_file():
        raise ValueError("E_V249_RUNTIME_TRANSITION_EVIDENCE_PATH")
...
def _read_json_file(path: Path) -> tuple[dict[str, Any], bytes]:
    ...
    if not isinstance(value, dict):
        raise ValueError("E_V249_RUNTIME_TRANSITION_JSON")
    return value, raw
...
def _runtime_transition_field_contract(root: Path) -> frozenset[str]:
    ...
    try:
        ...
        raise ValueError("E_V249_RUNTIME_TRANSITION_SCHEMA_CONTRACT") from exc
...
def _activation_member_digests(activation: Mapping[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(root_sets, dict):
        raise ValueError("E_V249_RUNTIME_TRANSITION_ACTIVATION")
    result: dict[str, str] = {}
...
def _normalize_timestamp(value: str | None) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise ValueError("E_V249_RUNTIME_TRANSITION_CAPTURED_AT")
    try:
        parsed = dt.datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ValueError("E_V249_RUNTIME_TRANSITION_CAPTURED_AT") from exc
...
def _load_route_context(
    *,
    ...
    if (
        ...
        or active.get("generation_id") != "V2.49"
        or active.get("state") != "active_current"
    ):
        raise ValueError("E_V249_RUNTIME_TRANSITION_ACTIVE")
    ...
    if active.get("activation_manifest_sha256") != activation_digest:
        raise ValueError("E_V249_RUNTIME_TRANSITION_ACTIVE_DIGEST")
    if (
        activation.get("schema_version")
        != "goal-teams-activation-manifest-v2.49"
        or activation.get("generation_id") != "V2.49"
    ...
    if (
        ...
        != "goal-teams-project-route-v2.49"
        or identity.get("target_policy_generation") != "V2.49"
        or "controller_product_version" in identity
    ):
        raise ValueError("E_V249_RUNTIME_TRANSITION_VERSION_AXIS")
    ...
    if not isinstance(prompt_manifest_path, str):
        raise ValueError("E_V249_RUNTIME_TRANSITION_PROMPT_MANIFEST")
    prompt_manifest, prompt_manifest_raw = _read_repo_json(root, prompt_manifest_path)
    if (
        prompt_manifest.get("schema_version")
        != "goal-teams-prompt-manifest-v2.49"
        or prompt_manifest.get("generation_id") != "V2.49"
    ...
    if expected_route_id is None or route_receipt.get("route_id") != expected_route_id:
        raise ValueError("E_V249_RUNTIME_TRANSITION_ROUTE")
    ...
    if (
        route_receipt.get("generation_id") != "V2.49"
        ...
        raise ValueError("E_V249_RUNTIME_TRANSITION_ROUTE_RECEIPT")
    ...
    for relative in expected_current_paths:
        ...
        if (
            ...
            raise ValueError("E_V249_RUNTIME_TRANSITION_CURRENT_DIGEST")
    ...
    for relative in sorted(required_paths):
        ...
        if member_digests.get(relative) != observed:
            raise ValueError(f"E_V249_RUNTIME_TRANSITION_MEMBER_DIGEST:{relative}")
...
def _load_authorization(
    ...
    if (
        ...
        != "goal-teams-project-start-authorization-v2.49"
        ...
        raise ValueError("E_V249_RUNTIME_TRANSITION_AUTHORIZATION")
...
def _validation_clock(value: dt.datetime | None) -> dt.datetime:
    ...
    if observed.tzinfo is None or observed.utcoffset() is None:
        raise ValueError("E_V249_CONTROLLER_HANDOFF_TIME")
...
def _verify_handoff_signature(
    ...
    """Verify the V2.48 handoff with the one pinned owner SSH public key."""

    if not isinstance(signature, str) or not signature.strip():
        return False
    try:
        with tempfile.TemporaryDirectory(prefix="goal-teams-v249-handoff-") as directory:
            temp = Path(directory)
...
def validate_controller_handoff(
    ...
    *,
    ...
    """Validate the externally issued, host-signed V2.48 handoff."""
    ...
    _append_if(
        ...
        "E_V249_CONTROLLER_HANDOFF_SCHEMA",
    )
    payload = value.get("signed_payload")
    if not isinstance(payload, dict):
        errors.append("E_V249_CONTROLLER_HANDOFF_SCHEMA")
        payload = {}
    ...
    _append_if(
        ...
        "E_V249_CONTROLLER_HANDOFF_IDENTITY_DRIFT",
    )
    _append_if(
        errors,
    ...
    _append_if(
        ...
        "E_V249_CONTROLLER_HANDOFF_AUTHORIZATION_DRIFT",
    )

    installed = payload.get("installed_v248_current_state")
    ...
    _append_if(
        ...
        "E_V249_CONTROLLER_HANDOFF_SIGNER_DRIFT",
    )
    ...
    _append_if(
        ...
        "E_V249_CONTROLLER_HANDOFF_PAYLOAD_DIGEST",
    )
    now = _validation_clock(validation_time)
    try:
        issued = _parse_timestamp(payload.get("issued_at"), "E_V249_CONTROLLER_HANDOFF_TIME")
        expires = _parse_timestamp(
            payload.get("expires_at"), "E_V249_CONTROLLER_HANDOFF_TIME"
        )
        if issued >= expires:
            errors.append("E_V249_CONTROLLER_HANDOFF_TIME")
        if now < issued:
            errors.append("E_V249_CONTROLLER_HANDOFF_TIME_DRIFT")
        if now >= expires:
            errors.append("E_V249_CONTROLLER_HANDOFF_EXPIRED")
    except ValueError as exc:
        errors.append(str(exc))

    if not _verify_handoff_signature(payload, str(value.get("ssh_signature", ""))):
        errors.append("E_V249_CONTROLLER_HANDOFF_SIGNATURE")
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        return {
            "ok": False,
            "passed": False,
            "errors": ["E_V249_RUNTIME_LAUNCH_REQUIRED"],
        }
    value = receipt
    errors: list[str] = []
    expected_fields = {
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_LAUNCH_ADAPTER",
    )
    try:
        launched = _parse_timestamp(
            value.get("launched_at"), "E_V249_RUNTIME_LAUNCH_TIME"
        )
        issued = _parse_timestamp(
            handoff_payload.get("issued_at"), "E_V249_RUNTIME_LAUNCH_TIME"
        )
        expires = _parse_timestamp(
            handoff_payload.get("expires_at"), "E_V249_RUNTIME_LAUNCH_TIME"
        )
        if launched < issued or launched >= expires:
            errors.append("E_V249_RUNTIME_LAUNCH_TIME")
    except ValueError as exc:
        errors.append(str(exc))
    _append_if(
        errors,
...
def observe_transition(
    *,
    ...
    if stage not in {"candidate", "released"}:
        raise ValueError("E_V249_RUNTIME_TRANSITION_STAGE")
    if project_size not in {"discussion", "small", "medium", "large"}:
        raise ValueError("E_V249_RUNTIME_TRANSITION_PROJECT_SIZE")
    ...
    if COMMIT_RE.fullmatch(source_commit) is None or COMMIT_RE.fullmatch(source_tree) is None:
        raise ValueError("E_V249_RUNTIME_TRANSITION_IDENTITY")
    if not isinstance(adapter_identity, str) or not adapter_identity.strip():
        raise ValueError("E_V249_RUNTIME_TRANSITION_ADAPTER_IDENTITY")
    if not isinstance(controller_handoff_receipt, dict):
        raise ValueError("E_V249_CONTROLLER_HANDOFF_REQUIRED")
    if not isinstance(runtime_launch_receipt, dict):
        raise ValueError("E_V249_RUNTIME_LAUNCH_REQUIRED")
    ...
    receipt: dict[str, Any] = {
        ...
        "transition_id": transition_id or f"V249-TRANSITION-{uuid.uuid4().hex}",
        "stage": stage,
...
def validate_transition(
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        errors.append("E_V249_RUNTIME_TRANSITION_RECEIPT")
    try:
        ...
        _append_if(
            errors,
            set(value) != expected_fields,
            "E_V249_RUNTIME_TRANSITION_SCHEMA_FIELDS",
        )
    _append_if(
        errors,
        value.get("schema_version")
        != "goal-teams-v2.49-runtime-transition-receipt-v1",
        "E_V249_RUNTIME_TRANSITION_SCHEMA",
    )
    _append_if(
        errors,
        any(
        ...
        "E_V249_RUNTIME_TRANSITION_RAW_LINEAGE",
    )
    if value.get("fresh_process_observed") is not True:
        errors.append("E_V249_FRESH_RUNTIME_REQUIRED")
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_TRANSITION_PROCESS",
    )
    _append_if(
        errors,
        value.get("stage") != expected_stage,
        "E_V249_RUNTIME_TRANSITION_STAGE",
    )
    _append_if(
        errors,
        value.get("generation_id") != "V2.49"
        or value.get("loaded_runtime_product_version")
        != LOADED_RUNTIME_PRODUCT_VERSION,
        "E_V249_RUNTIME_TRANSITION_VERSION_AXIS",
    )
    _append_if(
        errors,
        value.get("receipt_state") != "current",
        "E_V249_RUNTIME_TRANSITION_STALE",
    )
    _append_if(
        errors,
        ...
        "E_V249_RUNTIME_TRANSITION_ASSURANCE",
    )
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_TRANSITION_IDENTITY",
    )
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_TRANSITION_PROJECT_SIZE",
    )
    try:
        _parse_timestamp(
            value.get("captured_at"), "E_V249_RUNTIME_TRANSITION_CAPTURED_AT"
        )
    except ValueError as exc:
        errors.append(str(exc))
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_TRANSITION_DIGEST",
    )
    _append_if(
        errors,
        ...
        "E_V249_RUNTIME_TRANSITION_LOADED_PATHS",
    )
    _append_if(
        errors,
        ...
        "E_V249_RUNTIME_TRANSITION_CURRENT_DIGEST",
    )
    expected_aliases = {
    ...
    if isinstance(digests, dict):
        _append_if(
            ...
            "E_V249_RUNTIME_TRANSITION_APPROVED_PROMPT_DIGEST",
        )
    ...
    try:
        ...
        if not route_evidence_current:
            errors.append("E_V249_RUNTIME_TRANSITION_ROUTE_RECEIPT_DIGEST")
    ...
    try:
        ...
        if not route_evidence_current or "ROUTE" in code:
            ...
            errors.append(
                code
                if code.startswith("E_V249_")
                else "E_V249_RUNTIME_TRANSITION_CURRENT_INPUT"
            )
    if observed_context is not None:
        _append_if(
            ...
            "E_V249_RUNTIME_TRANSITION_ROUTE_RECEIPT_DIGEST",
        )
        _append_if(
            errors,
    ...
    try:
        ...
        errors.append("E_V249_RUNTIME_TRANSITION_AUTHORIZATION_RECEIPT_DIGEST")
    ...
    try:
        ...
        _append_if(
            ...
            "E_V249_RUNTIME_TRANSITION_ADAPTER_CODE_DIGEST",
        )
    except (OSError, ValueError):
        errors.append("E_V249_RUNTIME_TRANSITION_ADAPTER_CODE_DIGEST")

    handoff = value.get("controller_handoff_receipt")
    if not isinstance(handoff, dict):
        errors.append("E_V249_CONTROLLER_HANDOFF_REQUIRED")
        handoff = {}
    else:
        _append_if(
            errors,
            ...
            "E_V249_CONTROLLER_HANDOFF_DIGEST",
        )
        handoff_verdict = validate_controller_handoff(
            handoff,
            expected_repository=REPOSITORY,
    ...
    if not isinstance(launch, dict):
        errors.append("E_V249_RUNTIME_LAUNCH_REQUIRED")
        launch = {}
    else:
        _append_if(
            errors,
    ...
    _append_if(
        ...
        "E_V249_RUNTIME_TRANSITION_RECEIPT_DIGEST",
    )
    deduplicated = list(dict.fromkeys(errors))
    ok = not deduplicated
    return {
...
def main() -> int:
    ...
    try:
        if not args.child:
            raise ValueError("E_V249_RUNTIME_CHILD_MODE_REQUIRED")
        envelope = json.loads(sys.stdin.read())
        if not isinstance(envelope, dict) or set(envelope) != {
            "controller_handoff_receipt",
            "runtime_launch_receipt",
        }:
            raise ValueError("E_V249_RUNTIME_STDIN_RECEIPTS_REQUIRED")
