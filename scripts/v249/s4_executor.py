#!/usr/bin/env python3
"""Execute and reconcile the V2.49 S4 publish/install contract.
...
TAG = "v2.49"
TITLE = "Goal Teams V2.49"
CANONICAL_REMOTE = "git@github.com:vibe-coding-era/goal-teams.git"
CANONICAL_RELEASE_URL = (
    "https://github.com/vibe-coding-era/goal-teams/releases/tag/v2.49"
)
RELEASE_PROFILE_RELATIVE = Path("references/release-profiles/v2.49.json")
SCHEMA_PATH = ROOT / "schemas" / "v2.49" / "release-control.schema.json"
RUNTIME_TRANSITION_SCHEMA_PATH = (
    ROOT / "schemas" / "v2.49" / "runtime-transition-receipt.schema.json"
)
RUNTIME_TRANSITION_SCHEMA_ID = (
    "https://goal-teams.local/schemas/v2.49/"
    "runtime-transition-receipt.schema.json"
)
SCHEMA_REF = (
    "https://goal-teams.local/schemas/v2.49/release-control.schema.json"
...
JOURNAL_LAYOUT = (
    ...
    ("tag_push", "tag_push", "refs/tags/v2.49", "external_service"),
    ("release_create", "release_create", "v2.49", "external_service"),
    (
    ...
    (
        "asset_upload:goal-teams-V2.49.tar.gz",
        "asset_upload",
        "goal-teams-V2.49.tar.gz",
        "external_service",
    ),
    ("release_publish", "release_publish", "v2.49", "external_service"),
    ("install", "install", "canonical CODEX_HOME", "local_install"),
)


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.49 receipts."""
...
def _load_module(name: str, path: Path) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise S4ExecutionError("E_V249_S4_MODULE")
...
def _load_release_profile_contract(repository_root: Path) -> dict[str, str]:
    ...
    try:
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_RELEASE_PROFILE")
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise S4ExecutionError("E_V249_S4_RELEASE_PROFILE") from exc
    if (
...
class OperationJournal:
    ...
    def _entry(self, step_id: str) -> dict[str, Any]:
        for entry in self._entries:
            if entry["step_id"] == step_id:
                return entry
        raise S4ExecutionError("E_V249_S4_JOURNAL_STEP")

    def attempt(self, step_id: str) -> None:
        entry = self._entry(step_id)
        if entry["attempted_this_run"]:
            raise S4ExecutionError("E_V249_S4_JOURNAL_REPLAY")
    ...
    def observe(self, step_id: str, observed: str) -> None:
        if observed not in {"absent", "exact", "conflict", "unavailable"}:
            raise S4ExecutionError("E_V249_S4_JOURNAL_OBSERVATION")
        entry = self._entry(step_id)
...
class Draft202012OutcomeValidator:
    ...
    def __init__(
        ...
        try:
            ...
            raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA_UNAVAILABLE") from exc
        if (
            ...
            raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA_INVALID")
        if (
        ...
        if importlib.util.find_spec("jsonschema") is not None:
            try:
                ...
                raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA_INVALID") from exc
        node = shutil.which("node")
        if node is None:
            raise S4ExecutionError("E_V249_S4_SCHEMA_VALIDATOR_UNAVAILABLE")
        self._node = node
    ...
    def _run_ajv(
        ...
        if result.returncode != 0:
            raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA")
    ...
    def validate(self, receipt: Mapping[str, Any]) -> None:
        if self._python_validator is not None:
            try:
                ...
                raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA") from exc
            return
...
def _parse_http_json(result: CommandResult) -> tuple[int, dict[str, object]]:
    ...
    if match is None:
        raise S4ExecutionError("E_V249_S4_GITHUB_RESPONSE")
    separator = normalized.find("\n\n")
    body = normalized[separator + 2 :] if separator >= 0 else ""
    try:
        payload = json.loads(body) if body.strip() else {}
    except json.JSONDecodeError as exc:
        raise S4ExecutionError("E_V249_S4_GITHUB_RESPONSE") from exc
...
def _parse_annotated_tag(
    ...
    if not separator:
        raise S4ExecutionError("E_V249_S4_TAG_NOT_ANNOTATED")
    values: dict[str, str] = {}
    ...
    if (
        ...
        if normalized_message != TITLE:
            raise S4ExecutionError("E_V249_S4_TAG_MESSAGE_DRIFT")
        raise S4ExecutionError("E_V249_S4_TAG_NOT_ANNOTATED")
    return {
        "tag": expected_tag,
...
class CommandBackend:
    ...
    def _require_repository(repository: str) -> None:
        if repository != REPOSITORY:
            raise S4ExecutionError("E_V249_S4_REPOSITORY_TARGET")
    ...
    def read_fetch_remote(self) -> str:
        return _checked(
            ...
            cwd=self.root,
            code="E_V249_S4_GIT_REMOTE_READ",
        ).stdout.strip()
    ...
    def create_annotated_tag(self, tag: str, commit: str, title: str) -> None:
        if tag != TAG or title != TITLE or COMMIT_RE.fullmatch(commit) is None:
            raise S4ExecutionError("E_V249_S4_TAG_CREATE_INPUT")
        _checked(
            self.runner,
            ["git", "tag", "-a", tag, commit, "-m", title],
            cwd=self.root,
            code="E_V249_S4_TAG_CREATE",
        )
    ...
    def read_tag(self, tag: str) -> dict[str, object] | None:
        ...
        for line in result.stdout.splitlines():
            ...
            if len(parts) != 2 or COMMIT_RE.fullmatch(parts[0]) is None:
                raise S4ExecutionError("E_V249_S4_TAG_READBACK")
    ...
    def read_release(
        ...
        if result.returncode != 0 or status != 200:
            raise S4ExecutionError("E_V249_S4_RELEASE_READBACK")
        return payload
    ...
    def create_draft_release(
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_RELEASE_CREATE_INPUT")
        _checked(
            self.runner,
            [
                "gh",
            ...
            cwd=self.root,
            code="E_V249_S4_RELEASE_CREATE",
        )

    def upload_asset(self, repository: str, tag: str, path: Path) -> None:
        self._require_repository(repository)
        if path.name not in CANONICAL_ASSET_NAMES:
            raise S4ExecutionError("E_V249_S4_ASSET_UPLOAD_INPUT")
        _checked(
            self.runner,
            [
                "gh",
    ...
    def publish_release(self, repository: str, tag: str) -> None:
        ...
        _checked(
            ...
            cwd=self.root,
            code="E_V249_S4_RELEASE_PUBLISH",
        )
    ...
    def download_asset(
        ...
        if name not in CANONICAL_ASSET_NAMES:
            raise S4ExecutionError("E_V249_S4_ASSET_DOWNLOAD_INPUT")
        ...
        _checked(
            ...
            cwd=self.root,
            code="E_V249_S4_ASSET_DOWNLOAD",
        )
        if not target.is_file() or target.is_symlink():
            raise S4ExecutionError("E_V249_S4_ASSET_DOWNLOAD")
    ...
    def read_installed_state(self) -> dict[str, object] | None:
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_INSTALLED_STATE")
        try:
            value = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise S4ExecutionError("E_V249_S4_INSTALLED_STATE") from exc
    ...
    def read_installed_version(self) -> str | None:
        ...
        if not path.is_file() or path.is_symlink():
            raise S4ExecutionError("E_V249_S4_INSTALLED_VERSION")
        try:
            return path.read_text(encoding="utf-8").strip()
        except (OSError, UnicodeDecodeError) as exc:
            raise S4ExecutionError("E_V249_S4_INSTALLED_VERSION") from exc
    ...
    def _file_record(path: Path, relative: str) -> dict[str, object]:
        try:
            ...
            raise S4ExecutionError("E_V249_S4_INSTALLED_PACKAGE_DRIFT") from exc
        if (
            ...
            code = (
                "E_V249_S4_INSTALLED_PACKAGE_SYMLINK"
                if stat.S_ISLNK(metadata.st_mode)
                else "E_V249_S4_INSTALLED_PACKAGE_DRIFT"
            )
            raise S4ExecutionError(code)
        ...
        try:
            with os.fdopen(descriptor, "rb", closefd=True) as handle:
                ...
                if (
                    ...
                    raise S4ExecutionError(
                        "E_V249_S4_INSTALLED_PACKAGE_DRIFT"
                    )
                for chunk in iter(lambda: handle.read(1024 * 1024), b""):
                    digest.update(chunk)
    ...
    def _walk_regular_tree(root: Path) -> tuple[list[dict[str, object]], set[str]]:
        try:
            ...
            raise S4ExecutionError("E_V249_S4_INSTALLED_PACKAGE_DRIFT") from exc
        if stat.S_ISLNK(metadata.st_mode):
            raise S4ExecutionError("E_V249_S4_INSTALLED_PACKAGE_SYMLINK")
        if not stat.S_ISDIR(metadata.st_mode):
            raise S4ExecutionError("E_V249_S4_INSTALLED_PACKAGE_DRIFT")
    ...
    def verify_installed_payload(
        ...
        if stored != package_files:
            raise S4ExecutionError("E_V249_S4_INSTALLED_STATE_MANIFEST_DRIFT")
        ...
        self._reject_symlink_ancestors(
            skill_root, "E_V249_S4_INSTALLED_PACKAGE_SYMLINK"
        )
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_INSTALLED_AGENT_DRIFT")
        package_by_path = {
        ...
        for name in sorted(hashes):
            ...
            self._reject_symlink_ancestors(
                path, "E_V249_S4_INSTALLED_AGENT_SYMLINK"
            )
            try:
                record = self._file_record(path, ".")
            except S4ExecutionError as exc:
                if exc.code == "E_V249_S4_INSTALLED_PACKAGE_SYMLINK":
                    raise S4ExecutionError(
                        "E_V249_S4_INSTALLED_AGENT_SYMLINK"
                    ) from exc
                raise S4ExecutionError("E_V249_S4_INSTALLED_AGENT_DRIFT") from exc
...
def _validate_local_assets(
    *,
    ...
    if actual_root != expected_root:
        raise S4ExecutionError("E_V249_S4_RELEASE_ROOT")
    paths = _asset_paths(actual_root, version)
    if tuple(paths) != CANONICAL_ASSET_NAMES:
        raise S4ExecutionError("E_V249_S4_ASSET_SET")
    assets: list[dict[str, object]] = []
    for name, path in paths.items():
        try:
            metadata = path.lstat()
        except OSError as exc:
            raise S4ExecutionError("E_V249_S4_ASSET_SET") from exc
        if (
            not stat.S_ISREG(metadata.st_mode)
            or metadata.st_nlink != 1
    ...
    if expected_assets != assets or control.get("asset_set_digest") != canonical_sha256(assets):
        raise S4ExecutionError("E_V249_S4_LOCAL_ASSET_DRIFT")
    try:
        record = json.loads(paths["_release.json"].read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise S4ExecutionError("E_V249_S4_RELEASE_RECORD") from exc
    if (
        ...
        raise S4ExecutionError("E_V249_S4_RELEASE_RECORD")
...
def _default_prewrite_boundary_validator(
    *,
    ...
    if not all(isinstance(value, dict) for value in (boundary, s2, integrity)):
        raise S4ExecutionError("E_V249_S4_REPOSITORY_BOUNDARY_REQUIRED")
    ...
    if (
        ...
        raise S4ExecutionError("E_V249_S4_REPOSITORY_BOUNDARY_COMMANDS")
    module = _load_module(
        "_goalteams_v249_repository_boundary_for_s4",
        repository_root / "scripts/v249/repository_boundary.py",
    )
    try:
        ...
        if original_verdict.get("ok") is not True:
            ...
            raise ValueError(code or "E_V249_REPOSITORY_BOUNDARY_NOT_CURRENT")
        local_validation = {
            "passed": True,
        ...
        if re.fullmatch(r"E_[A-Z0-9_:-]+", code) is None:
            code = "E_V249_S4_REPOSITORY_BOUNDARY_REVALIDATION"
        raise S4ExecutionError(code) from exc
    comparable = (
        "source_commit",
        "source_tree",
    ...
    if (
        ...
        raise S4ExecutionError("E_V249_S4_REPOSITORY_BOUNDARY_REVALIDATION")
    return {
        "ok": True,
        "passed": True,
...
def _validate_release_identity(
    ...
    *,
    ...
    if (
        ...
        raise S4ExecutionError("E_V249_S4_RELEASE_DRIFT")
    observed: dict[str, dict[str, object]] = {}
    for raw in value["assets"]:
        if not isinstance(raw, dict) or not isinstance(raw.get("name"), str):
            raise S4ExecutionError("E_V249_S4_REMOTE_ASSET_DRIFT")
        name = raw["name"]
    ...
    if require_complete and set(observed) != set(expected_assets):
        raise S4ExecutionError("E_V249_S4_REMOTE_ASSET_DRIFT")
    if value.get("html_url") != CANONICAL_RELEASE_URL:
        raise S4ExecutionError("E_V249_S4_RELEASE_DRIFT")
...
def _download_one_exact(
    *,
    ...
    if (
        ...
        raise S4ExecutionError("E_V249_S4_DOWNLOAD_DRIFT")
    return {
        "name": name,
        "asset_id": remote["id"],
        "size": metadata.st_size,
...
def _verify_remote_asset(
    *,
    ...
    if remote is None:
        raise S4ExecutionError("E_V249_S4_REMOTE_ASSET_ABSENT")
    with tempfile.TemporaryDirectory(prefix="goal-teams-v249-asset-readback-") as temp:
        _download_one_exact(
            backend=backend,
            name=name,
            expected=local_assets[name],
...
def _parse_release_file_manifest(path: Path) -> list[dict[str, object]]:
    try:
        ...
        raise S4ExecutionError("E_V249_S4_RELEASE_FILES") from exc
    if not data.endswith(b"\n"):
        raise S4ExecutionError("E_V249_S4_RELEASE_FILES")
...
def _validate_release_file_binding(
    ...
    if (
        ...
        raise S4ExecutionError("E_V249_S4_RELEASE_FILES_BINDING")
                ...
                if step_id == "tag_local_create":
                    ...
                    if release is None:
                        ...
                        if name not in remote:
                            ...
                            with tempfile.TemporaryDirectory(
                                prefix="goal-teams-v249-reconcile-asset-"
                            ) as temp:
                                _download_one_exact(
                                    backend=backend,
                                    name=name,
...
def validate_outcome_receipt(
    ...
    if receipt.get("schema_validation") != selected.identity:
        raise S4ExecutionError("E_V249_S4_OUTCOME_SCHEMA_IDENTITY")
    _assert_outcome_invariants(receipt)
...
def _base_outcome(
    *,
    ...
    return {
        "schema_version": "goal-teams-v2.49-s4-outcome-receipt-v2",
    ...
    *,
    ...
    if validator is None:
        ...
        receipt["schema_validation"] = {
            ...
            "error_code": validator_error or "E_V249_S4_SCHEMA_VALIDATOR_UNAVAILABLE",
        }
        receipt["receipt_sha256"] = canonical_sha256(receipt)
        return receipt
    try:
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_CONTROL_IDENTITY")
        ...
        if not isinstance(repository_identity, dict):
            raise S4ExecutionError("E_V249_S4_AUTHORIZATION")
        ...
        if expected_fetch != CANONICAL_REMOTE or expected_push != CANONICAL_REMOTE:
            raise S4ExecutionError("E_V249_S4_AUTHORIZATION_REMOTE")
        ...
        if (
            ...
            raise S4ExecutionError("E_V249_S4_GIT_TRANSPORT_NOT_SSH")
        ...
        with tempfile.TemporaryDirectory(prefix="goal-teams-v249-s4-") as temp:
            work = Path(temp)
            ...
            if (
                ...
                raise S4ExecutionError("E_V249_S4_INSTALLED_IDENTITY_DRIFT")
            if exact_before:
                ...
                if not _installed_state_matches(
                    ...
                    raise S4ExecutionError("E_V249_S4_INSTALLED_IDENTITY_READBACK")
        ...
        code = (
            ...
            if isinstance(raw, S4ExecutionError)
                ...
                "E_V249_S4_RUNTIME_OSERROR"
                if isinstance(raw, OSError)
                else "E_V249_S4_RUNTIME_EXCEPTION"
            )
        )
        ...
        validator_error = (
            ...
            in {
                "E_V249_S4_SCHEMA_VALIDATOR_UNAVAILABLE",
                "E_V249_S4_OUTCOME_SCHEMA_UNAVAILABLE",
            }
            else None
        )
