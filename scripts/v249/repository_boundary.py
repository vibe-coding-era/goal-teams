#!/usr/bin/env python3
"""Run and validate the V2.49 repository-boundary integrity gate.
...
def _load_module(name: str, path: Path) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise ValueError(f"E_V249_BOUNDARY_MODULE:{path.name}")
    ...
    *,
    ...
    for key, argv in commands.items():
        ...
        if result.returncode != 0:
            raise ValueError("E_V249_REPOSITORY_BOUNDARY_SOURCE_READBACK")
    ...
    if (
        ...
        raise ValueError("E_V249_REPOSITORY_BOUNDARY_SOURCE_NOT_FROZEN")
    return {
        "check_state": "passed",
        "revalidated_now": True,
...
def resolve_release_directory(
    ...
    *,
    ...
    release_directory = observed_root / "V2.49"
    if observed_root != expected_root or not release_directory.is_dir():
        raise ValueError("E_V249_REPOSITORY_BOUNDARY_RELEASE_ROOT")
...
def boundary_contract_digests(
    *, repository_root: Path | None = None
    ...
    validator_paths = (
        ...
        root / "scripts/v249/repository_boundary.py",
    )
    if (
        ...
        raise ValueError("E_V249_REPOSITORY_BOUNDARY_VALIDATOR")
    validator_rows = [
        {
...
def build_boundary_receipt(
    *,
    ...
    receipt: dict[str, Any] = {
        "schema_version": "goal-teams-v2.49-repository-boundary-receipt-v1",
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        return {
            "ok": False,
            "passed": False,
            "errors": ["E_V249_REPOSITORY_BOUNDARY_MISSING"],
            "may_enter_s4": False,
        }
    errors: list[str] = []
    expected = {
    ...
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_EXPECTATION")
    ...
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_CLAIM_SCOPE")
    ...
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_RECEIPT_MODE")
    ...
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_SOURCE_NOT_FROZEN")
    ...
    if (
        ...
        errors.append("E_V249_REPOSITORY_BOUNDARY_COMMANDS")
...
def run_repository_boundary(
    *,
    ...
    release_flow = _load_module(
        "_goalteams_v249_boundary_release_flow",
    ...
    if (
        ...
        raise ValueError("E_V249_SAME_ASSET_INTEGRITY_RECEIPT")
    ...
    if not isinstance(public_asset_sources, dict) or not public_asset_sources:
        raise ValueError("E_V249_REPOSITORY_BOUNDARY_ASSET_SOURCES")
    for source in public_asset_sources.values():
        ...
        try:
            ...
            raise ValueError("E_V249_REPOSITORY_BOUNDARY_ASSET_SOURCES") from exc
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(s2, dict):
            raise ValueError("E_V249_S2_RECEIPT_MISSING")
        ...
        if args.reuse_receipt is not None:
            ...
            if not isinstance(reused, dict):
                raise ValueError("E_V249_REPOSITORY_BOUNDARY_MISSING")
