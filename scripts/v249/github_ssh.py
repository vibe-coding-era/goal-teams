#!/usr/bin/env python3
"""Fail-closed GitHub SSH transport helpers for V2.49.
...
def validate_github_ssh_remote(remote_url: object) -> dict[str, Any]:
    ...
    if not isinstance(remote_url, str):
        return {
            "ok": False,
            "passed": False,
            "error_code": "E_V249_GITHUB_SSH_REMOTE",
        }
    ...
    if match is None:
        return {
            "ok": False,
            "passed": False,
            "error_code": "E_V249_GITHUB_SSH_ONLY",
...
def build_push_argv(remote_name: str, refspec: str) -> list[str]:
    ...
    if REMOTE_NAME_RE.fullmatch(remote_name) is None:
        raise ValueError("E_V249_GIT_REMOTE_NAME")
    if REFSPEC_RE.fullmatch(refspec) is None or refspec.startswith("-"):
        raise ValueError("E_V249_GIT_REFSPEC")
...
def github_operation_surface(operation: str) -> str:
    ...
    raise ValueError("E_V249_GITHUB_OPERATION")
