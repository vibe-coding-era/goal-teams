"""Fail-closed V2.65 compatibility metadata and receipt handling."""
...
SCHEMA_VERSION = "goal-teams-compatibility-v2.65-v1"
RECEIPT_SCHEMA_VERSION = "goal-teams-runtime-binding-v2.65-v1"
...
class CompatibilityError(ValueError):
    """Stable V2.65 compatibility validation failure."""
...
def _string_list(value: Any, field: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) and item for item in value):
        raise CompatibilityError("E_V265_LIST", f"{field} must be a non-empty string array")
    if len(value) != len(set(value)):
        raise CompatibilityError("E_V265_LIST_DUPLICATE", f"{field} contains duplicates")
    return list(value)


def _safe_path(value: Any) -> str:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise CompatibilityError("E_V265_PATH_INVALID", "node path must be a non-empty POSIX relative path")
    path = PurePosixPath(value)
    if path.is_absolute() or any(part in {"", ".", ".."} for part in path.parts):
        raise CompatibilityError("E_V265_PATH_ESCAPE", f"unsafe node path: {value!r}")
    return value
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CompatibilityError("E_V265_JSON", f"cannot load metadata: {path}") from exc
    if not isinstance(value, dict):
        raise CompatibilityError("E_V265_SHAPE", "metadata root must be an object")
    return value


def load_compatibility_metadata(path: Path | str) -> dict[str, Any]:
    """Load typed V2.65 metadata without resolving filesystem references."""

    value = _load_json(Path(path))
    if value.get("schema_version") != SCHEMA_VERSION:
        raise CompatibilityError("E_V265_SCHEMA", "unsupported compatibility metadata schema")
    nodes = value.get("nodes")
    if not isinstance(nodes, list) or not nodes:
        raise CompatibilityError("E_V265_NODES", "nodes must be a non-empty array")
    node_ids: set[str] = set()
    validated_nodes: list[dict[str, Any]] = []
    for node in nodes:
        if not isinstance(node, dict):
            raise CompatibilityError("E_V265_NODE", "node must be an object")
        node_id = node.get("id")
        if not isinstance(node_id, str) or not node_id:
            raise CompatibilityError("E_V265_NODE_ID", "node id must be a non-empty string")
        if node_id in node_ids:
            raise CompatibilityError("E_V265_NODE_ID_DUPLICATE", f"duplicate node id: {node_id}")
        kind = node.get("kind")
        if not isinstance(kind, str) or kind not in NODE_KINDS:
            raise CompatibilityError("E_V265_NODE_KIND", f"unsupported node kind: {kind!r}")
        state = node.get("state")
        if not isinstance(state, str) or not state:
            raise CompatibilityError("E_V265_NODE_STATE", f"node {node_id} lacks explicit state")
        validated = dict(node)
        ...
        if any(dependency not in node_ids for dependency in validated["depends_on"]):
            raise CompatibilityError("E_V265_DEPENDENCY_ORDER", f"node {node_id} depends on a later or unknown node")
        node_ids.add(node_id)
        validated_nodes.append(validated)
    routes = value.get("routes")
    if not isinstance(routes, list) or not routes:
        raise CompatibilityError("E_V265_ROUTES", "routes must be a non-empty array")
    route_keys: set[tuple[str, str]] = set()
    validated_routes: list[dict[str, Any]] = []
    for route in routes:
        if not isinstance(route, dict):
            raise CompatibilityError("E_V265_ROUTE", "route must be an object")
        ...
        if not isinstance(host_id, str) or not isinstance(requested_model, str):
            raise CompatibilityError("E_V265_ROUTE_IDENTITY", "route host and requested model are required")
        key = (host_id, requested_model)
        if key in route_keys:
            raise CompatibilityError("E_V265_ROUTE_DUPLICATE", f"duplicate route: {host_id}/{requested_model}")
        ...
        if route_refs[0] != "portable-core" or host_id not in route_refs:
            raise CompatibilityError("E_V265_ROUTE_ORDER", "route must start at portable-core and include its host")
        if any(ref not in node_ids for ref in route_refs):
            raise CompatibilityError("E_V265_ROUTE_REF", "route has an unknown node reference")
        ...
        if connection_class not in {"direct_responses", "direct_anthropic", "bridge_required", "unsupported_direct"}:
            raise CompatibilityError("E_V265_CONNECTION_CLASS", "unsupported connection class")
        verification_state = route.get("verification_state")
        if verification_state not in VERIFICATION_STATES:
            raise CompatibilityError("E_V265_VERIFICATION_STATE", "unsupported verification state")
        if route.get("resolved_model") != requested_model:
            raise CompatibilityError("E_V265_MODEL_REWRITE", "metadata must not silently rewrite a requested model")
        if connection_class in {"bridge_required", "unsupported_direct"} and verification_state != "blocked":
            raise CompatibilityError("E_V265_BLOCKED_STATE", "non-direct route must remain blocked without approved runtime evidence")
        route_keys.add(key)
...
def _route(metadata: dict[str, Any], host_id: str, requested_model: str) -> dict[str, Any]:
    for route in metadata["routes"]:
        if route["host_id"] == host_id and route["requested_model"] == requested_model:
            return dict(route)
    raise CompatibilityError("E_V265_ROUTE_UNKNOWN", f"no route for {host_id}/{requested_model}")
...
def validate_runtime_binding_receipt(receipt: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    if receipt.get("schema_version") != RECEIPT_SCHEMA_VERSION:
        return {"ok": False, "errors": ["E_V265_RECEIPT_SCHEMA"]}
    try:
    ...
    if (
        ...
        errors.append("E_V265_RECEIPT_ROUTE_DRIFT")
