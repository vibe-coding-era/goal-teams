#!/usr/bin/env python3
"""Execute and reconcile the V2.67 S4 publish/install contract.
...
TAG = "v2.67"
TITLE = "Goal Teams V2.67"
CANONICAL_REMOTE = "git@github.com:vibe-coding-era/goal-teams.git"
CANONICAL_RELEASE_URL = (
    "https://github.com/vibe-coding-era/goal-teams/releases/tag/v2.67"
)
DRAFT_RELEASE_URL_RE = re.compile(
    r"^https://github\.com/vibe-coding-era/goal-teams/releases/tag/"
    r"untagged-[0-9a-f]{20,64}$"
)
RELEASE_PROFILE_RELATIVE = Path("references/release-profiles/v2.67.json")
SCHEMA_PATH = ROOT / "schemas" / "v2.67" / "release-control.schema.json"
RUNTIME_TRANSITION_SCHEMA_PATH = (
    ROOT / "schemas" / "v2.67" / "runtime-transition-receipt.schema.json"
)
RUNTIME_TRANSITION_SCHEMA_ID = (
    "https://goal-teams.local/schemas/v2.67/"
    "runtime-transition-receipt.schema.json"
)
SCHEMA_REF = (
    "https://goal-teams.local/schemas/v2.67/release-control.schema.json"
...
JOURNAL_LAYOUT = (
    ("tag_local_create", "tag_local_create", "refs/tags/v2.67", "local_repository"),
    ("tag_push", "tag_push", "refs/tags/v2.67", "external_service"),
    ("release_create", "release_create", "v2.67", "external_service"),
    (
    ...
    (
        "asset_upload:goal-teams-V2.67.tar.gz",
        "asset_upload",
        "goal-teams-V2.67.tar.gz",
        "external_service",
    ),
    ("release_publish", "release_publish", "v2.67", "external_service"),
    ("install", "install", "canonical CODEX_HOME", "local_install"),
)


def derive_resume_operation_ids(
    journal: Sequence[Mapping[str, object]],
) -> tuple[str, ...]:
    """Return only canonical S4 operations lacking exact confirmation.

    The persisted journal must contain the exact canonical sequence.  Missing,
    unknown, reordered, duplicate, or invalid-state entries fail closed so an
    ambiguous receipt cannot widen the resume scope.  Confirmed operations are
    never selected for replay.
    """

    canonical_ids = tuple(step_id for step_id, *_ in JOURNAL_LAYOUT)
    states: dict[str, object] = {}
    observed_ids: list[str] = []
    for entry in journal:
        if not isinstance(entry, Mapping):
            raise ValueError("E_V267_S4_RESUME_JOURNAL")
        step_id = entry.get("step_id")
        if (
            not isinstance(step_id, str)
            or step_id not in canonical_ids
            or step_id in states
        ):
            raise ValueError("E_V267_S4_RESUME_JOURNAL")
        state = entry.get("state")
        if state not in {"not_started", "attempted", "confirmed", "uncertain"}:
            raise ValueError("E_V267_S4_RESUME_JOURNAL")
        observed_ids.append(step_id)
        states[step_id] = state
    if tuple(observed_ids) != canonical_ids:
        raise ValueError("E_V267_S4_RESUME_JOURNAL")
    return tuple(
        step_id for step_id in canonical_ids if states.get(step_id) != "confirmed"
    )


def _prepare_formal_install_temp_root(repository_root: Path) -> Path:
    """Create one repository-owned, symlink-free S4 install temp root."""

    try:
        if repository_root.is_symlink():
            raise OSError("repository root is a symlink")
        root = repository_root.resolve(strict=True)
        if not root.is_dir():
            raise OSError("repository root is not a directory")
        develops = root / "develops"
        if not develops.exists():
            os.mkdir(develops, 0o700)
        for path in (root, develops):
            metadata = os.lstat(path)
            if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
                raise OSError("unsafe temp-root ancestor")
        target = develops / ".v267-s4-tmp"
        ...
        if (
            ...
            raise OSError("unsafe install temp root")
        if target.resolve(strict=True) != target:
            raise OSError("install temp root resolves elsewhere")
        nofollow = getattr(os, "O_NOFOLLOW", None)
        if nofollow is None:
            raise OSError("O_NOFOLLOW is unavailable")
        probe = target / f".probe-{os.getpid()}-{secrets.token_hex(8)}"
        descriptor = os.open(
            probe,
        ...
        raise S4ExecutionError("E_V267_S4_INSTALL_TMPDIR_UNSAFE") from exc


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.67 receipts."""
...
class CommandBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self.install_temp_root = install_temp_root
    ...
    def install(self, bundle: Path, identity_path: Path) -> None:
        temp_root = _prepare_formal_install_temp_root(self.root)
        if self.install_temp_root is not None and self.install_temp_root != temp_root:
            raise S4ExecutionError("E_V267_S4_INSTALL_TMPDIR_UNSAFE")
        _checked(
            self.runner,
            [
                "/usr/bin/env",
...
def _default_prewrite_boundary_validator(
    *,
    ...
    module = _load_module(
        ...
        repository_root / "scripts/v267/repository_boundary.py",
    )
    try:
...
def _verify_remote_asset(
    *,
    ...
    readback_temp_root = _prepare_formal_install_temp_root(repository_root)
    if readback_temp_root != expected_temp_root:
        raise S4ExecutionError("E_V267_S4_INSTALL_TMPDIR_UNSAFE")
    with tempfile.TemporaryDirectory(
        prefix="goal-teams-v267-asset-readback-", dir=readback_temp_root
    ) as temp:
        _download_one_exact(
            backend=backend,
            name=name,
...
def _base_outcome(
    *,
    ...
    return {
        "schema_version": "goal-teams-v2.67-s4-outcome-receipt-v2",
...
def execute_s4(
    *,
    ...
    try:
        ...
        install_temp_root = _prepare_formal_install_temp_root(root)
        context["install_temp_root"] = str(install_temp_root)
        if isinstance(selected_backend, CommandBackend):
            selected_backend.install_temp_root = install_temp_root
        ...
        for name in CANONICAL_ASSET_NAMES:
            ...
            release_value, remote_assets = _verify_remote_asset(
                ...
                release_body=release_body,
                repository_root=root,
                expected_temp_root=install_temp_root,
            )
            context["remote_assets"] = remote_assets
        ...
        download_temp_root = _prepare_formal_install_temp_root(root)
        if download_temp_root != install_temp_root:
            raise S4ExecutionError("E_V267_S4_INSTALL_TMPDIR_UNSAFE")
        with tempfile.TemporaryDirectory(
            prefix="goal-teams-v267-s4-", dir=download_temp_root
        ) as temp:
            work = Path(temp)
