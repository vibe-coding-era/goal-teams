"""Project and verify thin host-role assets from V2.66 canonical role metadata."""
...
PLAN_PATH = Path("references/compatibility/v2.66/role-projections.json")


def _safe_path(value: Any) -> str:
    if not isinstance(value, str) or not value or "\\" in value:
        raise ValueError("E_V266_PROJECTION_PATH_INVALID")
...
def _load_plan(root: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("E_V266_PROJECTION_PLAN_INVALID") from exc
    if not isinstance(plan, dict) or plan.get("schema_version") != "goal-teams-compatibility-v2.66-role-projections-v1":
        raise ValueError("E_V266_PROJECTION_PLAN_INVALID")
...
def _render(target: dict[str, Any], role: dict[str, Any], portable_core_ref: str) -> str:
    ...
    if not isinstance(role_id, str) or not role_id:
        raise ValueError("E_V266_PROJECTION_ROLE_INVALID")
    if format_name == "toml":
        return f'name = "{role_id}"\ndescription = "V2.66 thin compatibility projection for {role_id}"\ncanonical_ref = "{canonical_ref}"\nportable_core_ref = "{portable_core_ref}"\n'
    if format_name == "markdown":
        return f"---\ntype: Goal Teams Host Role Projection\ntitle: V2.66 {role_id} Claude Code Projection\ndescription: Thin Claude Code projection for the canonical {role_id} compatibility role.\ntimestamp: 2026-08-07T00:00:00+08:00\nokf_version: \"0.1\"\nname: {role_id}\ncanonical_ref: {canonical_ref}\nportable_core_ref: {portable_core_ref}\n---\n\n# {role_id}\n\nThin V2.66 host projection; canonical role content is loaded from `canonical_ref`.\n"
    raise ValueError(f"E_V266_PROJECTION_FORMAT:{path}")
    ...
    for role in plan["roles"]:
        ...
        for target in role["targets"]:
            if not isinstance(target, dict):
                raise ValueError("E_V266_PROJECTION_TARGET_INVALID")
            path = _safe_path(target.get("path"))
            if path in expected:
                raise ValueError("E_V266_PROJECTION_TARGET_DUPLICATE")
...
def check_role_projections(root: Path | str) -> dict[str, Any]:
    ...
    for relative, rendered in expected.items():
        path = root_path / relative
        if not path.is_file(): errors.append("E_V266_PROJECTION_MISSING")
        elif path.read_text(encoding="utf-8") != rendered: errors.append("E_V266_PROJECTION_DRIFT")
    if actual_candidates - set(expected): errors.append("E_V266_PROJECTION_ORPHAN")
