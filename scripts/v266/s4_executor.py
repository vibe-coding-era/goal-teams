#!/usr/bin/env python3
"""Execute and reconcile the V2.66 S4 publish/install contract.
...
RELEASE_PROFILE_RELATIVE = Path("references/release-profiles/v2.66.json")
SCHEMA_PATH = ROOT / "schemas" / "v2.66" / "release-control.schema.json"
RUNTIME_TRANSITION_SCHEMA_PATH = (
    ROOT / "schemas" / "v2.66" / "runtime-transition-receipt.schema.json"
)
RUNTIME_TRANSITION_SCHEMA_ID = (
    "https://goal-teams.local/schemas/v2.66/"
    "runtime-transition-receipt.schema.json"
)
SCHEMA_REF = (
    "https://goal-teams.local/schemas/v2.66/release-control.schema.json"
...
JOURNAL_LAYOUT = (
    ("tag_local_create", "tag_local_create", "refs/tags/v2.66", "local_repository"),
    ("tag_push", "tag_push", "refs/tags/v2.66", "external_service"),
    ("release_create", "release_create", "v2.66", "external_service"),
    (
    ...
    (
        "asset_upload:goal-teams-V2.66.tar.gz",
        "asset_upload",
        "goal-teams-V2.66.tar.gz",
        "external_service",
    ),
    ("release_publish", "release_publish", "v2.66", "external_service"),
...
def derive_resume_operation_ids(
    ...
    for entry in journal:
        if not isinstance(entry, Mapping):
            raise ValueError("E_V266_S4_RESUME_JOURNAL")
...
def _prepare_formal_install_temp_root(repository_root: Path) -> Path:
    ...
    try:
        ...
        raise S4ExecutionError("E_V266_S4_INSTALL_TMPDIR_UNSAFE") from exc


def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.66 receipts."""
...
class CommandBackend:
    ...
    def install(self, bundle: Path, identity_path: Path) -> None:
        ...
        if self.install_temp_root is not None and self.install_temp_root != temp_root:
            raise S4ExecutionError("E_V266_S4_INSTALL_TMPDIR_UNSAFE")
        _checked(
            self.runner,
            [
                "/usr/bin/env",
...
def _default_prewrite_boundary_validator(
    *,
    ...
    module = _load_module(
        ...
        repository_root / "scripts/v266/repository_boundary.py",
    )
    try:
...
def _verify_remote_asset(
    *,
    ...
    if readback_temp_root != expected_temp_root:
        raise S4ExecutionError("E_V266_S4_INSTALL_TMPDIR_UNSAFE")
    with tempfile.TemporaryDirectory(
        prefix="goal-teams-v266-asset-readback-", dir=readback_temp_root
    ) as temp:
        _download_one_exact(
            backend=backend,
            name=name,
...
def _base_outcome(
    *,
    ...
    return {
        "schema_version": "goal-teams-v2.66-s4-outcome-receipt-v2",
    ...
    *,
    ...
    try:
        ...
        if download_temp_root != install_temp_root:
            raise S4ExecutionError("E_V266_S4_INSTALL_TMPDIR_UNSAFE")
        with tempfile.TemporaryDirectory(
            prefix="goal-teams-v266-s4-", dir=download_temp_root
        ) as temp:
            work = Path(temp)
