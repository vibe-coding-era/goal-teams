#!/usr/bin/env python3
"""Pure V2.66 release routing and receipt-chain helpers.
...
dedicated V2.66 runners.  A consistent receipt chain is evidence correlation;
...
from scripts.v266.runtime_transition import (
...
CURRENT_RELEASE_VERSION = "V2.66"
SECURITY_REVIEW_RUNNER_PATH = "scripts/checks/run-v266-release-security-review.py"
PUBLIC_ASSET_NAMES = {
...
V266_SECURITY_CORE_TARGET_PATHS = frozenset(
    {
...
def _load_security_required_target_paths() -> frozenset[str]:
    """Load the activation-bound V2.66 review denominator fail closed.
    ...
    if (
        ...
        != "goal-teams-v2.66-release-security-review-v2"
    ...
    if not V266_SECURITY_CORE_TARGET_PATHS.issubset(result):
...
def canonical_sha256(value: Any) -> str:
    """Return the canonical JSON SHA-256 used by V2.66 receipt bindings."""
...
def derive_release_plan(route: dict[str, Any]) -> dict[str, Any]:
    """Derive V2.66 S0-S4 without executing child processes."""
...
def build_s2_receipt(
    *,
    ...
    if {asset.get("name") for asset in asset_rows} != PUBLIC_ASSET_NAMES:
        raise ValueError("the exact V2.66 four-asset set is required")
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.66-s2-receipt-v1",
...
def _validate_full_regression_receipt(
    ...
    if (
        ...
        or denominator.get("generation_id") != "V2.66"
        ...
        or denominator.get("test_roots") != ["tests/v250", "tests/v266"]
...
def _validate_security_review_receipt(
    ...
    denominator_valid = bool(
        ...
        == "V250-RELEASE-SECURITY-IMPLEMENTATION"
        and denominator.get("generation_id") == "V2.66"
        ...
        == "references/current/generations/V2.66/contracts/release-security-review-manifest.json"
...
def build_s0_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.66-s0-receipt-v1",
            "gate_id": "s0_identity",
            "source_commit": source_commit,
            "source_tree": source_tree,
...
def build_s1_receipt(
    *,
    ...
    return _seal_receipt(
        {
            "schema_version": "goal-teams-v2.66-s1-receipt-v1",
...
def build_release_control_receipt(
    *,
    ...
    s4_preflight = _seal_receipt(
        {
            "schema_version": "goal-teams-v2.66-s4-preflight-receipt-v1",
...
def validate_release_control_receipt(
    ...
    *,
    ...
    if value.get("schema_version") != "goal-teams-v2.66-release-control-receipt-v1":
    ...
    if (
        ...
        != "goal-teams-v2.66-external-anchor-validation-v1"
